...
def test_data_version_detects_in_place_category_edit(client, seed_user, auth_headers):
    ...
    same = client.get(f"/api/data?knownVersion={v1}", headers=auth_headers).get_json()
    ...
    after = client.get(f"/api/data?knownVersion={v1}", headers=auth_headers).get_json()
...
def test_sync_trip_days_cannot_inject_into_foreign_trip(
    ...
    with get_db() as conn:
        n = conn.execute(
            "SELECT COUNT(*) AS n FROM trip_days WHERE id = ?",
            ("evil-day",),
        ).fetchone()["n"]
...
def test_sync_trip_days_owner_can_write_own_including_same_batch_new_trip(
    ...
    with get_db() as conn:
        row = conn.execute(
            "SELECT trip_id FROM trip_days WHERE id = ?",
            ("own-day",),
        ).fetchone()
...
def test_sync_archived_expense_loop_blocks_cross_trip_hijack(
    ...
    client.post(
        ...
        json={
            "expense": {
                "id": "exp-archived-victim",
                "tripId": "trip-archived-victim",
                "who": "Owner",
                "value": 250,
                "currency": "EUR",
                "euroValue": 250,
...
def test_user_data_delete_wipes_auth_sessions_and_feed(
    ...
    with get_db() as conn:
        ...
        assert conn.execute(
            "SELECT 1 FROM feed_likes WHERE user_id = ?",
            (seed_user,),
        ).fetchone()
        assert conn.execute(
    ...
    with get_db() as conn:
        ...
        assert not conn.execute(
            "SELECT 1 FROM feed_comments WHERE user_id = ?",
            (seed_user,),
        ).fetchone()
        assert not conn.execute(
            "SELECT 1 FROM auth_sessions WHERE user_id = ?",
...
def test_delete_user_data_wipes_cross_user_followed_you_notifications(
    ...
    dangling = [
        n
        for n in listed["notifications"]
        if n["type"] == "followed_you" and n["related_id"] == seed_user
    ]
    assert not dangling, (
        "follower-keyed notification pointing at a now-deleted user must be swept on account delete"
    )
...
def test_sync_writes_archived_trip_with_expenses(client, seed_user, auth_headers):
    ...
    body = pull.get_json()
    archived = next(
        (t for t in body["trips"] if t["id"] == "trip-archived-1"),
        None,
    )
...
def test_sync_does_not_delete_omitted_budget_ids(client, seed_user, auth_headers):
    ...
    assert (
        _seed_budget_via_post(
            client, auth_headers, "b-drop", label="Drop me", categoryId="c-x"
        ).status_code
        == 200
    )
...
def test_sync_empty_list_does_not_clear_budgets(client, seed_user, auth_headers):
    ...
    assert (
        _seed_budget_via_post(client, auth_headers, "b-wipe-2", categoryId="c-y").status_code == 200
    )
    ...
    assert {"b-wipe-1", "b-wipe-2"} <= ids, "/api/sync budgets:[] must no longer wipe budgets"
...
def test_sync_absent_budgets_preserves(client, seed_user, auth_headers):
    ...
    assert (
        _seed_budget_via_post(client, auth_headers, "b-preserve-2", categoryId="c-z").status_code
        == 200
    )
...
def test_sync_without_coverUrl_preserves_stored_cover(client, seed_user, auth_headers):
    ...
    with get_db() as conn:
        conn.execute(
            "UPDATE trips SET cover_url = ? WHERE id = 'tc'",
    ...
    trip = next(
        t
        for t in client.get("/api/data", headers=auth_headers).get_json()["trips"]
        if t["id"] == "tc"
    )
...
def test_sync_rejects_external_coverUrl(client, seed_user, auth_headers):
    ...
    with get_db() as conn:
        conn.execute(
            "UPDATE trips SET cover_url = ? WHERE id = 'tc2'",
