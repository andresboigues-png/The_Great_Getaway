...
def test_sync_cannot_create_dup_scope_budget(client, auth_headers, seed_user):
    ...
    assert (
        _post_budget(
            client, auth_headers, _budget("dup-a", categoryId="cat-x", user="Alice")
        ).status_code
        == 200
    )
    second = _post_budget(client, auth_headers, _budget("dup-b", categoryId="cat-x", user="Alice"))
...
def test_sync_nonmember_cannot_scope_to_trip(client, seed_user, third_user):
    ...
    assert (
        _post_budget(
            client, outsider_headers, _budget("out-b", tripId=trip_id, amount=300)
        ).status_code
        == 403
    )
