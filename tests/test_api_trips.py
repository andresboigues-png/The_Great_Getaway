...
def test_upsert_trip_preserves_cover_when_payload_omits_it(client, seed_user, auth_headers):
    ...
    seed = client.post(
        ...
        headers=auth_headers,
        json={
            "trip": {"id": "trip-cov", "name": "T", "country": "FR", "coverUrl": own_cover},
        },
    )
...
def test_upsert_trip_clears_cover_on_explicit_null(client, seed_user, auth_headers):
    ...
    seed = client.post(
        ...
        headers=auth_headers,
        json={
            "trip": {"id": "trip-cov2", "name": "T", "country": "FR", "coverUrl": own_cover},
        },
    )
    assert seed.status_code == 200, seed.get_data(as_text=True)
    client.post(
...
def test_upsert_trip_rejects_external_or_foreign_cover_url(client, seed_user, auth_headers):
    ...
    seed = client.post(
        ...
        headers=auth_headers,
        json={
            "trip": {"id": "trip-cov3", "name": "T", "country": "FR", "coverUrl": own_cover},
        },
    )
    assert seed.status_code == 200, seed.get_data(as_text=True)
    ...
    for bad in (
        ...
        res = client.post(
            ...
            headers=auth_headers,
            json={
                "trip": {"id": "trip-cov3", "name": "T", "country": "FR", "coverUrl": bad},
            },
        )
...
def test_trip_stale_clientUpdatedAt_returns_409(
    ...
    assert found["name"] == "Florence 2", "stale write should not have overwritten the second write"
...
def test_accept_invite_clears_invitee_trip_invite_notification(
    ...
    assert _invite_notif_count(seed_other_user, trip_id) == 1, (
        "invite should create the notification"
    )
    res = client.post(
        "/api/trips/invite/respond",
        headers=other_auth_headers,
...
def test_invite_trip_member_does_not_demote_accepted_planner(
    ...
    with get_db() as conn:
        ...
        assert row["role"] == "planner", f"accepted planner was demoted to {row['role']!r}"
...
def _seed_share_source(trip_id):
    ...
    with get_db() as conn:
        ...
        c.execute(
            "INSERT INTO trip_days (id, trip_id, day_number, name, morning) VALUES (?, ?, ?, ?, ?)",
...
def test_share_clone_respects_plan_and_wishlist_privacy(
    ...
    token = client.post(
        "/api/trips/share-src/share",
        headers=auth_headers,
        json={"showCost": False, "showPlans": False},
    ).get_json()["token"]

    new_id = client.post(
        f"/api/share/{token}/clone",
        headers=other_auth_headers,
    ).get_json()["tripId"]
    ...
    with get_db() as conn:
        ...
        mp = c.execute(
            "SELECT marked_places_json FROM trips WHERE id = ?",
            (new_id,),
...
def test_clone_drops_uploaded_cover_keeps_shared(client, seed_user, auth_headers):
    ...
    def _clone_with_cover(src_id, cover):
        ...
        with get_db() as conn:
            return conn.execute(
                "SELECT cover_url FROM trips WHERE id = ?",
                (new_id,),
            ).fetchone()["cover_url"]
    ...
    assert (
        _clone_with_cover(
            "cover-upl-src",
            f"/static/uploads/{seed_user}/personal.jpg",
        )
        is None
    )
...
def test_trip_countries_json_check_rejects_malformed_writes(
