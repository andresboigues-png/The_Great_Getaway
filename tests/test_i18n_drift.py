...
LOCALES_DIR = (
    Path(__file__).resolve().parent.parent / 'frontend' / 'static' / 'js' / 'src' / 'locales'
)
...
def test_no_unallowlisted_english_leaks(en, locale):
    ...
    leaks = [
        k
        for k, v in en.items()
        if k in other and other[k] == v and len(v) > 3 and not re.fullmatch(r"[\W\d]+", v)
    ]
...
def test_locale_has_every_key_en_has(en, locale):
    ...
    assert not missing, f"{locale}.ts is missing {len(missing)} key(s) from en.ts: {missing[:10]}"
