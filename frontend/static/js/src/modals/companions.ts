 ...
 *   - Existing unlinked entry → "Link" action → friend pop-up (a modal
 *     stacked on top of this one, so the choice is focused) → promotes
 *     the entry and fires the trip invite.
 *
...
export const openCompanionPickerModal = (tripId: string) => {
    ...
    const buildRow = (c: import('../types').Companion) => {
        ...
        // Unlinked → "Link" (opens the stacked friend pop-up, which includes
        // a "This is me" row); self-linked → "Unlink" (clear the link, keep the
    ...
    /** Friend-picker POP-UP — a second modal stacked ON TOP of the
     *  companions modal (Modal.ts supports stacking; closes top-down) so
     *  picking a person is a focused choice instead of an inline sheet
     *  competing with the roster + add-section for attention. Two modes:
     *    - `linkTargetName` set → "link this existing name": title is
     *      linkSheetTitle and a "This is me" row is offered.
     *    - `linkTargetName` null → plain "Add a friend": friendSheetTitle,
     *      no self row (a fresh add shouldn't re-add the owner).
     *  The parent roster refreshes on EVERY close (successful link, ✕,
     *  backdrop, Esc, hardware back) via onClose, so the list underneath
     *  is always current. */
    const openFriendPickerPopup = (linkTargetName: string | null) => {
        const { root: popupRoot, close: closePopup } = showModal({
            variant: 'glass-light',
            cardStyle: 'width: min(460px, calc(100vw - 24px)); max-width: 100%; max-height: 70vh; display: flex; flex-direction: column;',
            innerHTML: `
                <div style="display: flex; align-items: center; justify-content: space-between; gap: var(--space-3); margin-bottom: var(--space-3);">
                    <h2 style="margin: 0; font-size: var(--font-xl); color: #002d5b; font-weight: 800; letter-spacing: -0.03em;">${esc(linkTargetName ? t('companions.linkSheetTitle') : t('companions.friendSheetTitle'))}</h2>
                    <button type="button" id="companionFriendPopupClose" class="close-x-btn" aria-label="${esc(t('companions.rowCloseTitle'))}">${iconSvg('close', { size: 16 })}</button>
                </div>
        ...
        const friendListEl = (q(popupRoot, '#companionFriendPopupList') as HTMLElement);
        (q(popupRoot, '#companionFriendPopupClose') as HTMLButtonElement).onclick = () => closePopup();
        ...
        const buildFriendList = () => {
            ...
            const selfRow = (linkTargetName && myId)
                ? `
        ...
        // A2-I3: refetch every open, not just the first. A link/unlink earlier
        // in this same modal session frees (or claims) a friend; a once-per-
        // open cache would keep showing the stale candidate set. The loading
        // placeholder in the initial markup covers the fetch window.
        void (async () => {
            cachedFriends = await fetchAcceptedFriends();
            buildFriendList();
        })();

        // Delegated clicks inside the POP-UP — link-self + friend-add live
        // here (the pop-up has its own modal root, so the parent modal's
        // delegated listener never sees these clicks).
        popupRoot.addEventListener('click', (ev) => { void (async () => {
            ...
            if (linkSelfBtn) {
                if (linkTargetName && myId) {
                    const c = findTripCompanion(trip, linkTargetName);
                    if (c) {
                        c.linkedUserId = myId;
                    ...
                    closePopup(); // onClose refreshes the parent roster
                }
                return;
            }
            ...
            if (addBtn) {
                ...
                if (!inviteRes.ok) {
                    ...
                    showLiquidAlert(msg);
                    return; // leave the pop-up open; no optimistic link, no false toast
                }
    ...
    (q(root, '#companionPickerAddFriendBtn') as HTMLButtonElement).onclick = () => {
        openFriendPickerPopup(null); // ADD mode, not link-existing → no self row
    };
    ...
    root.addEventListener('click', (ev) => { void (async () => {
        ...
        // Promote an unlinked entry → link pop-up scoped to "link this name"
        // (offers a "This is me" row + friends). The link-self / friend-add
        // handlers live on the pop-up's own root (its clicks never reach us).
        const linkBtn = (target.closest('.picker-link-btn') as HTMLElement | null);
        if (linkBtn?.dataset.name) {
            openFriendPickerPopup(linkBtn.dataset.name);
            return;
        }
    })(); });
};
