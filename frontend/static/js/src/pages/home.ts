...
import { showLiquidAlert, showConfirmModal, generateId, esc } from '../utils.js';
...
// `activeMarkers` (the {dayId: marker} lookup) used to live here — it
// was a leftover from the Leaflet-era pin-edit flow that read marker
// positions on save. The current Google-Maps flow re-renders the
// whole home page after a save (navigate('home') in saveDayPin) so
// paintDayMarkers gets called with fresh state; nothing reads the
// cache between renders. Removed in B1 slice 4 along with the
// day-marker extraction.
...
export function renderHome() {
    ...
    if (!activeTrip) {
        // B1 slice 2: empty-state HTML moved to ./home/welcomeCard.ts.
        // The slideshow controller still owns its rotation lifecycle —
        // it pre-fills `displayImages[0]` / `displayQuotes[0]` for the
        // initial paint, then mutates #homeHeroImg.src + #homeQuote
        // textContent on a 6s timer once .start(div) runs below.
        div.innerHTML = buildEmptyStateHtml(displayImages, displayQuotes);
        slideshow.start(div);
        ...
        // B1 slice 2: greeting picker moved to ./home/welcomeCard.ts.
        ...
        setTimeout(() => {
            ...
            if (mapContainer && typeof google !== 'undefined' && google.maps && activeTrip) {
                ...
                // ── Map search banner ──────────────────────────────────
                // B1 slice 3: the 200-line Places-autocomplete IIFE
                // moved to ./home/mapSearch.ts. Closure inputs (map,
                // activeTrip, plus the four shared InfoWindow / Places-
                // service helpers defined further up in renderHome)
                // pass through a context object so the wiring stays a
                ...
                // B1 slice 4: day-marker creation moved to
                // ./home/dayMarkers.ts. The forEach + the Anchor-pin
                // SVG data-URL + the shared Street-View InfoWindow
                // all live there now. The returned {dayId: marker}
                // lookup is unused at this call site (the legacy
    ...
    if (activeTrip) {
        ...
        //
        // B1 first slice (D6+B1): the ~260 lines of HTML-builder
        // closures (buildDayCardBody / buildOptionsStack /
        // buildPathTabHtml) moved to ./home/pathTab.ts. We keep the
        // local `buildPathTab()` wrapper here so the rest of
        // renderHome's repaint logic and the click delegations don't
        // need to know about the inputs — they just call buildPathTab()
        // and get the same string back. `editingDayId` is a closure-
        // bound `let` further down that flips on pin-edit; passing it
        // by reference (via a getter inside buildPathTab) keeps the
        // module pure-function shape.
        const buildPathTab = (): string => buildPathTabHtml({
            activeTrip,
            tripDays,
            tripIsEditable,
            editingDayId,
        ...
        const repaintPath = () => {
            ...
            pathTabInner.innerHTML = buildPathTab();
