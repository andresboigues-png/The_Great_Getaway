...
`):e.activity&&e.description?`${e.activity}: ${e.description}`:e.activity||e.description||``}var k=null,A=[];function j(){let e=document.createElement(`div`),t=l.trips.find(e=>e.id===l.activeTripId);if(!t)return e.innerHTML=`
        ...
        <div style="${F}">
            ...
            `,r.querySelector(`#aiManageTodoBtn`)?.addEventListener(`click`,()=>g(`todo`)),r.querySelectorAll(`.marked-day-select, .marked-time-select`).forEach(e=>{e.onchange=()=>{let i=e.dataset.placeId;if(!i)return;let a=r.querySelector(`.ai-marked-card[data-place-id="${i}"]`);if(!a)return;let o=a.querySelector(`.marked-day-select`),s=a.querySelector(`.marked-time-select`);c(t,i,o?.value||null,s?.value||null),n(`state:changed`),_(t)}})};D(),T&&E(T,N,S);let M=e.querySelector(`#aiExtraContext`);M&&(M.oninput=e=>{t.aiContext=e.target.value,n(`state:changed`)});let I=e.querySelector(`#aiKeyInput`),L=e.querySelector(`#aiKeyToggleBtn`),R=e.querySelector(`#aiKeyHelpBtn`),z=e.querySelector(`#aiKeyStatus`),B=()=>{if(!z)return;let e=(l.geminiApiKey||``).trim();if(!e){z.textContent=`No key saved — paste one above to enable AI generation.`,z.style.color=`#a85d00`;return}let t=e.startsWith(`AIza`)&&e.length>=30;z.textContent=t?`✓ Key saved on this device.`:`⚠ Saved, but the format looks off (Gemini keys usually start with "AIza"). Click i for help.`,z.style.color=t?`#1a6b3c`:`#a85d00`};B(),I&&I.addEventListener(`input`,()=>{l.geminiApiKey=I.value,n(`state:changed`),B()}),L&&I&&L.addEventListener(`click`,()=>{let e=I.type===`text`;I.type=e?`password`:`text`,L.textContent=e?`👁`:`🙈`,L.title=e?`Show key`:`Hide key`}),R&&R.addEventListener(`click`,()=>{let{root:e,close:t}=p({cardClass:`card glass`,cardStyle:`width: 520px; max-width: calc(100vw - 32px); max-height: 90vh; overflow-y: auto; padding: 28px 32px; border-radius: 28px; background: white;`,innerHTML:`
                    ...
                    `});e.querySelector(`#aiKeyHelpClose`)?.addEventListener(`click`,t),e.querySelector(`#aiKeyHelpDone`)?.addEventListener(`click`,t)}),[`#aiDateFrom`,`#aiDateTo`].forEach(t=>{let n=e.querySelector(t);n&&n.addEventListener(`change`,()=>D())});let V=e.querySelector(`#generateBtn`),H=async()=>{let r=y(e,`#itineraryOutput`),o=y(e,`#aiDateFrom`).value,s=y(e,`#aiDateTo`).value,c=document.getElementById(`aiExtraContext`)?.value??``;if(!o||!s){m(`Pick your travel dates first.`);return}if(s<o){m(`End date must be on or after the start date.`);return}let u=new Date(o),d=new Date(s),f=Math.max(1,Math.round((d.getTime()-u.getTime())/864e5)+1),p=i(t).filter(e=>e.forAI&&e.forManual),h=``;if(p.length>0){let e=(l.tripDays||[]).filter(e=>e.tripId===t.id&&e.dayNumber>0),n=t=>e.find(e=>e.id===t)?.dayNumber;h=`\n\nThe user has marked these specific places to include in the itinerary. Please incorporate them where they fit, respecting any day/time assignments where given:\n${p.map(e=>{let t=e.dayId?n(e.dayId):null,r=t?`, on Day ${t}`:``,i=e.timeOfDay?`, ${e.timeOfDay}`:``,a=e.address?` (${e.address})`:``;return`- ${e.name}${a}${r}${i}`}).join(`
`)}`}let g=c+h;t.aiContext=c,t.aiNumDays=f,n(`state:changed`),r.innerHTML=`<div style="text-align:center;padding:60px;"><div class="spinner-ring" style="width:40px;height:40px;border:3px solid rgba(0,113,227,0.15);border-top-color:#005bb8;border-radius:50%;animation:spin 1s linear infinite;margin:0 auto 20px;"></div><div style="color:var(--text-primary);font-weight:600;">Consulting Gemini AI…</div><div style="color:var(--text-secondary);font-size:0.82rem;margin-top:6px;">This usually takes 5-15 seconds. Maps lookups for each place add a few more.</div></div>`,r.scrollIntoView({behavior:`smooth`});let _=V?.innerHTML||``;V&&(V.disabled=!0,V.innerHTML=`⌛ Generating…`);try{let e=await(await x(`/api/generate_itinerary`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({destination:S,numDays:f,dateFrom:o,dateTo:s,context:g,gemini_key:(l.geminiApiKey||``).trim()})})).json();if(e.error)throw Error(e.error);T=e.itinerary,T==null?delete t.aiPlan:t.aiPlan=T,n(`state:changed`),E(T,f,S),r.scrollIntoView({behavior:`smooth`})}catch(e){let t=e.message||``,n=`Something went wrong while generating your plan.`,i=``;/UNAVAILABLE|503|overloaded/i.test(t)?(n=`Gemini is overloaded right now.`,i=`This usually clears in 30-60 seconds.`):/quota|limit|RESOURCE_EXHAUSTED|429/i.test(t)?(n=`Daily AI quota reached for this key.`,i=`Try again tomorrow, or use a different Gemini key in Settings → AI Engine.`):/key|api[_ ]?key|UNAUTHENTICATED|401|403/i.test(t)?(n=`AI key isn't accepted by Gemini.`,i=`Open Settings → AI Engine and check the key, or generate a new one.`):/network|fetch|timed?[\- ]?out|ECONN/i.test(t)&&(n=`Network hiccup talking to the AI.`,i=`Check your connection and retry.`),r.innerHTML=`
