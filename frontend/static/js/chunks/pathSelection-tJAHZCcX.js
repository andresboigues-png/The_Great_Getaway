import{Rt as e,S as t,Wt as n,Zt as r,x as i,zt as a}from"../app.bundle.js";var o=`home_path_card_collapsed_day_ids`;function s(){try{let e=localStorage.getItem(o);if(!e)return new Set;let t=JSON.parse(e);return new Set(Array.isArray(t)?t:[])}catch{return new Set}}function c(e){try{localStorage.setItem(o,JSON.stringify([...e]))}catch{}}function l(e){let t=s();return t.has(e)?(t.delete(e),c(t),!1):(t.add(e),c(t),!0)}function u(e){return s().has(e)}function d(e,t){let n=s(),r=n.has(e);t&&!r?(n.add(e),c(n)):!t&&r&&(n.delete(e),c(n))}function f(t,r,o){let{isAnchor:s,isSelected:c}=r,l=s?`<div style="background: var(--gradient-anchor-deep); color: white; width: 48px; height: 48px; border-radius: 50%; border: 3px solid white; display: flex; align-items: center; justify-content: center; flex-shrink:0; box-shadow: 0 8px 18px rgba(212,160,23,0.28);">
               ...
               <span style="font-size: 0.6rem; font-weight: 800; text-transform: uppercase; opacity: 0.85; letter-spacing: 0.05em; line-height:1;">${e(i(`pathTab.dayBadgeLabel`))}</span>
               <span style="font-size: 1.25rem; font-weight: 800; line-height: 1.05;">${t.dayNumber}</span>
           </div>`,u=s?i(`pathTab.hubTitle`):e(t.name||i(`tripMedia.dayBucketDay`,{n:t.dayNumber})),d=[];if(s){d.push(o&&o.country?e(n(o.country)):e(i(`pathTab.hubSubtitleFallback`)));let r=(o.documents||[]).filter(e=>e.dayId===t.id),a=(o.photos||[]).filter(e=>e.dayId===t.id),s=r.length+(t.tickets||[]).length,c=a.length+(t.photos||[]).length;c&&d.push(`<span style="background:rgba(52,199,89,0.12); color:#1a6b3c; padding:2px 8px; border-radius:999px; font-size:0.7rem; font-weight:800;">📸 ${c}</span>`),s&&d.push(`<span style="background:rgba(88,86,214,0.12); color:#5856d6; padding:2px 8px; border-radius:999px; font-size:0.7rem; font-weight:800;">📎 ${s}</span>`)}else d.push(`📅 ${a(t.date)||i(`pathTab.setDatePlaceholder`)}`),t.lat?d.push(`<span style="color: #005bb8;">${e(i(`pathTab.locationSet`))}</span>`):d.push(`<span class="day-card__pin-hint">${e(i(`pathTab.pinThisDay`))}</span>`),t.date&&d.push(`<span class="day-card__weather" data-weather-date="${e(t.date)}"></span>`);let f=c&&t.notes&&!s?`
        <div style="margin-top: 12px; padding: 12px 14px; background: rgba(0,113,227,0.04); border-radius: 14px; border-left: 3px solid var(--accent-blue);">
            <div style="font-size: 0.7rem; font-weight: 800; text-transform: uppercase; color: #005bb8; margin-bottom: 4px; letter-spacing: 0.05em;">${e(i(`pathTab.journalPreviewLabel`))}</div>
            <p style="margin: 0; font-size: 0.9rem; line-height: 1.45; color: #002d5b; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden;">${e(t.notes)}</p>
        </div>
    `:``,p=`
        <button type="button" class="path-card-collapse-btn" data-day-id="${e(t.id)}"
            aria-label="${e(i(`pathTab.toggleOptionsAria`,{title:u}))}" title="${e(i(`pathTab.toggleOptionsTitle`))}">
    ...
    `;return`
        ...
        ${f}
    `}function p(t,n,r,a){if(!t||!r)return``;let{isAnchor:o}=n,s=[];if(o?s.push(`<button class="path-primary-btn path-primary-btn--anchor path-checklist-btn" data-day-id="${e(t.id)}">${e(i(`pathTab.btnChecklist`))}</button>`):s.push(`<button class="path-primary-btn day-detail-btn" data-day-id="${e(t.id)}">${e(i(`pathTab.btnOpenFullPlan`))}</button>`),a===t.id)s.push(`<button class="day-action-btn day-action-btn--success day-pin-save-btn" data-day-id="${e(t.id)}">${e(i(`pathTab.btnSavePin`))}</button>`),s.push(`<button class="day-action-btn day-action-btn--danger-fill day-pin-delete-btn" data-day-id="${e(t.id)}">${e(i(`pathTab.btnCancelPinEdit`))}</button>`);else{let n=t.lat?i(o?`pathTab.btnEditAnchorPin`:`pathTab.btnEditPin`):i(o?`pathTab.btnSetAnchorPin`:`pathTab.btnAddPin`);s.push(`<button class="day-action-btn day-action-btn--neutral day-pin-toggle-btn" data-day-id="${e(t.id)}"><span>${e(n)}</span></button>`)}return o?(s.push(`<button class="day-action-btn day-action-btn--neutral path-documents-btn" data-day-id="${e(t.id)}"><span>${e(i(`pathTab.btnDocuments`))}</span></button>`),s.push(`<button class="day-action-btn day-action-btn--neutral path-photos-btn" data-day-id="${e(t.id)}"><span>${e(i(`pathTab.btnPhotos`))}</span></button>`)):(s.push(`<button class="day-action-btn day-action-btn--neutral day-journaling-btn" data-day-id="${e(t.id)}"><span>${e(i(`pathTab.btnJournaling`))}</span></button>`),s.push(`<button class="day-action-btn day-action-btn--danger day-delete-btn" data-day-id="${e(t.id)}"><span>${e(i(`pathTab.btnDeleteDay`))}</span></button>`)),`<div class="path-options-stack">${s.join(``)}</div>`}function m(n){let{activeTrip:r,tripDays:o,tripIsEditable:s,editingDayId:c}=n,l=[...o].sort((e,t)=>e.dayNumber-t.dayNumber),d=l.find(e=>e.dayNumber===0)||null,m=l.filter(e=>e.dayNumber>0),h=x(r,l),g=l.find(e=>e.id===h)||null;if(l.length===0)return`<div class="card glass" style="padding:28px; border-radius:18px; text-align:center; color:var(--text-secondary);">${e(i(`pathTab.emptyState`))}</div>`;let _=m.length,v=g?.dayNumber===0?t(`path.summaryHub`,_,{count:_}):g?i(`path.summaryDay`,{day:g.dayNumber,total:_}):t(`path.summaryNone`,_,{count:_}),y=(()=>{let e=new Date;return`${e.getFullYear()}-${String(e.getMonth()+1).padStart(2,`0`)}-${String(e.getDate()).padStart(2,`0`)}`})(),b=l.map(t=>{let n=t.id===h,r=t.dayNumber===0,o=!r&&t.date===y,s=`path-chip${r?` path-chip--anchor`:``}${o?` path-chip--today`:``}${n?` is-selected`:``}`,c=r?i(`pathTab.chipHubTooltip`):`${o?i(`pathTab.chipTodayPrefix`)+` · `:``}${i(`tripMedia.dayBucketDay`,{n:t.dayNumber})}${t.name?` — `+t.name:``}${t.date?` · `+(a(t.date)||t.date):``}`,l=r?`<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" stroke-width="1.2" stroke-linejoin="round" aria-hidden="true"><polygon points="12 2 15 8.5 22 9.3 17 14.3 18.2 21.3 12 18 5.8 21.3 7 14.3 2 9.3 9 8.5"/></svg>`:String(t.dayNumber);return`<button type="button" class="${s}" data-path-chip-day-id="${e(t.id)}" title="${e(c)}" aria-label="${e(c)}" aria-pressed="${n}">${l}</button>`}).join(``),S=s?`<button type="button" class="path-chip path-chip--add" id="pathAddDayChip" title="${e(i(`pathTab.addNewDay`))}" aria-label="${e(i(`pathTab.addNewDay`))}">+</button>`:``,C=l.findIndex(e=>e.id===h),w=C<=0,T=C<0||C>=l.length-1,E=[];if(d){let t=g?.id===d.id,n=u(d.id);E.push(`
            <div class="path-column path-column--anchor${n?` is-collapsed`:``}">
                <div class="path-card path-card--anchor${t?` is-selected`:``}" data-day-id="${e(d.id)}">
                    ${f(d,{isAnchor:!0,isSelected:t},r)}
                </div>
                ${p(d,{isAnchor:!0},s,c)}
            </div>
        `)}if(g&&g.dayNumber>0){let t=u(g.id);E.push(`
            <div class="path-column path-column--selected${t?` is-collapsed`:``}">
                <div class="path-card path-card--selected" data-day-id="${e(g.id)}">
        ...
        <div class="path-strip">
            <button type="button" class="path-nav-btn" id="pathPrevBtn" title="${e(i(`pathTab.previousDay`))}" aria-label="${e(i(`pathTab.previousDay`))}" ${w?`disabled`:``}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6"></polyline></svg>
            </button>
            <div class="path-chips" role="group" aria-label="${e(i(`pathTab.tripDaysGroupAria`))}">
                ${b}
                ${S}
            </div>
            <button type="button" class="path-nav-btn" id="pathNextBtn" title="${e(i(`pathTab.nextDay`))}" aria-label="${e(i(`pathTab.nextDay`))}" ${T?`disabled`:``}>
...
//# sourceMappingURL=pathSelection-tJAHZCcX.js.map