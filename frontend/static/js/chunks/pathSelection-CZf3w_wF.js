import{Bt as e,Gt as t,S as n,en as r,x as i,zt as a}from"../app.bundle.js";var o=`home_path_card_collapsed_day_ids`;function s(){try{let e=localStorage.getItem(o);if(!e)return new Set;let t=JSON.parse(e);return new Set(Array.isArray(t)?t:[])}catch{return new Set}}function c(e){try{localStorage.setItem(o,JSON.stringify([...e]))}catch{}}function l(e){let t=s();return t.has(e)?(t.delete(e),c(t),!1):(t.add(e),c(t),!0)}function u(e){return s().has(e)}function d(e,t){let n=s(),r=n.has(e);t&&!r?(n.add(e),c(n)):!t&&r&&(n.delete(e),c(n))}function f(n,r,o){let{isAnchor:s,isSelected:c}=r,l=s?`<div style="background: var(--gradient-anchor-deep); color: white; width: 48px; height: 48px; border-radius: 50%; border: 3px solid white; display: flex; align-items: center; justify-content: center; flex-shrink:0; box-shadow: 0 8px 18px rgba(212,160,23,0.28);">
           ...
           </div>`,u=s?i(`pathTab.hubTitle`):a(n.name||i(`tripMedia.dayBucketDay`,{n:n.dayNumber})),d=[];if(s){d.push(o&&o.country?a(t(o.country)):a(i(`pathTab.hubSubtitleFallback`)));let e=(o.documents||[]).filter(e=>e.dayId===n.id),r=(o.photos||[]).filter(e=>e.dayId===n.id),s=e.length+(n.tickets||[]).length,c=r.length+(n.photos||[]).length;c&&d.push(`<span style="background:rgba(52,199,89,0.12); color:#1a6b3c; padding:2px 8px; border-radius:999px; font-size:0.7rem; font-weight:800;">📸 ${c}</span>`),s&&d.push(`<span style="background:rgba(88,86,214,0.12); color:#5856d6; padding:2px 8px; border-radius:999px; font-size:0.7rem; font-weight:800;">📎 ${s}</span>`)}else d.push(`📅 ${e(n.date)||i(`pathTab.setDatePlaceholder`)}`),n.lat?d.push(`<span style="color: #005bb8;">${a(i(`pathTab.locationSet`))}</span>`):d.push(`<span class="day-card__pin-hint">${a(i(`pathTab.pinThisDay`))}</span>`),n.date&&d.push(`<span class="day-card__weather" data-weather-date="${a(n.date)}"></span>`);let f=c&&n.notes&&!s?`
    ...
    `:``,p=`
        <button type="button" class="path-card-collapse-btn" data-day-id="${a(n.id)}"
            aria-label="${a(i(`pathTab.toggleOptionsAria`,{title:u}))}" title="${a(i(`pathTab.toggleOptionsTitle`))}">
    ...
    `;return`
        ...
        ${f}
    `}function p(e,t,n,r){if(!e||!n)return``;let{isAnchor:o}=t,s=[];if(o?s.push(`<button class="path-primary-btn path-primary-btn--anchor path-checklist-btn" data-day-id="${a(e.id)}">${a(i(`pathTab.btnChecklist`))}</button>`):s.push(`<button class="path-primary-btn day-detail-btn" data-day-id="${a(e.id)}">${a(i(`pathTab.btnOpenFullPlan`))}</button>`),r===e.id)s.push(`<button class="day-action-btn day-action-btn--success day-pin-save-btn" data-day-id="${a(e.id)}">${a(i(`pathTab.btnSavePin`))}</button>`),s.push(`<button class="day-action-btn day-action-btn--danger-fill day-pin-delete-btn" data-day-id="${a(e.id)}">${a(i(`pathTab.btnCancelPinEdit`))}</button>`);else{let t=e.lat?i(o?`pathTab.btnEditAnchorPin`:`pathTab.btnEditPin`):i(o?`pathTab.btnSetAnchorPin`:`pathTab.btnAddPin`);s.push(`<button class="day-action-btn day-action-btn--neutral day-pin-toggle-btn" data-day-id="${a(e.id)}"><span>${a(t)}</span></button>`)}return o?(s.push(`<button class="day-action-btn day-action-btn--neutral path-documents-btn" data-day-id="${a(e.id)}"><span>${a(i(`pathTab.btnDocuments`))}</span></button>`),s.push(`<button class="day-action-btn day-action-btn--neutral path-photos-btn" data-day-id="${a(e.id)}"><span>${a(i(`pathTab.btnPhotos`))}</span></button>`)):(s.push(`<button class="day-action-btn day-action-btn--neutral day-journaling-btn" data-day-id="${a(e.id)}"><span>${a(i(`pathTab.btnJournaling`))}</span></button>`),s.push(`<button class="day-action-btn day-action-btn--danger day-delete-btn" data-day-id="${a(e.id)}"><span>${a(i(`pathTab.btnDeleteDay`))}</span></button>`)),`<div class="path-options-stack">${s.join(``)}</div>`}function m(t){let{activeTrip:r,tripDays:o,tripIsEditable:s,editingDayId:c}=t,l=[...o].sort((e,t)=>e.dayNumber-t.dayNumber),d=l.find(e=>e.dayNumber===0)||null,m=l.filter(e=>e.dayNumber>0),h=x(r,l),g=l.find(e=>e.id===h)||null;if(l.length===0)return`<div class="card glass" style="padding:28px; border-radius:18px; text-align:center; color:var(--text-secondary);">${a(i(`pathTab.emptyState`))}</div>`;let _=m.length,v=g?.dayNumber===0?n(`path.summaryHub`,_,{count:_}):g?i(`path.summaryDay`,{day:g.dayNumber,total:_}):n(`path.summaryNone`,_,{count:_}),y=(()=>{let e=new Date;return`${e.getFullYear()}-${String(e.getMonth()+1).padStart(2,`0`)}-${String(e.getDate()).padStart(2,`0`)}`})(),b=l.map(t=>{let n=t.id===h,r=t.dayNumber===0,o=!r&&t.date===y,s=`path-chip${r?` path-chip--anchor`:``}${o?` path-chip--today`:``}${n?` is-selected`:``}`,c=r?i(`pathTab.chipHubTooltip`):`${o?i(`pathTab.chipTodayPrefix`)+` · `:``}${i(`tripMedia.dayBucketDay`,{n:t.dayNumber})}${t.name?` — `+t.name:``}${t.date?` · `+(e(t.date)||t.date):``}`,l=r?`<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" stroke-width="1.2" stroke-linejoin="round" aria-hidden="true"><polygon points="12 2 15 8.5 22 9.3 17 14.3 18.2 21.3 12 18 5.8 21.3 7 14.3 2 9.3 9 8.5"/></svg>`:String(t.dayNumber);return`<button type="button" class="${s}" data-path-chip-day-id="${a(t.id)}" title="${a(c)}" aria-label="${a(c)}" aria-pressed="${n}">${l}</button>`}).join(``),S=s?`<button type="button" class="path-chip path-chip--add" id="pathAddDayChip" title="${a(i(`pathTab.addNewDay`))}" aria-label="${a(i(`pathTab.addNewDay`))}">+</button>`:``,C=l.findIndex(e=>e.id===h),w=C<=0,T=C<0||C>=l.length-1,E=[];if(d){let e=g?.id===d.id,t=u(d.id);E.push(`
        ...
        `)}if(g&&g.dayNumber>0){let e=u(g.id);E.push(`
            <div class="path-column path-column--selected${e?` is-collapsed`:``}">
                <div class="path-card path-card--selected" data-day-id="${a(g.id)}">
...
//# sourceMappingURL=pathSelection-CZf3w_wF.js.map