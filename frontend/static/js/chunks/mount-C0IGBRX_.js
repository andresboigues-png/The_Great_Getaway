import{r as e,t}from"./vendor-react-CYVQMBjw.js";import{$ as n,A as r,B as i,D as a,G as o,O as s,P as c,Q as l,R as u,T as d,W as f,Y as p,Z as m,_ as h,a as g,et as _,g as v,h as y,r as b,t as x,tt as S,x as C}from"../app.bundle.js";var w=e();function T(e){if(!e)return``;let t=Array.isArray(e.items)?e.items.filter(Boolean):[];if(t.length>0)return`<ul class="ai-plan-block__list">${t.map(e=>E(e)).join(``)}</ul>`;if(e.description){let t=String(e.description).split(/\n+/).map(e=>e.trim()).filter(Boolean);return t.length>1?`<ul class="ai-plan-block__list">${t.map(e=>`<li>${f(e.replace(/^[-•*]\s*/,``))}</li>`).join(``)}</ul>`:`<div class="ai-plan-block__desc">${f(e.description)}</div>`}return``}function E(e){if(typeof e==`string`)return`<li class="ai-plan-block__item">${f(e)}</li>`;if(!e||typeof e!=`object`)return``;let t=e,n=String(t.text||``);if(!n)return``;if(t.verified&&t.placeId){let e=t.photoUrl?`<img class="ai-place-card__photo" src="${f(t.photoUrl)}" alt="" referrerpolicy="no-referrer" loading="lazy">`:`<div class="ai-place-card__photo ai-place-card__photo--empty" aria-hidden="true">📍</div>`,r=typeof t.rating==`number`?`<span class="ai-place-card__rating">★ ${t.rating.toFixed(1)}${t.userRatingsTotal?` <span class="ai-place-card__rating-count">(${D(t.userRatingsTotal)})</span>`:``}</span>`:``,i=t.address?`<span class="ai-place-card__address">${f(t.address)}</span>`:``,a=t.why?`<span class="ai-place-card__why">${f(t.why)}</span>`:``,o=t.fact?`<span class="ai-place-card__fact">✨ ${f(t.fact)}</span>`:``,s=t.mapsUrl||`https://www.google.com/maps/place/?q=place_id:${encodeURIComponent(t.placeId)}`,c=t.verifiedName||n;return`
        ...
        <li class="ai-plan-block__item ai-plan-block__item--unverified">
            ...
            ${r}
            ${i}
        </li>`}function D(e){return!Number.isFinite(e)||e<0?``:e<1e3?String(e):e<1e6?`${(Math.round(e/100)/10).toFixed(1).replace(/\.0$/,``)}k`:`${(Math.round(e/1e5)/10).toFixed(1).replace(/\.0$/,``)}M`}function O(e){return typeof e==`string`?e:e&&typeof e==`object`&&typeof e.text==`string`?e.text:String(e??``)}function k(e){if(!e)return``;let t=Array.isArray(e.items)?e.items.filter(Boolean):[];return t.length>0?[e.activity?`${e.activity}:`:``,...t.map(e=>`- ${O(e)}`)].filter(Boolean).join(`
`):e.activity&&e.description?`${e.activity}: ${e.description}`:e.activity||e.description||``}var A=null,j=[];function M(){let e=document.createElement(`div`),t=_.trips.find(e=>e.id===_.activeTripId);if(!t)return e.innerHTML=`
            ...
            </div>`,setTimeout(()=>{e.querySelector(`#aiStartJourneyBtn`)?.addEventListener(`click`,()=>c()),typeof google<`u`&&google.maps&&n(new google.maps.Map(document.getElementById(`emptyMap`),{center:{lat:20,lng:0},zoom:2,minZoom:2,restriction:{latLngBounds:{north:85,south:-85,west:-180,east:180},strictBounds:!0},styles:[]}),[])},100),e;let C=t.country||``,w=(_.tripDays||[]).filter(e=>e.tripId===_.activeTripId&&e.dayNumber>0&&e.date).map(e=>e.date).sort(),E=_.expenses.filter(e=>e.tripId===_.activeTripId&&e.date).sort((e,t)=>e.date.localeCompare(t.date)).map(e=>e.date),D=t.dateFrom||w[0]||E[0]||``,O=t.dateTo||w[w.length-1]||E[E.length-1]||``,M=t.aiPlan||null,N=t.aiContext||``,P=t.aiNumDays||1,F=u(t),I=`font-family:-apple-system,BlinkMacSystemFont,'SF Pro Display','SF Pro Text',sans-serif;`;return e.innerHTML=`
        <div style="${I}">
            <!-- Header -->
            <div style="padding:32px 0 24px;">
                ...
                <p style="margin:0;color:var(--text-secondary);font-size:1rem;">${x(`ai.subtitlePlanning`,{country:f(C)})}</p>
            </div>
            ...
            <div class="ai-page-2col" style="display:grid;grid-template-columns:380px 1fr;gap:24px;margin-bottom:32px;">
                ...
                <div id="aiControlsPanel" style="display:flex;flex-direction:column;gap:16px;min-height:700px;">
                    ...
                    <div class="card glass" style="padding:20px;flex:0 0 auto;">
                        ...
                        <div style="display:flex; flex-direction:column; gap:12px;">
                            <div>
                                ...
                                <input id="aiDateFrom" type="date" class="glass-input" value="${D}" style="width:100%; box-sizing: border-box;">
                            </div>
                            <div>
                                ...
                                <input id="aiDateTo" type="date" class="glass-input" value="${O}" style="width:100%; box-sizing: border-box;" min="${D}">
                            </div>
                    ...
                    <div class="card glass" style="padding:20px;flex:1 1 auto;display:flex;flex-direction:column;min-height:0;">
                        ...
                        <textarea id="aiExtraContext" class="glass-input" style="width:100%; resize:none; font-size:0.9rem; box-sizing: border-box; flex:1 1 auto; min-height:120px;" placeholder="e.g. Vegetarian friendly, no walking more than 2km...">${N}</textarea>
                    </div>
                    <!-- Generate -->
                    ${F?`<button id="generateBtn" class="ai-generate-btn" style="width:100%; border-radius: var(--radius-lg);flex:0 0 auto;">${x(`ai.generateBtn`)}</button>`:(()=>{let e=i(t);return`<div class="card glass" style="padding:16px; border-radius: var(--radius-lg); text-align:center; color: var(--text-secondary); font-size: 0.85rem; flex:0 0 auto;">
            ...
            <div id="itineraryOutput" style="margin-bottom: 60px;"></div>
        </div>`,setTimeout(()=>{let i=e.querySelector(`#aiDateFrom`),c=e.querySelector(`#aiDateTo`),u=e.querySelector(`#aiDateHint`);if(i&&c){let e=u?.textContent||``,t=u?.style.color||``,n=()=>{c.min=i.value||``,i.value&&c.value&&c.value<i.value?(u&&(u.textContent=`End date must be on or after the start date.`,u.style.color=`#a82424`),c.setCustomValidity(`End date must be on or after the start date.`)):(u&&(u.textContent=e,u.style.color=t),c.setCustomValidity(``))};i.addEventListener(`change`,n),c.addEventListener(`change`,n),n()}let w=e=>{if(!A)return;let n=t.id+`_ai`;if(_.mapViews&&_.mapViews[n]){let e=_.mapViews[n];A.setCenter({lat:e.lat,lng:e.lng}),A.setZoom(e.zoom);return}if(t.viewport){let e=t.viewport;A.fitBounds(new google.maps.LatLngBounds({lat:e.south,lng:e.west},{lat:e.north,lng:e.east}));return}let r=e.replace(/\(USA\)/g,``).trim();r.includes(` - `)&&(r=r.split(` - `)[1]+`, USA`),new google.maps.Geocoder().geocode({address:r},(e,t)=>{t===`OK`&&e[0]&&A.fitBounds(e[0].geometry.viewport)})};if(typeof google<`u`&&google.maps){let r=document.getElementById(`aiGoogleMap`);r&&(A=new google.maps.Map(r,{center:{lat:20,lng:0},zoom:2,minZoom:2,mapTypeId:`roadmap`,disableDefaultUI:!0,restriction:{latLngBounds:{north:85,south:-85,west:-180,east:180},strictBounds:!0},styles:[]}),n(A,[]),w(C),A.addListener(`idle`,()=>{let e=t.id+`_ai`;_.mapViews||={};let n=A.getCenter();_.mapViews[e]={lat:n.lat(),lng:n.lng(),zoom:A.getZoom()},S(`state:changed`)}));let i=e.querySelector(`#aiZoomBadge`);i&&(i.onclick=()=>{let e=t.id+`_ai`;_.mapViews&&_.mapViews[e]&&delete _.mapViews[e],w(C)})}let E=M,D=(n,r,i)=>{let o=p(e,`#itineraryOutput`);if(!n||!n.length){o.innerHTML=``;return}o.innerHTML=`
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:24px;">
                    <div>
                        <!-- Phase G UX: text colour switched from white to a
                             ...
                             cards below. -->
                        <h2 style="margin:0;font-size:2rem;font-weight:800;letter-spacing:-0.03em;background:var(--gradient-title);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;${I}">${x(`ai.resultHeading`,{numDays:r,country:i})}</h2>
                ...
                ${F?`<div style="display:flex;gap:12px;margin-top:24px;"><button id="acceptPlanBtn" class="btn" style="flex:2;background:var(--accent-blue);color:white;padding:16px;font-size:1.1rem;border-radius:16px;font-weight:700;box-shadow:0 10px 20px rgba(0,122,255,0.2);cursor:pointer;">${x(`ai.acceptPlanBtn`)}</button></div>`:``}`;let s=p(o,`#itineraryDays`),c=[];if(n.forEach((e,t)=>{let n=document.createElement(`div`);n.className=`card glass`,n.style.cssText=`border-radius:18px;overflow:hidden;transition:box-shadow 0.3s,border-color 0.3s;${I}`,n.innerHTML=`
                    ...
                    </div>`,s.appendChild(n),c.push(n)}),A){j.forEach(e=>e.setMap(null)),j=[];let e=new google.maps.LatLngBounds,t=new google.maps.Geocoder,r=(n,r)=>{let a=n.mainLocation||n.title||i;!n.mainLocation&&n.title&&(a=n.title.replace(/Exploring |Day Trip to |Visit |Touring |Arrival in |Departure from |Day \d+:? /gi,``).trim()),t.geocode({address:a+`, `+i},(t,i)=>{if(i===`OK`&&t[0]){let i=t[0].geometry.location;n.lat=i.lat(),n.lon=i.lng();let a=new google.maps.Marker({position:i,map:A,label:{text:String(n.day),color:`white`,fontWeight:`800`},icon:{path:google.maps.SymbolPath.CIRCLE,scale:16,fillColor:`#0071e3`,fillOpacity:1,strokeWeight:2,strokeColor:`white`}});a.addListener(`click`,()=>{c.forEach(e=>{e.style.boxShadow=``,e.style.borderColor=``});let e=c[r];e&&(e.style.boxShadow=`0 0 0 3px var(--accent-blue), 0 8px 32px rgba(0,113,227,0.25)`,e.style.borderColor=`var(--accent-blue)`,e.scrollIntoView({behavior:`smooth`,block:`center`}))}),j.push(a),e.extend(i),j.length>0&&A.fitBounds(e)}})};n.forEach((e,t)=>setTimeout(()=>r(e,t),t*500))}let l=document.getElementById(`acceptPlanBtn`);l&&(l.onclick=()=>{if(!n)return;let e=_.tripDays.filter(e=>e.tripId===t.id&&e.dayNumber>0);_.tripDays=_.tripDays.filter(e=>!(e.tripId===t.id&&e.dayNumber>0)),e.forEach(e=>g(e.id)),a(t),n.forEach((e,n)=>{let r=e.date||new Date().toISOString().split(`T`)[0],i=`day_`+Date.now()+`_`+n,a={id:i,tripId:t.id,date:r,name:e.title||`Day ${n+1}`,dayNumber:n+1,lat:e.lat,lng:e.lon,photos:[],tickets:[],notes:``,plan:{morning:k(e.morning),afternoon:k(e.afternoon),evening:k(e.evening)}};_.tripDays.push(a),y(a);let o=[[`morning`,e.morning],[`afternoon`,e.afternoon],[`evening`,e.evening]];for(let[e,n]of o){let r=Array.isArray(n?.items)?n.items:[];for(let n of r)d(t,n,i,e)}}),v(t),S(`state:changed`),l.innerHTML=`✓ Plan Accepted! (View in Home)`,l.style.background=`#34c759`,l.disabled=!0})},O=()=>{let n=e.querySelector(`#aiTodoListPanel`);if(!n)return;let i=s(t).filter(e=>e.forManual),a=i.filter(e=>e.forAI);if(i.length===0){n.innerHTML=`
            ...
            `,n.querySelector(`#aiManageTodoBtn`)?.addEventListener(`click`,()=>h(`todo`)),n.querySelectorAll(`.marked-day-select, .marked-time-select`).forEach(e=>{e.onchange=()=>{let i=e.dataset.placeId;if(!i)return;let a=n.querySelector(`.ai-marked-card[data-place-id="${i}"]`);if(!a)return;let o=a.querySelector(`.marked-day-select`),s=a.querySelector(`.marked-time-select`);r(t,i,o?.value||null,s?.value||null),S(`state:changed`),v(t)}})};O(),E&&D(E,P,C);let N=e.querySelector(`#aiExtraContext`);N&&(N.oninput=e=>{t.aiContext=e.target.value,S(`state:changed`)});let L=e.querySelector(`#aiKeyInput`),R=e.querySelector(`#aiKeyToggleBtn`),z=e.querySelector(`#aiKeyHelpBtn`),B=e.querySelector(`#aiKeyStatus`),V=()=>{if(!B)return;let e=(_.geminiApiKey||``).trim();if(!e){B.textContent=`No key saved — paste one above to enable AI generation.`,B.style.color=`#a85d00`;return}let t=e.startsWith(`AIza`)&&e.length>=30;B.textContent=t?`✓ Key saved on this device.`:`⚠ Saved, but the format looks off (Gemini keys usually start with "AIza"). Click i for help.`,B.style.color=t?`#1a6b3c`:`#a85d00`};V(),L&&L.addEventListener(`input`,()=>{_.geminiApiKey=L.value,S(`state:changed`),V()}),R&&L&&R.addEventListener(`click`,()=>{let e=L.type===`text`;L.type=e?`password`:`text`,R.textContent=e?`👁`:`🙈`,R.title=e?`Show key`:`Hide key`}),z&&z.addEventListener(`click`,()=>{let{root:e,close:t}=l({cardClass:`card glass`,cardStyle:`width: 520px; max-width: calc(100vw - 32px); max-height: 90vh; overflow-y: auto; padding: 28px 32px; border-radius: 28px; background: white;`,innerHTML:`
                    ...
                    `});e.querySelector(`#aiKeyHelpClose`)?.addEventListener(`click`,t),e.querySelector(`#aiKeyHelpDone`)?.addEventListener(`click`,t)}),[`#aiDateFrom`,`#aiDateTo`].forEach(t=>{let n=e.querySelector(t);n&&n.addEventListener(`change`,()=>O())});let H=e.querySelector(`#generateBtn`),U=async()=>{let n=p(e,`#itineraryOutput`),r=p(e,`#aiDateFrom`).value,i=p(e,`#aiDateTo`).value,a=document.getElementById(`aiExtraContext`)?.value??``;if(!r||!i){m(x(`ai.toastPickDates`));return}if(i<r){m(x(`ai.toastEndBeforeStart`));return}let o=new Date(r),c=new Date(i),l=Math.max(1,Math.round((c.getTime()-o.getTime())/864e5)+1),u=s(t).filter(e=>e.forAI&&e.forManual),d=``;if(u.length>0){let e=(_.tripDays||[]).filter(e=>e.tripId===t.id&&e.dayNumber>0),n=t=>e.find(e=>e.id===t)?.dayNumber;d=`\n\nThe user has marked these specific places to include in the itinerary. Please incorporate them where they fit, respecting any day/time assignments where given:\n${u.map(e=>{let t=e.dayId?n(e.dayId):null,r=t?`, on Day ${t}`:``,i=e.timeOfDay?`, ${e.timeOfDay}`:``,a=e.address?` (${e.address})`:``;return`- ${e.name}${a}${r}${i}`}).join(`
`)}`}let h=a+d;t.aiContext=a,t.aiNumDays=l,S(`state:changed`),n.innerHTML=`<div style="text-align:center;padding:60px;"><div class="spinner-ring" style="width:40px;height:40px;border:3px solid rgba(0,113,227,0.15);border-top-color:#005bb8;border-radius:50%;animation:spin 1s linear infinite;margin:0 auto 20px;"></div><div style="color:var(--text-primary);font-weight:600;">${x(`ai.loadingTitle`)}</div><div style="color:var(--text-secondary);font-size:0.82rem;margin-top:6px;">${x(`ai.loadingBody`)}</div></div>`,n.scrollIntoView({behavior:`smooth`});let g=H?.innerHTML||``;H&&(H.disabled=!0,H.innerHTML=x(`ai.generatingBtn`));try{let e=await(await b(`/api/generate_itinerary`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({destination:C,numDays:l,dateFrom:r,dateTo:i,context:h,gemini_key:(_.geminiApiKey||``).trim()})})).json();if(e.error)throw Error(e.error);E=e.itinerary,E==null?delete t.aiPlan:t.aiPlan=E,S(`state:changed`),D(E,l,C),n.scrollIntoView({behavior:`smooth`})}catch(e){let t=e.message||``,r=x(`ai.errorGeneric`),i=``;/UNAVAILABLE|503|overloaded/i.test(t)?(r=x(`ai.errorOverloaded`),i=x(`ai.errorOverloadedHint`)):/quota|limit|RESOURCE_EXHAUSTED|429/i.test(t)?(r=x(`ai.errorQuota`),i=x(`ai.errorQuotaHint`)):/key|api[_ ]?key|UNAUTHENTICATED|401|403/i.test(t)?(r=x(`ai.errorBadKey`),i=x(`ai.errorBadKeyHint`)):/network|fetch|timed?[\- ]?out|ECONN/i.test(t)&&(r=x(`ai.errorNetwork`),i=x(`ai.errorNetworkHint`)),n.innerHTML=`
                    <div class="card glass" style="text-align:center;padding:32px 28px;">
                        ...
                        <details style="margin:0 0 18px;text-align:left;background:rgba(255,59,48,0.04);border:1px solid rgba(255,59,48,0.16);border-radius:10px;padding:8px 12px;">
                            ...
                            <pre style="margin:8px 0 0;font-size:0.72rem;color:#666;font-family:monospace;white-space:pre-wrap;word-break:break-word;">${f(t||x(`ai.errorUnknown`))}</pre>
                        </details>
                ...
                `,m(r),n.querySelector(`#aiRetryBtn`)?.addEventListener(`click`,()=>U())}finally{H&&(H.disabled=!1,H.innerHTML=g)}};H?.addEventListener(`click`,U)},0),e}var N=t();function P(){let e=(0,w.useRef)(null);return(0,w.useEffect)(()=>{let t=e.current;t&&(t.innerHTML=``,t.appendChild(M()))},[]),(0,N.jsx)(`div`,{ref:e})}function F(e){C(e,(0,w.createElement)(P))}export{F as mountAI};
//# sourceMappingURL=mount-C0IGBRX_.js.map