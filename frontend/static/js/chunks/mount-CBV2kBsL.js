import{r as e,t}from"./vendor-react-CYVQMBjw.js";import{B as n,C as r,E as i,F as a,G as o,J as s,L as c,M as l,O as u,T as d,V as f,X as p,Y as m,Z as h,h as g,m as _,p as v,q as y,r as b,t as x,y as S}from"../app.bundle.js";var C=e();function w(e){if(!e)return``;let t=Array.isArray(e.items)?e.items.filter(Boolean):[];if(t.length>0)return`<ul class="ai-plan-block__list">${t.map(e=>T(e)).join(``)}</ul>`;if(e.description){let t=String(e.description).split(/\n+/).map(e=>e.trim()).filter(Boolean);return t.length>1?`<ul class="ai-plan-block__list">${t.map(e=>`<li>${n(e.replace(/^[-•*]\s*/,``))}</li>`).join(``)}</ul>`:`<div class="ai-plan-block__desc">${n(e.description)}</div>`}return``}function T(e){if(typeof e==`string`)return`<li class="ai-plan-block__item">${n(e)}</li>`;if(!e||typeof e!=`object`)return``;let t=e,r=String(t.text||``);if(!r)return``;if(t.verified&&t.placeId){let e=t.photoUrl?`<img class="ai-place-card__photo" src="${n(t.photoUrl)}" alt="" referrerpolicy="no-referrer" loading="lazy">`:`<div class="ai-place-card__photo ai-place-card__photo--empty" aria-hidden="true">📍</div>`,i=typeof t.rating==`number`?`<span class="ai-place-card__rating">★ ${t.rating.toFixed(1)}${t.userRatingsTotal?` <span class="ai-place-card__rating-count">(${E(t.userRatingsTotal)})</span>`:``}</span>`:``,a=t.address?`<span class="ai-place-card__address">${n(t.address)}</span>`:``,o=t.why?`<span class="ai-place-card__why">${n(t.why)}</span>`:``,s=t.fact?`<span class="ai-place-card__fact">✨ ${n(t.fact)}</span>`:``,c=t.mapsUrl||`https://www.google.com/maps/place/?q=place_id:${encodeURIComponent(t.placeId)}`,l=t.verifiedName||r;return`
...
`):e.activity&&e.description?`${e.activity}: ${e.description}`:e.activity||e.description||``}var k=null,A=[];function j(){let e=document.createElement(`div`),t=p.trips.find(e=>e.id===p.activeTripId);if(!t)return e.innerHTML=`
            ...
            </div>`,setTimeout(()=>{e.querySelector(`#aiStartJourneyBtn`)?.addEventListener(`click`,()=>l()),typeof google<`u`&&google.maps&&m(new google.maps.Map(document.getElementById(`emptyMap`),{center:{lat:20,lng:0},zoom:2,minZoom:2,restriction:{latLngBounds:{north:85,south:-85,west:-180,east:180},strictBounds:!0},styles:[]}),[])},100),e;let S=t.country||``,C=(p.tripDays||[]).filter(e=>e.tripId===p.activeTripId&&e.dayNumber>0&&e.date).map(e=>e.date).sort(),T=p.expenses.filter(e=>e.tripId===p.activeTripId&&e.date).sort((e,t)=>e.date.localeCompare(t.date)).map(e=>e.date),E=t.dateFrom||C[0]||T[0]||``,D=t.dateTo||C[C.length-1]||T[T.length-1]||``,j=t.aiPlan||null,M=t.aiContext||``,N=t.aiNumDays||1,P=a(t),F=`font-family:-apple-system,BlinkMacSystemFont,'SF Pro Display','SF Pro Text',sans-serif;`;return e.innerHTML=`
        <div style="${F}">
            <!-- Header -->
            ...
            <div class="ai-page-2col" style="display:grid;grid-template-columns:380px 1fr;gap:24px;margin-bottom:32px;">
                ...
                <div id="aiControlsPanel" style="display:flex;flex-direction:column;gap:16px;min-height:700px;">
                    ...
                    <!-- Generate -->
                    ${P?`<button id="generateBtn" class="ai-generate-btn" style="width:100%; border-radius: var(--radius-lg);flex:0 0 auto;">✦ Generate My Itinerary</button>`:(()=>{let e=c(t);return`<div class="card glass" style="padding:16px; border-radius: var(--radius-lg); text-align:center; color: var(--text-secondary); font-size: 0.85rem; flex:0 0 auto;">
            ...
            <div id="itineraryOutput" style="margin-bottom: 60px;"></div>
        </div>`,setTimeout(()=>{let a=e=>{if(!k)return;let n=t.id+`_ai`;if(p.mapViews&&p.mapViews[n]){let e=p.mapViews[n];k.setCenter({lat:e.lat,lng:e.lng}),k.setZoom(e.zoom);return}if(t.viewport){let e=t.viewport;k.fitBounds(new google.maps.LatLngBounds({lat:e.south,lng:e.west},{lat:e.north,lng:e.east}));return}let r=e.replace(/\(USA\)/g,``).trim();r.includes(` - `)&&(r=r.split(` - `)[1]+`, USA`),new google.maps.Geocoder().geocode({address:r},(e,t)=>{t===`OK`&&e[0]&&k.fitBounds(e[0].geometry.viewport)})};if(typeof google<`u`&&google.maps){let n=document.getElementById(`aiGoogleMap`);n&&(k=new google.maps.Map(n,{center:{lat:20,lng:0},zoom:2,minZoom:2,mapTypeId:`roadmap`,disableDefaultUI:!0,restriction:{latLngBounds:{north:85,south:-85,west:-180,east:180},strictBounds:!0},styles:[]}),m(k,[]),a(S),k.addListener(`idle`,()=>{let e=t.id+`_ai`;p.mapViews||={};let n=k.getCenter();p.mapViews[e]={lat:n.lat(),lng:n.lng(),zoom:k.getZoom()},h(`state:changed`)}));let r=e.querySelector(`#aiZoomBadge`);r&&(r.onclick=()=>{let e=t.id+`_ai`;p.mapViews&&p.mapViews[e]&&delete p.mapViews[e],a(S)})}let c=j,l=(i,a,s)=>{let c=o(e,`#itineraryOutput`);if(!i||!i.length){c.innerHTML=``;return}c.innerHTML=`
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:24px;">
                    <div>
                        <!-- Phase G UX: text colour switched from white to a
                             ...
                             cards below. -->
                        <h2 style="margin:0;font-size:2rem;font-weight:800;letter-spacing:-0.03em;background:var(--gradient-title);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;${F}">${a}-Day ${s} Itinerary</h2>
                ...
                ${P?`<div style="display:flex;gap:12px;margin-top:24px;"><button id="acceptPlanBtn" class="btn" style="flex:2;background:var(--accent-blue);color:white;padding:16px;font-size:1.1rem;border-radius:16px;font-weight:700;box-shadow:0 10px 20px rgba(0,122,255,0.2);cursor:pointer;">Accept Plan & Add to Trip</button></div>`:``}`;let l=o(c,`#itineraryDays`),u=[];if(i.forEach((e,t)=>{let r=document.createElement(`div`);r.className=`card glass`,r.style.cssText=`border-radius:18px;overflow:hidden;transition:box-shadow 0.3s,border-color 0.3s;${F}`,r.innerHTML=`
            ...
            `,r.querySelector(`#aiManageTodoBtn`)?.addEventListener(`click`,()=>g(`todo`)),r.querySelectorAll(`.marked-day-select, .marked-time-select`).forEach(e=>{e.onchange=()=>{let n=e.dataset.placeId;if(!n)return;let i=r.querySelector(`.ai-marked-card[data-place-id="${n}"]`);if(!i)return;let a=i.querySelector(`.marked-day-select`),o=i.querySelector(`.marked-time-select`);u(t,n,a?.value||null,o?.value||null),h(`state:changed`),_(t)}})};C(),c&&l(c,N,S);let T=e.querySelector(`#aiExtraContext`);T&&(T.oninput=e=>{t.aiContext=e.target.value,h(`state:changed`)});let E=e.querySelector(`#aiKeyInput`),D=e.querySelector(`#aiKeyToggleBtn`),M=e.querySelector(`#aiKeyHelpBtn`),I=e.querySelector(`#aiKeyStatus`),L=()=>{if(!I)return;let e=(p.geminiApiKey||``).trim();if(!e){I.textContent=`No key saved — paste one above to enable AI generation.`,I.style.color=`#a85d00`;return}let t=e.startsWith(`AIza`)&&e.length>=30;I.textContent=t?`✓ Key saved on this device.`:`⚠ Saved, but the format looks off (Gemini keys usually start with "AIza"). Click i for help.`,I.style.color=t?`#1a6b3c`:`#a85d00`};L(),E&&E.addEventListener(`input`,()=>{p.geminiApiKey=E.value,h(`state:changed`),L()}),D&&E&&D.addEventListener(`click`,()=>{let e=E.type===`text`;E.type=e?`password`:`text`,D.textContent=e?`👁`:`🙈`,D.title=e?`Show key`:`Hide key`}),M&&M.addEventListener(`click`,()=>{let{root:e,close:t}=s({cardClass:`card glass`,cardStyle:`width: 520px; max-width: calc(100vw - 32px); max-height: 90vh; overflow-y: auto; padding: 28px 32px; border-radius: 28px; background: white;`,innerHTML:`
                    ...
                    `});e.querySelector(`#aiKeyHelpClose`)?.addEventListener(`click`,t),e.querySelector(`#aiKeyHelpDone`)?.addEventListener(`click`,t)}),[`#aiDateFrom`,`#aiDateTo`].forEach(t=>{let n=e.querySelector(t);n&&n.addEventListener(`change`,()=>C())});let R=e.querySelector(`#generateBtn`),z=async()=>{let r=o(e,`#itineraryOutput`),a=o(e,`#aiDateFrom`).value,s=o(e,`#aiDateTo`).value,u=document.getElementById(`aiExtraContext`)?.value??``;if(!a||!s){y(`Pick your travel dates first.`);return}let d=new Date(a),f=new Date(s),m=Math.max(1,Math.round((f.getTime()-d.getTime())/864e5)+1),g=i(t).filter(e=>e.forAI&&e.forManual),_=``;if(g.length>0){let e=(p.tripDays||[]).filter(e=>e.tripId===t.id&&e.dayNumber>0),n=t=>e.find(e=>e.id===t)?.dayNumber;_=`\n\nThe user has marked these specific places to include in the itinerary. Please incorporate them where they fit, respecting any day/time assignments where given:\n${g.map(e=>{let t=e.dayId?n(e.dayId):null,r=t?`, on Day ${t}`:``,i=e.timeOfDay?`, ${e.timeOfDay}`:``,a=e.address?` (${e.address})`:``;return`- ${e.name}${a}${r}${i}`}).join(`
`)}`}let v=u+_;t.aiContext=u,t.aiNumDays=m,h(`state:changed`),r.innerHTML=`<div style="text-align:center;padding:60px;"><div class="spinner-ring" style="width:40px;height:40px;border:3px solid rgba(0,113,227,0.15);border-top-color:#005bb8;border-radius:50%;animation:spin 1s linear infinite;margin:0 auto 20px;"></div><div style="color:var(--text-primary);font-weight:600;">Consulting Gemini AI…</div><div style="color:var(--text-secondary);font-size:0.82rem;margin-top:6px;">This usually takes 5-15 seconds. Maps lookups for each place add a few more.</div></div>`,r.scrollIntoView({behavior:`smooth`});let b=R?.innerHTML||``;R&&(R.disabled=!0,R.innerHTML=`⌛ Generating…`);try{let e=await(await x(`/api/generate_itinerary`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({destination:S,numDays:m,dateFrom:a,dateTo:s,context:v,gemini_key:(p.geminiApiKey||``).trim()})})).json();if(e.error)throw Error(e.error);c=e.itinerary,c==null?delete t.aiPlan:t.aiPlan=c,h(`state:changed`),l(c,m,S),r.scrollIntoView({behavior:`smooth`})}catch(e){let t=e.message||``,i=`Something went wrong while generating your plan.`,a=``;/UNAVAILABLE|503|overloaded/i.test(t)?(i=`Gemini is overloaded right now.`,a=`This usually clears in 30-60 seconds.`):/quota|limit|RESOURCE_EXHAUSTED|429/i.test(t)?(i=`Daily AI quota reached for this key.`,a=`Try again tomorrow, or use a different Gemini key in Settings → AI Engine.`):/key|api[_ ]?key|UNAUTHENTICATED|401|403/i.test(t)?(i=`AI key isn't accepted by Gemini.`,a=`Open Settings → AI Engine and check the key, or generate a new one.`):/network|fetch|timed?[\- ]?out|ECONN/i.test(t)&&(i=`Network hiccup talking to the AI.`,a=`Check your connection and retry.`),r.innerHTML=`
                    <div class="card glass" style="text-align:center;padding:32px 28px;">
                        <div style="font-size:2.4rem;margin-bottom:8px;">😬</div>
                        <h2 style="color:#a82424;margin:0 0 6px;font-size:1.2rem;">${n(i)}</h2>
                        ${a?`<p style="margin:0 0 18px;color:var(--text-secondary);font-size:0.9rem;line-height:1.5;">${n(a)}</p>`:``}
                        <details style="margin:0 0 18px;text-align:left;background:rgba(255,59,48,0.04);border:1px solid rgba(255,59,48,0.16);border-radius:10px;padding:8px 12px;">
                            ...
                            <pre style="margin:8px 0 0;font-size:0.72rem;color:#666;font-family:monospace;white-space:pre-wrap;word-break:break-word;">${n(t||`Unknown error`)}</pre>
                        </details>
                ...
                `,y(i),r.querySelector(`#aiRetryBtn`)?.addEventListener(`click`,()=>z())}finally{R&&(R.disabled=!1,R.innerHTML=b)}};R?.addEventListener(`click`,z)},0),e}var M=t();function N(){let e=(0,C.useRef)(null);return(0,C.useEffect)(()=>{let t=e.current;t&&(t.innerHTML=``,t.appendChild(j()))},[]),(0,M.jsx)(`div`,{ref:e})}function P(e){S(e,(0,C.createElement)(N))}export{P as mountAI};
//# sourceMappingURL=mount-CBV2kBsL.js.map