import{r as e,t}from"./vendor-react-CAxw18f3.js";import{$ as n,E as r,F as i,G as a,O as o,Q as s,S as c,U as l,V as u,X as d,Y as f,_ as p,a as m,at as h,g,it as _,j as v,k as y,n as b,ot as x,q as S,r as C,t as w,v as T}from"../app.bundle.js";var E=e();function D(e){if(!e)return``;let t=Array.isArray(e.items)?e.items.filter(Boolean):[];if(t.length>0)return`<ul class="ai-plan-block__list">${t.map(e=>O(e)).join(``)}</ul>`;if(e.description){let t=String(e.description).split(/\n+/).map(e=>e.trim()).filter(Boolean);return t.length>1?`<ul class="ai-plan-block__list">${t.map(e=>`<li>${f(e.replace(/^[-•*]\s*/,``))}</li>`).join(``)}</ul>`:`<div class="ai-plan-block__desc">${f(e.description)}</div>`}return``}function O(e){if(typeof e==`string`)return`<li class="ai-plan-block__item">${f(e)}</li>`;if(!e||typeof e!=`object`)return``;let t=e,n=String(t.text||``);if(!n)return``;if(t.verified&&t.placeId){let e=t.photoUrl?`<img class="ai-place-card__photo" src="${f(t.photoUrl)}" alt="" referrerpolicy="no-referrer" loading="lazy">`:`<div class="ai-place-card__photo ai-place-card__photo--empty" aria-hidden="true">📍</div>`,r=typeof t.rating==`number`?`<span class="ai-place-card__rating">★ ${t.rating.toFixed(1)}${t.userRatingsTotal?` <span class="ai-place-card__rating-count">(${k(t.userRatingsTotal)})</span>`:``}</span>`:``,i=t.address?`<span class="ai-place-card__address">${f(t.address)}</span>`:``,a=t.why?`<span class="ai-place-card__why">${f(t.why)}</span>`:``,o=t.fact?`<span class="ai-place-card__fact">✨ ${f(t.fact)}</span>`:``,s=t.mapsUrl||`https://www.google.com/maps/place/?q=place_id:${encodeURIComponent(t.placeId)}`,c=t.verifiedName||n;return`
...
`):e.activity&&e.description?`${e.activity}: ${e.description}`:e.activity||e.description||``}var M=null,N=[];function P(){let e=document.createElement(`div`),t=h.trips.find(e=>e.id===h.activeTripId);if(!t)return e.innerHTML=`
            ...
            </div>`,setTimeout(()=>{e.querySelector(`#aiStartJourneyBtn`)?.addEventListener(`click`,()=>i()),typeof google<`u`&&google.maps&&_(new google.maps.Map(document.getElementById(`emptyMap`),{center:{lat:20,lng:0},zoom:2,minZoom:2,gestureHandling:a(),restriction:{latLngBounds:{north:85,south:-85,west:-180,east:180},strictBounds:!0},styles:[]}),[])},100),e;let c=t.country||``,E=(h.tripDays||[]).filter(e=>e.tripId===h.activeTripId&&e.dayNumber>0&&e.date).map(e=>e.date).sort(),O=h.expenses.filter(e=>e.tripId===h.activeTripId&&e.date).sort((e,t)=>e.date.localeCompare(t.date)).map(e=>e.date),k=t.dateFrom||E[0]||O[0]||``,A=t.dateTo||E[E.length-1]||O[O.length-1]||``,P=t.aiPlan||null,F=t.aiContext||``,I=t.aiNumDays||1,L=u(t),R=`font-family:-apple-system,BlinkMacSystemFont,'SF Pro Display','SF Pro Text',sans-serif;`;return e.innerHTML=`
        <div style="${R}">
            <!-- Header -->
            <div style="padding:32px 0 24px;">
                ...
                <p style="margin:0;color:var(--text-secondary);font-size:1rem;">${w(`ai.subtitlePlanning`,{country:f(c)})}</p>
            </div>
            ...
            <div class="ai-page-2col" style="display:grid;grid-template-columns:380px 1fr;gap:24px;margin-bottom:32px;">
                ...
                <div id="aiControlsPanel" style="display:flex;flex-direction:column;gap:16px;min-height:700px;">
                    ...
                    <!-- Generate -->
                    ${L?`<button id="generateBtn" class="ai-generate-btn" style="width:100%; border-radius: var(--radius-lg);flex:0 0 auto;">${w(`ai.generateBtn`)}</button>`:(()=>{let e=l(t);return`<div class="card glass" style="padding:16px; border-radius: var(--radius-lg); text-align:center; color: var(--text-secondary); font-size: 0.85rem; flex:0 0 auto;">
            ...
            <div id="itineraryOutput" style="margin-bottom: 60px;"></div>
        </div>`,setTimeout(()=>{let i=e.querySelector(`#aiDateFrom`),l=e.querySelector(`#aiDateTo`),u=e.querySelector(`#aiDateHint`);if(i&&l){let e=u?.textContent||``,t=u?.style.color||``,n=()=>{l.min=i.value||``,i.value&&l.value&&l.value<i.value?(u&&(u.textContent=`End date must be on or after the start date.`,u.style.color=`#a82424`),l.setCustomValidity(`End date must be on or after the start date.`)):(u&&(u.textContent=e,u.style.color=t),l.setCustomValidity(``))};i.addEventListener(`change`,n),l.addEventListener(`change`,n),n()}let E=e=>{if(!M)return;let n=t.id+`_ai`;if(h.mapViews&&h.mapViews[n]){let e=h.mapViews[n];M.setCenter({lat:e.lat,lng:e.lng}),M.setZoom(e.zoom);return}if(t.viewport){let e=t.viewport;M.fitBounds(new google.maps.LatLngBounds({lat:e.south,lng:e.west},{lat:e.north,lng:e.east}));return}let r=e.replace(/\(USA\)/g,``).trim();r.includes(` - `)&&(r=r.split(` - `)[1]+`, USA`),new google.maps.Geocoder().geocode({address:r},(e,t)=>{t===`OK`&&e[0]&&M.fitBounds(e[0].geometry.viewport)})};if(typeof google<`u`&&google.maps){let n=document.getElementById(`aiGoogleMap`);n&&(M=new google.maps.Map(n,{center:{lat:20,lng:0},zoom:2,minZoom:2,mapTypeId:`roadmap`,disableDefaultUI:!0,gestureHandling:a(),restriction:{latLngBounds:{north:85,south:-85,west:-180,east:180},strictBounds:!0},styles:[]}),_(M,[]),E(c),M.addListener(`idle`,()=>{let e=t.id+`_ai`;h.mapViews||={};let n=M.getCenter();h.mapViews[e]={lat:n.lat(),lng:n.lng(),zoom:M.getZoom()},x(`state:changed`)}));let r=e.querySelector(`#aiZoomBadge`);r&&(r.onclick=()=>{let e=t.id+`_ai`;h.mapViews&&h.mapViews[e]&&delete h.mapViews[e],E(c)})}let O=P,k=(n,i,a)=>{let c=s(e,`#itineraryOutput`);if(!n||!n.length){c.innerHTML=``;return}c.innerHTML=`
                ...
                ${L?`<div style="display:flex;gap:12px;margin-top:24px;"><button id="acceptPlanBtn" class="btn" style="flex:2;background:var(--accent-blue);color:white;padding:16px;font-size:1.1rem;border-radius:16px;font-weight:700;box-shadow:0 10px 20px rgba(0,122,255,0.2);cursor:pointer;">${w(`ai.acceptPlanBtn`)}</button></div>`:``}`;let l=s(c,`#itineraryDays`),u=[];if(n.forEach((e,t)=>{let n=document.createElement(`div`);n.className=`card glass`,n.style.cssText=`border-radius:18px;overflow:hidden;transition:box-shadow 0.3s,border-color 0.3s;${R}`,n.innerHTML=`
                ...
                `,n.querySelector(`#aiGoToTodoBtn`)?.addEventListener(`click`,()=>T(`todo`));return}if(i.length===0){n.innerHTML=`
                    <div class="card glass" style="padding: 20px; border-radius: 18px; border: 1.5px dashed rgba(155, 89, 182, 0.35); background: rgba(155, 89, 182, 0.04);">
                        <div style="display:flex; align-items:center; gap:10px; margin-bottom:6px;">
                            <span style="font-size: 1.2rem;">📋</span>
                            <h3 style="margin:0; color:#7c3a9e; font-weight:800; letter-spacing:-0.01em;">${b(`ai.todoPanelNoneTickedTitle`,r.length)}</h3>
                        </div>
                ...
                `,n.querySelector(`#aiGoToTodoBtn`)?.addEventListener(`click`,()=>T(`todo`));return}let a=e.querySelector(`#aiDateFrom`),o=e.querySelector(`#aiDateTo`),s=!!(a?.value&&o?.value),c=(h.tripDays||[]).filter(e=>e.tripId===t.id&&e.dayNumber>0).sort((e,t)=>e.dayNumber-t.dayNumber),l=e=>`
                <option value="" ${e?``:`selected`}>${w(`ai.dayOptionAny`)}</option>
                ${c.map(t=>`
                    <option value="${f(t.id)}" ${t.id===e?`selected`:``}>
                        ${w(`ai.dayOptionDay`,{num:t.dayNumber})}${t.date?` — ${d(t.date)||t.date}`:``}
                    </option>
                `).join(``)}
            `,u=e=>`
            ...
            `).join(``);n.innerHTML=`
                <div class="card glass" style="padding:20px; border-radius:18px; border: 1.5px solid rgba(155, 89, 182, 0.25);">
                    <div style="display:flex; align-items:center; gap:10px; margin-bottom:14px; flex-wrap:wrap;">
                        <span style="font-size: 1.2rem;">📋</span>
                        <h3 style="margin:0; color:#7c3a9e; font-weight:800; letter-spacing:-0.01em;">${w(`ai.todoPanelTickedTitle`)} <span style="background:rgba(155,89,182,0.12); color:#7c3a9e; font-size:0.7rem; padding:2px 8px; border-radius:999px; margin-left:6px;">${b(`ai.todoPanelTickedCount`,i.length)}</span></h3>
                    ...
                    <p style="font-size:0.82rem; color:var(--text-secondary); margin:0 0 12px; line-height:1.5;">${w(s?`ai.todoPanelHintWithDates`:`ai.todoPanelHintNoDates`)}</p>
                    <div style="display:grid; grid-template-columns: repeat(auto-fill, minmax(260px, 1fr)); gap:12px;">
                        ${m}
                    </div>
                </div>
            `,n.querySelector(`#aiManageTodoBtn`)?.addEventListener(`click`,()=>T(`todo`)),n.querySelectorAll(`.marked-day-select, .marked-time-select`).forEach(e=>{e.onchange=()=>{let r=e.dataset.placeId;if(!r)return;let i=n.querySelector(`.ai-marked-card[data-place-id="${r}"]`);if(!i)return;let a=i.querySelector(`.marked-day-select`),o=i.querySelector(`.marked-time-select`);v(t,r,a?.value||null,o?.value||null),x(`state:changed`),p(t)}})};A(),O&&k(O,I,c);let F=e.querySelector(`#aiExtraContext`);F&&(F.oninput=e=>{t.aiContext=e.target.value,x(`state:changed`)});let z=e.querySelector(`#aiKeyInput`),B=e.querySelector(`#aiKeyToggleBtn`),V=e.querySelector(`#aiKeyHelpBtn`),H=e.querySelector(`#aiKeyStatus`),U=()=>{if(!H)return;let e=(h.geminiApiKey||``).trim();if(!e){H.textContent=w(`ai.keyStatusEmpty`),H.style.color=`#a85d00`;return}let t=e.startsWith(`AIza`)&&e.length>=30;H.textContent=t?`✓ Key saved on this device.`:`⚠ Saved, but the format looks off (Gemini keys usually start with "AIza"). Click i for help.`,H.style.color=t?`#1a6b3c`:`#a85d00`};U(),z&&z.addEventListener(`input`,()=>{h.geminiApiKey=z.value,x(`state:changed`),U()}),B&&z&&B.addEventListener(`click`,()=>{let e=z.type===`text`;z.type=e?`password`:`text`,B.textContent=e?`👁`:`🙈`,B.title=w(e?`ai.keyToggleShow`:`ai.keyToggleHide`)}),V&&V.addEventListener(`click`,()=>{let{root:e,close:t}=S({cardClass:`card glass`,cardStyle:`width: 520px; max-width: calc(100vw - 32px); max-height: 90vh; overflow-y: auto; padding: 28px 32px; border-radius: 28px; background: white;`,innerHTML:`
                    ...
                    `});e.querySelector(`#aiKeyHelpClose`)?.addEventListener(`click`,t),e.querySelector(`#aiKeyHelpDone`)?.addEventListener(`click`,t)}),[`#aiDateFrom`,`#aiDateTo`].forEach(t=>{let n=e.querySelector(t);n&&n.addEventListener(`change`,()=>A())});let W=e.querySelector(`#generateBtn`),G=async()=>{let r=s(e,`#itineraryOutput`),i=s(e,`#aiDateFrom`).value,a=s(e,`#aiDateTo`).value,o=document.getElementById(`aiExtraContext`)?.value??``;if(!i||!a){n(w(`ai.toastPickDates`));return}if(a<i){n(w(`ai.toastEndBeforeStart`));return}let l=new Date(i),u=new Date(a),d=Math.max(1,Math.round((u.getTime()-l.getTime())/864e5)+1),p=y(t).filter(e=>e.forAI&&e.forManual),m=``;if(p.length>0){let e=(h.tripDays||[]).filter(e=>e.tripId===t.id&&e.dayNumber>0),n=t=>e.find(e=>e.id===t)?.dayNumber;m=`\n\nThe user has marked these specific places to include in the itinerary. Please incorporate them where they fit, respecting any day/time assignments where given:\n${p.map(e=>{let t=e.dayId?n(e.dayId):null,r=t?`, on Day ${t}`:``,i=e.timeOfDay?`, ${e.timeOfDay}`:``,a=e.address?` (${e.address})`:``;return`- ${e.name}${a}${r}${i}`}).join(`
`)}`}let g=o+m;t.aiContext=o,t.aiNumDays=d,x(`state:changed`),r.innerHTML=`<div style="text-align:center;padding:60px;"><div class="spinner-ring" style="width:40px;height:40px;border:3px solid rgba(0,113,227,0.15);border-top-color:#005bb8;border-radius:50%;animation:spin 1s linear infinite;margin:0 auto 20px;"></div><div style="color:var(--text-primary);font-weight:600;">${w(`ai.loadingTitle`)}</div><div style="color:var(--text-secondary);font-size:0.82rem;margin-top:6px;">${w(`ai.loadingBody`)}</div></div>`,r.scrollIntoView({behavior:`smooth`});let _=W?.innerHTML||``;W&&(W.disabled=!0,W.innerHTML=w(`ai.generatingBtn`));try{let e=await(await C(`/api/generate_itinerary`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({destination:c,numDays:d,dateFrom:i,dateTo:a,context:g,gemini_key:(h.geminiApiKey||``).trim()})})).json();if(e.error)throw Error(e.error);O=e.itinerary,O==null?delete t.aiPlan:t.aiPlan=O,x(`state:changed`),k(O,d,c),r.scrollIntoView({behavior:`smooth`})}catch(e){let t=e.message||``,i=w(`ai.errorGeneric`),a=``;/UNAVAILABLE|503|overloaded/i.test(t)?(i=w(`ai.errorOverloaded`),a=w(`ai.errorOverloadedHint`)):/quota|limit|RESOURCE_EXHAUSTED|429/i.test(t)?(i=w(`ai.errorQuota`),a=w(`ai.errorQuotaHint`)):/key|api[_ ]?key|UNAUTHENTICATED|401|403/i.test(t)?(i=w(`ai.errorBadKey`),a=w(`ai.errorBadKeyHint`)):/network|fetch|timed?[\- ]?out|ECONN/i.test(t)&&(i=w(`ai.errorNetwork`),a=w(`ai.errorNetworkHint`)),r.innerHTML=`
                    <div class="card glass" style="text-align:center;padding:32px 28px;">
                        <div style="font-size:2.4rem;margin-bottom:8px;">😬</div>
                        <h2 style="color:#a82424;margin:0 0 6px;font-size:1.2rem;">${f(i)}</h2>
                        ${a?`<p style="margin:0 0 18px;color:var(--text-secondary);font-size:0.9rem;line-height:1.5;">${f(a)}</p>`:``}
                        <details style="margin:0 0 18px;text-align:left;background:rgba(255,59,48,0.04);border:1px solid rgba(255,59,48,0.16);border-radius:10px;padding:8px 12px;">
                            ...
                            <pre style="margin:8px 0 0;font-size:0.72rem;color:#666;font-family:monospace;white-space:pre-wrap;word-break:break-word;">${f(t||w(`ai.errorUnknown`))}</pre>
                        </details>
                ...
                `,n(i),r.querySelector(`#aiRetryBtn`)?.addEventListener(`click`,()=>G())}finally{W&&(W.disabled=!1,W.innerHTML=_)}};W?.addEventListener(`click`,G)},0),e}var F=t();function I(){let e=(0,E.useRef)(null);return(0,E.useEffect)(()=>{let t=e.current;t&&(t.innerHTML=``,t.appendChild(P()))},[]),(0,F.jsx)(`div`,{ref:e})}function L(e){c(e,(0,E.createElement)(I))}export{L as mountAI};
//# sourceMappingURL=mount-DgQc4QbQ.js.map