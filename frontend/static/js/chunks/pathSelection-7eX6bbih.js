import{Dt as e,Et as t,Mt as n,Wt as r,b as i,y as a}from"../app.bundle.js";var o=`home_path_card_collapsed_day_ids`;function s(){try{let e=localStorage.getItem(o);if(!e)return new Set;let t=JSON.parse(e);return new Set(Array.isArray(t)?t:[])}catch{return new Set}}function c(e){try{localStorage.setItem(o,JSON.stringify([...e]))}catch{}}function l(e){let t=s();return t.has(e)?(t.delete(e),c(t),!1):(t.add(e),c(t),!0)}function u(e){return s().has(e)}function d(e,t){let n=s(),r=n.has(e);t&&!r?(n.add(e),c(n)):!t&&r&&(n.delete(e),c(n))}function f(r,i,o){let{isAnchor:s,isSelected:c}=i,l=s?`<div style="background: var(--gradient-anchor-deep); color: white; width: 48px; height: 48px; border-radius: 50%; border: 3px solid white; display: flex; align-items: center; justify-content: center; flex-shrink:0; box-shadow: 0 8px 18px rgba(212,160,23,0.28);">
               ...
               <span style="font-size: 0.6rem; font-weight: 800; text-transform: uppercase; opacity: 0.85; letter-spacing: 0.05em; line-height:1;">${t(a(`pathTab.dayBadgeLabel`))}</span>
               <span style="font-size: 1.25rem; font-weight: 800; line-height: 1.05;">${r.dayNumber}</span>
           </div>`,u=s?a(`pathTab.hubTitle`):t(r.name||a(`tripMedia.dayBucketDay`,{n:r.dayNumber})),d=[];if(s){d.push(o&&o.country?t(n(o.country)):t(a(`pathTab.hubSubtitleFallback`)));let e=(o.documents||[]).filter(e=>e.dayId===r.id),i=(o.photos||[]).filter(e=>e.dayId===r.id),s=e.length+(r.tickets||[]).length,c=i.length+(r.photos||[]).length;c&&d.push(`<span style="background:rgba(52,199,89,0.12); color:#1a6b3c; padding:2px 8px; border-radius:999px; font-size:0.7rem; font-weight:800;">📸 ${c}</span>`),s&&d.push(`<span style="background:rgba(88,86,214,0.12); color:#5856d6; padding:2px 8px; border-radius:999px; font-size:0.7rem; font-weight:800;">📎 ${s}</span>`)}else d.push(`📅 ${e(r.date)||a(`pathTab.setDatePlaceholder`)}`),r.lat?d.push(`<span style="color: #005bb8;">${t(a(`pathTab.locationSet`))}</span>`):d.push(`<span class="day-card__pin-hint">${t(a(`pathTab.pinThisDay`))}</span>`),r.date&&d.push(`<span class="day-card__weather" data-weather-date="${t(r.date)}"></span>`);let f=c&&r.notes&&!s?`
        <div style="margin-top: 12px; padding: 12px 14px; background: rgba(0,113,227,0.04); border-radius: 14px; border-left: 3px solid var(--accent-blue);">
            <div style="font-size: 0.7rem; font-weight: 800; text-transform: uppercase; color: #005bb8; margin-bottom: 4px; letter-spacing: 0.05em;">${t(a(`pathTab.journalPreviewLabel`))}</div>
    ...
    `:``,p=`
        <button type="button" class="path-card-collapse-btn" data-day-id="${t(r.id)}"
            aria-label="${t(a(`pathTab.toggleOptionsAria`,{title:u}))}" title="${t(a(`pathTab.toggleOptionsTitle`))}">
    ...
    `;return`
        ...
        ${f}
    `}function p(e,n,r,i){if(!e||!r)return``;let{isAnchor:o}=n,s=[];if(o?s.push(`<button class="path-primary-btn path-primary-btn--anchor path-checklist-btn" data-day-id="${t(e.id)}">${t(a(`pathTab.btnChecklist`))}</button>`):s.push(`<button class="path-primary-btn day-detail-btn" data-day-id="${t(e.id)}">${t(a(`pathTab.btnOpenFullPlan`))}</button>`),i===e.id)s.push(`<button class="day-action-btn day-action-btn--success day-pin-save-btn" data-day-id="${t(e.id)}">${t(a(`pathTab.btnSavePin`))}</button>`),s.push(`<button class="day-action-btn day-action-btn--danger-fill day-pin-delete-btn" data-day-id="${t(e.id)}">${t(a(`pathTab.btnCancelPinEdit`))}</button>`);else{let n=e.lat?a(o?`pathTab.btnEditAnchorPin`:`pathTab.btnEditPin`):a(o?`pathTab.btnSetAnchorPin`:`pathTab.btnAddPin`);s.push(`<button class="day-action-btn day-action-btn--neutral day-pin-toggle-btn" data-day-id="${t(e.id)}"><span>${t(n)}</span></button>`)}return o?(s.push(`<button class="day-action-btn day-action-btn--neutral path-documents-btn" data-day-id="${t(e.id)}"><span>${t(a(`pathTab.btnDocuments`))}</span></button>`),s.push(`<button class="day-action-btn day-action-btn--neutral path-photos-btn" data-day-id="${t(e.id)}"><span>${t(a(`pathTab.btnPhotos`))}</span></button>`)):(s.push(`<button class="day-action-btn day-action-btn--neutral day-journaling-btn" data-day-id="${t(e.id)}"><span>${t(a(`pathTab.btnJournaling`))}</span></button>`),s.push(`<button class="day-action-btn day-action-btn--danger day-delete-btn" data-day-id="${t(e.id)}"><span>${t(a(`pathTab.btnDeleteDay`))}</span></button>`)),`<div class="path-options-stack">${s.join(``)}</div>`}function m(n){let{activeTrip:r,tripDays:o,tripIsEditable:s,editingDayId:c}=n,l=[...o].sort((e,t)=>e.dayNumber-t.dayNumber),d=l.find(e=>e.dayNumber===0)||null,m=l.filter(e=>e.dayNumber>0),h=x(r,l),g=l.find(e=>e.id===h)||null;if(l.length===0)return`<div class="card glass" style="padding:28px; border-radius:18px; text-align:center; color:var(--text-secondary);">${t(a(`pathTab.emptyState`))}</div>`;let _=m.length,v=g?.dayNumber===0?i(`path.summaryHub`,_,{count:_}):g?a(`path.summaryDay`,{day:g.dayNumber,total:_}):i(`path.summaryNone`,_,{count:_}),y=(()=>{let e=new Date;return`${e.getFullYear()}-${String(e.getMonth()+1).padStart(2,`0`)}-${String(e.getDate()).padStart(2,`0`)}`})(),b=l.map(n=>{let r=n.id===h,i=n.dayNumber===0,o=!i&&n.date===y,s=`path-chip${i?` path-chip--anchor`:``}${o?` path-chip--today`:``}${r?` is-selected`:``}`,c=i?a(`pathTab.chipHubTooltip`):`${o?a(`pathTab.chipTodayPrefix`)+` · `:``}${a(`tripMedia.dayBucketDay`,{n:n.dayNumber})}${n.name?` — `+n.name:``}${n.date?` · `+(e(n.date)||n.date):``}`,l=i?`<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" stroke-width="1.2" stroke-linejoin="round" aria-hidden="true"><polygon points="12 2 15 8.5 22 9.3 17 14.3 18.2 21.3 12 18 5.8 21.3 7 14.3 2 9.3 9 8.5"/></svg>`:String(n.dayNumber);return`<button type="button" class="${s}" data-path-chip-day-id="${t(n.id)}" title="${t(c)}" aria-label="${t(c)}" aria-pressed="${r}">${l}</button>`}).join(``),S=s?`<button type="button" class="path-chip path-chip--add" id="pathAddDayChip" title="${t(a(`pathTab.addNewDay`))}" aria-label="${t(a(`pathTab.addNewDay`))}">+</button>`:``,C=l.findIndex(e=>e.id===h),w=C<=0,T=C<0||C>=l.length-1,E=[];if(d){let e=g?.id===d.id,n=u(d.id);E.push(`
        ...
        <div class="path-strip">
            <button type="button" class="path-nav-btn" id="pathPrevBtn" title="${t(a(`pathTab.previousDay`))}" aria-label="${t(a(`pathTab.previousDay`))}" ${w?`disabled`:``}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6"></polyline></svg>
            </button>
            <div class="path-chips" role="group" aria-label="${t(a(`pathTab.tripDaysGroupAria`))}">
                ${b}
                ${S}
            </div>
            <button type="button" class="path-nav-btn" id="pathNextBtn" title="${t(a(`pathTab.nextDay`))}" aria-label="${t(a(`pathTab.nextDay`))}" ${T?`disabled`:``}>
...
//# sourceMappingURL=pathSelection-7eX6bbih.js.map