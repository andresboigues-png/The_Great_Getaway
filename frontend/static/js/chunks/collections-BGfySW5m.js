import{$ as e,At as t,B as n,C as r,Ct as i,Et as a,Ft as o,G as s,Gt as c,L as l,P as u,Q as d,S as f,Wt as p,_t as m,a as h,gt as g,mt as _,q as v,vt as y,x as b,y as x,yt as S}from"../app.bundle.js";var C=async(e,t)=>{let n=p.archivedTrips.find(t=>t.id===e)||p.trips.find(t=>t.id===e);if(n&&(n.isPublic=t!==`private`,n.publicShowExpenses=t===`public-full`,c(`state:changed`),p.user))try{await d(n)}catch{}},w=t=>{let n=p.archivedTrips.find(e=>e.id===t);n&&i({title:`Restore Trip?`,message:`This will move the trip back to your active list.`,confirmText:`Restore`,onConfirm:()=>{n.isArchived=!1,n.expenses&&(p.expenses=[...p.expenses,...n.expenses],delete n.expenses),n.tripDays&&(p.tripDays=[...p.tripDays,...n.tripDays],delete n.tripDays),p.trips.push(n),p.archivedTrips=p.archivedTrips.filter(e=>e.id!==t),p.activeTripId=t,c(`state:changed`),s(t),e(`home`)}})},T=t=>{i({title:`Delete Permanently?`,message:`This trip and all its memories will be gone forever.`,confirmText:`Delete`,onConfirm:async()=>{if(p.archivedTrips=p.archivedTrips.filter(e=>e.id!==t),c(`state:changed`),p.user)try{await fetch(f(`/api/trips/delete`),{method:`DELETE`,headers:{"Content-Type":`application/json`},body:JSON.stringify({user_id:p.user.id,trip_id:t})})}catch{}e(`collections`)}})};function E(s){let d=typeof s==`string`?p.archivedTrips.find(e=>e.id===s)||p.trips.find(e=>e.id===s):s,f=document.createElement(`div`);if(!d)return f.innerHTML=`<p style="padding: 40px; text-align: center;">${a(x(`archivedDetail.notFound`))}</p>`,f;let b=(d.expenses||[]).filter(e=>!e.isSettlement),T=b.reduce((e,t)=>e+(t.euroValue||0),0),E=d.tripDays||[],D=E.filter(e=>(e.dayNumber||0)>0).length,O=Array.isArray(d.photos)?d.photos:[],k=Array.isArray(d.documents)?d.documents:[],A=E.reduce((e,t)=>e+(t.photos||[]).length,0)+O.length,j=E.reduce((e,t)=>e+(t.tickets||[]).length,0)+k.length,M=null;if(d.coverUrl&&(M=d.coverUrl),!M&&O.length>0&&(M=O[0].src),!M){for(let e of E)if(e.photos&&e.photos.length>0){M=e.photos[0];break}}let N=M?`background: linear-gradient(135deg, rgba(0,45,91,0.55), rgba(88,86,214,0.45)), url(${a(M)}) center/cover no-repeat;`:`background: linear-gradient(135deg, #007aff 0%, #5856d6 60%, #34c759 130%);`,P=`#ffffff`,F=`rgba(255,255,255,0.85)`,I=`rgba(255,255,255,0.16)`,L=`1px solid rgba(255,255,255,0.25)`,R=(e,t,n)=>`
    ...
    `;f.innerHTML=`
        <div class="archived-hero" style="position:relative; overflow:hidden; border-radius:36px; padding:48px 52px; ${N} box-shadow: 0 30px 80px rgba(0, 45, 91, 0.25); margin-bottom: 32px; border: 1px solid rgba(255,255,255,0.18);">
            ...
            <div class="archived-hero__actions">
                ...
                <!-- Share button — now ALWAYS visible (no isPublic
                     ...
                     first. -->
                <button id="shareTripBtn" type="button" data-trip-id="${a(d.id)}" title="${a(x(`archivedDetail.shareBtnTitle`))}" aria-label="${a(x(`archivedDetail.shareBtnTitle`))}"
                    class="ad-pill-glass">
                ...
                <button id="cloneTripBtn" type="button" data-trip-id="${a(d.id)}" title="${a(x(`archivedDetail.cloneBtnTitle`))}" aria-label="${a(x(`archivedDetail.cloneBtnAria`))}"
                    class="ad-pill-glass">
                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                ...
                <button class="restore-trip-btn" data-trip-id="${a(d.id)}" type="button" style="background:#ffffff; color:#002d5b; padding:10px 18px; border-radius:999px; font-weight:800; font-size:0.85rem; cursor:pointer; box-shadow:0 4px 14px rgba(0,0,0,0.18); border: 0;">${a(x(`archivedDetail.restoreBtn`))}</button>
            </div>
            ...
            <div style="position:relative; z-index:1; max-width: calc(100% - 260px);">
                ...
                <h1 style="font-size: 3.2rem; margin: 0; letter-spacing: -0.04em; color: ${P}; font-weight: 800; line-height: 1; text-shadow: 0 2px 24px rgba(0,0,0,0.2);">${a(d.name)}</h1>
                ${d.country?`<div style="margin-top:10px; font-size:1rem; color:${F}; font-weight:600; display:flex; align-items:center; gap:8px;">📍 ${a(d.country)}</div>`:``}
            </div>
            ...
            <div style="position:relative; z-index:1; display:flex; flex-wrap:wrap; gap:10px; margin-top:24px;">
                ...
                <div style="display:flex; align-items:center; gap:6px; background:${I}; border:${L}; padding:6px 14px; border-radius:999px; backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px);">
                    <select
                        ...
                        <option value="private" ${d.isPublic?``:`selected`} class="text-brand-navy">${a(x(`archivedDetail.visibilityPrivate`))}</option>
                        <option value="public-plan" ${d.isPublic&&!d.publicShowExpenses?`selected`:``} class="text-brand-navy">${a(x(`archivedDetail.visibilityPublicPlan`))}</option>
                        <option value="public-full" ${d.isPublic&&d.publicShowExpenses?`selected`:``} class="text-brand-navy">${a(x(`archivedDetail.visibilityPublicAll`))}</option>
                    </select>
                </div>
            </div>
        </div>
        ...
        ${(()=>{let e=e=>{if(!e)return null;let t=E.find(t=>t.id===e);return t?Number(t.dayNumber)===0?x(`archivedDetail.dayBadgeHub`):x(`tripMedia.dayBucketDay`,{n:t.dayNumber}):null},t=e=>{if(!e)return!1;let t=E.find(t=>t.id===e);return!!t&&Number(t.dayNumber)===0},n=n=>{if(t(n))return`<span style="background:rgba(52,199,89,0.12); color:#1a6b3c; padding:2px 10px; border-radius:999px; font-size:0.65rem; font-weight:800; text-transform:uppercase; letter-spacing:0.06em;">${a(x(`archivedDetail.dayBadgeHub`))}</span>`;let r=e(n);return r?`<span style="background:rgba(0,113,227,0.08); color:var(--accent-blue); padding:2px 10px; border-radius:999px; font-size:0.65rem; font-weight:800; text-transform:uppercase; letter-spacing:0.06em;">${a(r)}</span>`:`<span style="background:rgba(0,0,0,0.05); color:rgba(0,0,0,0.45); padding:2px 10px; border-radius:999px; font-size:0.65rem; font-weight:800; text-transform:uppercase; letter-spacing:0.06em;">${a(x(`archivedDetail.dayBucketUnsorted`))}</span>`},r=[],i=x(`tripMedia.docsFallbackName`);k.forEach(e=>r.push({name:e.name||i,url:e.url||``,dayId:e.dayId||null,source:`trip`,_key:e.id||`${e.name}-${e.url}`})),E.forEach(e=>{(e.tickets||[]).forEach((t,n)=>r.push({name:t.name||i,url:t.url||``,dayId:e.id,source:`day`,_key:`${e.id}#${n}`}))});let o=e=>{if(!e)return-1;let t=E.find(t=>t.id===e);return t?t.dayNumber:999};r.sort((e,t)=>o(e.dayId)-o(t.dayId));let s=[];O.forEach(e=>s.push({src:e.src||``,dayId:e.dayId||null,source:`trip`,_key:e.id||e.src})),E.forEach(e=>{(e.photos||[]).forEach((t,n)=>s.push({src:t,dayId:e.id,source:`day`,_key:`${e.id}#${n}`}))}),s.sort((e,t)=>o(e.dayId)-o(t.dayId));let c=e=>/^data:image\//i.test(e||``)||/\.(jpe?g|png|gif|webp|avif|heic|heif|bmp|tiff?|svg)(\?.*)?$/i.test(e||``);return(r.length===0?``:`
            ...
            `)})()}
    `,f.querySelector(`#backToCollectionsBtn`)?.addEventListener(`click`,()=>e(`collections`));let z=f.querySelector(`#shareTripBtn`);return z&&u(d.id).then(e=>{e?.shared&&(z.dataset.shared=`1`,z.dataset.postId=String(e.post_id),m(z,!0))}),f.addEventListener(`click`,async o=>{let s=o.target,u=s?.closest(`.restore-trip-btn`);if(u?.dataset.tripId){w(u.dataset.tripId);return}let f=s?.closest(`#cloneTripBtn`);if(f?.dataset.tripId){f.setAttribute(`disabled`,`true`);let n=f.innerHTML;f.innerHTML=a(x(`archivedDetail.cloneStatusCloning`));try{let i=await r(f.dataset.tripId);if(!i?.ok||!i.body?.tripId){t(x(`archivedDetail.cloneError`)),f.removeAttribute(`disabled`),f.innerHTML=n;return}let a=i.body.tripId;p.activeTripId=a,await l(),p.activeTripId=a,c(`state:changed`),t(x(`archivedDetail.cloneSuccess`)),e(`home`)}catch(e){console.error(`Clone failed:`,e),t(x(`archivedDetail.cloneError`)),f.removeAttribute(`disabled`),f.innerHTML=n}return}let b=s?.closest(`#shareTripBtn`);if(b){h({trip:d,onShareToFeed:()=>{if(b.dataset.shared===`1`){let e=Number(b.dataset.postId||0);if(!e)return;i({title:x(`archivedDetail.unshareConfirmTitle`),message:x(`archivedDetail.unshareConfirmBody`),confirmText:x(`archivedDetail.unshareConfirmBtn`),onConfirm:async()=>{let n=await v(e);if(!n||!n.ok){t(x(`archivedDetail.unshareError`));return}b.dataset.shared=`0`,b.dataset.postId=``,m(b,!1),t(x(`archivedDetail.unshareSuccess`))}});return}g(d,async e=>{let r=await n(d.id,e);if(!r||!r.ok){let e=r?.status??`no-response`,n=r?.body?.error||``;t(`Share failed — HTTP ${e}`+(n?` · ${n}`:``)),console.error(`[collections.share] failed`,{tripId:d.id,status:e,body:r?.body});return}let i=Number(r.body?.post_id)||0;i&&(b.dataset.shared=`1`,b.dataset.postId=String(i),m(b,!0)),r.body?.status===`already_shared`?t(x(e?`archivedDetail.shareUpdated`:`archivedDetail.shareAlready`)):t(x(`archivedDetail.shareSuccess`))})}});return}let C=s?.closest(`a[href]`);if(C&&y(C.href)){let e=o;if(!e.metaKey&&!e.ctrlKey&&!e.shiftKey&&e.button!==1){e.preventDefault();let t=C.querySelector(`span`)?.textContent?.trim()||`Document`;S(C.href,t);return}}let T=s?.closest(`.archived-day-block`);if(T?.dataset.dayId){let e=(d.tripDays||[]).find(e=>e.id===T.dataset.dayId);e&&_(e);return}}),f.addEventListener(`change`,e=>{let t=e.target?.closest(`.trip-privacy-select`);t?.dataset.tripId&&C(t.dataset.tripId,t.value)}),f}var D=async e=>{let t=document.getElementById(`app-container`);if(!t)return;let n=p.archivedTrips.find(t=>t.id===e)||p.trips.find(t=>t.id===e);if(n){t.innerHTML=``,t.appendChild(E(n));return}t.innerHTML=`<div style="padding:60px 20px; text-align:center; color:var(--text-secondary); font-size:0.95rem;">${x(`collections.loadingTrip`)}</div>`;try{let n=await b(`/api/public-trip/${encodeURIComponent(e)}`);if(!n.ok){t.innerHTML=`<div style="padding:60px 20px; text-align:center; color:var(--text-secondary);">${x(`collections.tripUnavailable`)}</div>`;return}let r=await n.json();if(!r?.trip){t.innerHTML=`<div style="padding:60px 20px; text-align:center; color:var(--text-secondary);">${x(`collections.tripNotFound`)}</div>`;return}t.innerHTML=``,t.appendChild(E(r.trip))}catch(e){console.error(`viewArchivedDetails fetch failed:`,e),t.innerHTML=`<div style="padding:60px 20px; text-align:center; color:var(--text-secondary);">${x(`collections.loadFailed`)}</div>`}};export{C as i,T as n,w as r,D as t};
//# sourceMappingURL=collections-BGfySW5m.js.map