import{n as e}from"./rolldown-runtime-Kw0j5LDr.js";import{$t as t,Cn as n,O as r,S as i,Zt as a,b as o,bt as s,cn as c,nn as l,sn as u,wn as d}from"../app.bundle.js";var f=e=>{if(!e||!e.id){c(i(`modals.pdfErrorNoTrip`));return}let n=e.name||i(`feed.tripFallback`),{root:r,close:d}=a({innerHTML:`
        <div style="display:flex; flex-direction:column; text-align:left;">
            ...
            <div style="display:flex; align-items:center; gap:14px; padding:18px 22px; background:linear-gradient(135deg, var(--accent-blue) 0%, #5856d6 100%); color:white; border-top-left-radius: var(--radius-3xl); border-top-right-radius: var(--radius-3xl);">
                <div style="width:44px; height:44px; border-radius:12px; background:rgba(255,255,255,0.18); backdrop-filter:blur(8px); border:1px solid rgba(255,255,255,0.28); display:inline-flex; align-items:center; justify-content:center; flex-shrink:0;">${t(`document`,{size:22})}</div>
                <div style="flex:1; min-width:0;">
                    ...
                    <p style="margin:3px 0 0; color:rgba(255,255,255,0.85); font-size:0.78rem; font-weight:600; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">
                        ${l(i(`modals.pdfSubtitlePrefix`))} <strong style="color:white;">${l(n)}</strong>
                    </p>
                </div>
            </div>
            ...
            <div style="display:flex; gap:10px; padding:18px 22px 22px;">
                <button type="button" id="cancelPdfBtn" class="flex-1"
                        style="font-weight:700; color:#002d5b; background:rgba(0,45,91,0.06); border:1px solid rgba(0,45,91,0.12); padding:11px 18px; border-radius:12px; cursor:pointer; font-size:0.9rem;">${l(i(`modals.pdfCancelBtn`))}</button>
                <button type="button" id="submitPdfBtn" class="flex-1"
                    ...
                    <span id="pdfBtnLabel">${l(i(`modals.pdfDownloadBtn`))}</span>
                </button>
            </div>
        </div>
    `,cardStyle:`max-width: 560px; width: min(560px, calc(100vw - 24px)); padding: 0; overflow: hidden; background: white;`});function f(e,t,n,r=!0){return`
            <label style="display:flex; align-items:flex-start; gap:8px; cursor:pointer; padding:10px 12px; border-radius:12px; transition: background 0.15s, border-color 0.15s; background:rgba(0,113,227,0.04); border:1px solid rgba(0,113,227,0.10);">
                ...
                <span style="min-width:0; flex:1;">
                    <span style="display:block; font-weight:700; color:#002d5b; font-size:0.86rem; line-height:1.2;">${l(t)}</span>
                    <span style="display:block; color:#4a5568; font-size:0.74rem; line-height:1.35; margin-top:2px;">${l(n)}</span>
                </span>
            </label>
        `}let p=u(r,`#cancelPdfBtn`),m=u(r,`#submitPdfBtn`),h=u(r,`#pdfBtnLabel`);p&&(p.onclick=()=>d()),m&&(m.onclick=async()=>{let t=r.querySelectorAll(`#pdfExportOptions input[type="checkbox"]`),n={};t.forEach(e=>{n[e.name]=e.checked}),n.locale=o(),m.disabled=!0,h&&(h.textContent=i(`modals.pdfStatusBuilding`));try{let t=await s(`/api/trips/${e.id}/pdf`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify(n)});if(!t.ok){let e=``;try{let n=await t.json();n&&typeof n.error==`string`&&(e=n.error)}catch{}c(e||i(`modals.pdfErrorBuild`));return}let r=await t.blob(),a=URL.createObjectURL(r),o=navigator.userAgent||``,l=/iPad|iPhone|iPod/.test(o)||o.includes(`Mac`)&&`ontouchend`in document,u=(e.name||`trip`).replace(/[^A-Za-z0-9 _-]/g,`_`).trim()||`trip`;if(l)window.open(a,`_blank`)||(window.location.href=a),setTimeout(()=>URL.revokeObjectURL(a),6e4);else{let e=document.createElement(`a`);e.href=a,e.download=`${u}.pdf`,document.body.appendChild(e),e.click(),setTimeout(()=>{document.body.removeChild(e),URL.revokeObjectURL(a)},100)}d()}catch{c(i(`modals.pdfErrorNetwork`))}finally{m.disabled=!1,h&&(h.textContent=i(`modals.pdfDownloadBtn`))}})},p=e({importTripFromFile:()=>C,openDownloadChooserModal:()=>x}),m=e=>(e||`trip`).replace(/[^A-Za-z0-9 _-]/g,`_`).trim()||`trip`;function h(e,t){let n=URL.createObjectURL(e),r=navigator.userAgent||``;if(/iPad|iPhone|iPod/.test(r)||r.includes(`Mac`)&&`ontouchend`in document){window.open(n,`_blank`)||(window.location.href=n),setTimeout(()=>URL.revokeObjectURL(n),6e4);return}let i=document.createElement(`a`);i.href=n,i.download=t,document.body.appendChild(i),i.click(),setTimeout(()=>{document.body.removeChild(i),URL.revokeObjectURL(n)},100)}async function g(e){try{let t=await s(`/api/trips/${e.id}/export`);if(!t.ok){let e=``;try{let n=await t.json();n&&typeof n.error==`string`&&(e=n.error)}catch{}return c(e||i(`modals.downloadZipError`)),!1}return h(await t.blob(),`${m(e.name)}.ggtrip.zip`),!0}catch{return c(i(`modals.downloadZipError`)),!1}}var _=`<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="9" y1="15" x2="15" y2="15"/><line x1="9" y1="11" x2="12" y2="11"/></svg>`,v=`<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 8v13H3V8"/><path d="M1 3h22v5H1z"/><line x1="10" y1="12" x2="14" y2="12"/></svg>`,y=`<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round" style="opacity:0.32; flex-shrink:0;"><polyline points="9 18 15 12 9 6"/></svg>`;function b(e,t,n,r,i){return`
        <button type="button" id="${e}" style="display:flex; align-items:center; gap:14px; width:100%; text-align:left; padding:14px 16px; border-radius:16px; border:1px solid rgba(0,0,0,0.08); background:#fff; cursor:pointer; transition:border-color 0.15s, box-shadow 0.15s, transform 0.1s;"
            ...
            ${y}
        </button>`}var x=e=>{if(!e||!e.id){c(i(`modals.pdfErrorNoTrip`));return}let{root:t,close:n}=a({innerHTML:`
        ...
        </div>`,cardStyle:`max-width: 420px; width: min(420px, calc(100vw - 24px)); background:#fff;`}),r=t.querySelector(`#chooserPdfBtn`),o=t.querySelector(`#chooserZipBtn`),s=t.querySelector(`#chooserCancelBtn`);if(s&&(s.onclick=()=>n()),r&&(r.onclick=()=>{n(),f(e)}),o){let t=o.querySelector(`[data-sub]`);o.onclick=async()=>{if(o.disabled=!0,r&&(r.disabled=!0),t&&(t.textContent=i(`modals.downloadZipStatus`)),await g(e)){n();return}o.disabled=!1,r&&(r.disabled=!1),t&&(t.textContent=i(`modals.downloadZipOptionBody`))}}},S=256*1024*1024;async function C(e){if(e.size&&e.size>S)return{ok:!1,error:i(`modals.importTripTooLarge`)};let t=new FormData;t.append(`file`,e);let a;try{a=await s(`/api/trips/import`,{method:`POST`,body:t})}catch{return{ok:!1,error:i(`modals.importTripError`)}}if(!a.ok){let e=``;try{let t=await a.json();t&&typeof t.error==`string`&&(e=t.error)}catch{}return{ok:!1,error:e||i(`modals.importTripError`)}}let o={};try{o=await a.json()}catch{}return o.tripId?(await r(),n.activeTripId=o.tripId,d(`state:changed`),{ok:!0}):{ok:!1,error:i(`modals.importTripError`)}}export{p as n,C as t};
//# sourceMappingURL=tripExport-kFH7y_rz.js.map