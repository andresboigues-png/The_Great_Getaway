import{$t as e,O as t,Xt as n,Yt as r,Zt as i,fn as a,k as o,qt as s,rn as c}from"../app.bundle.js";var l=(e,t)=>`<span style="display:inline-flex;align-items:center;gap:6px;justify-content:center">${s(e)}${n(r(t))}</span>`,u=`home_path_card_collapsed_day_ids`;function d(){try{let e=localStorage.getItem(u);if(!e)return new Set;let t=JSON.parse(e);return new Set(Array.isArray(t)?t:[])}catch{return new Set}}function f(e){try{localStorage.setItem(u,JSON.stringify([...e]))}catch{}}function p(e){let t=d();return t.has(e)?(t.delete(e),f(t),!1):(t.add(e),f(t),!0)}function m(e){return d().has(e)}function h(e,t){let n=d(),r=n.has(e);t&&!r?(n.add(e),f(n)):!t&&r&&(n.delete(e),f(n))}function g(e,a,o){let{isAnchor:l,isSelected:u}=a,d=l?`<div style="background: var(--gradient-anchor-deep); color: white; width: 48px; height: 48px; border-radius: 50%; border: 3px solid white; display: flex; align-items: center; justify-content: center; flex-shrink:0; box-shadow: 0 8px 18px rgba(212,160,23,0.28);">
               ...
               <span style="font-size: 0.6rem; font-weight: 800; text-transform: uppercase; opacity: 0.85; letter-spacing: 0.05em; line-height:1;">${n(t(`pathTab.dayBadgeLabel`))}</span>
               <span style="font-size: 1.25rem; font-weight: 800; line-height: 1.05;">${e.dayNumber}</span>
           </div>`,f=l?t(`pathTab.hubTitle`):n(e.name||t(`tripMedia.dayBucketDay`,{n:e.dayNumber})),p=[];if(l){p.push(o&&o.country?n(c(o.country)):n(t(`pathTab.hubSubtitleFallback`)));let r=(o.documents||[]).filter(t=>t.dayId===e.id),i=(o.photos||[]).filter(t=>t.dayId===e.id),a=r.length+(e.tickets||[]).length,l=i.length+(e.photos||[]).length;l&&p.push(`<span style="display:inline-flex; align-items:center; gap:4px; background:rgba(52,199,89,0.12); color:#1a6b3c; padding:2px 8px; border-radius:999px; font-size:0.7rem; font-weight:800;">${s(`photo`,{size:13})}${l}</span>`),a&&p.push(`<span style="display:inline-flex; align-items:center; gap:4px; background:rgba(88,86,214,0.12); color:#5856d6; padding:2px 8px; border-radius:999px; font-size:0.7rem; font-weight:800;">${s(`document`,{size:13})}${a}</span>`)}else p.push(`<button type="button" class="day-card__date-btn" data-day-id="${n(e.id)}" aria-label="${n(t(`pathTab.setDatePlaceholder`))}" style="display:inline-flex; align-items:center; gap:5px; background:none; border:none; padding:0; margin:0; font:inherit; color:inherit; cursor:pointer;">${s(`calendar`,{size:14})}${i(e.date)||t(`pathTab.setDatePlaceholder`)}</button>`),e.lat?p.push(`<span style="color: #005bb8;">${n(t(`pathTab.locationSet`))}</span>`):p.push(`<span class="day-card__pin-hint" style="display:inline-flex; align-items:center; gap:4px;">${s(`pinned`,{size:13})}${n(r(t(`pathTab.pinThisDay`)))}</span>`),e.date&&p.push(`<span class="day-card__weather" data-weather-date="${n(e.date)}"></span>`);let m=u&&e.notes&&!l?`
        <div style="margin-top: 12px; padding: 12px 14px; background: rgba(0,113,227,0.04); border-radius: 14px; border-left: 3px solid var(--accent-blue);">
            <div style="font-size: 0.7rem; font-weight: 800; text-transform: uppercase; color: #005bb8; margin-bottom: 4px; letter-spacing: 0.05em;">${n(t(`pathTab.journalPreviewLabel`))}</div>
    ...
    `:``,h=`
        <button type="button" class="path-card-collapse-btn" data-day-id="${n(e.id)}"
            aria-label="${n(t(`pathTab.toggleOptionsAria`,{title:f}))}" title="${n(t(`pathTab.toggleOptionsTitle`))}">
    ...
    `;return`
        ...
        ${m}
    `}function _(e,r,i,a){if(!e||!i)return``;let{isAnchor:o}=r,s=[];if(o?s.push(`<button class="path-primary-btn path-primary-btn--anchor path-checklist-btn" data-day-id="${n(e.id)}">${l(`checklist`,t(`pathTab.btnChecklist`))}</button>`):s.push(`<button class="path-primary-btn day-detail-btn" data-day-id="${n(e.id)}">${l(`plan`,t(`pathTab.btnOpenFullPlan`))}</button>`),a===e.id)s.push(`<button class="day-action-btn day-action-btn--success day-pin-save-btn" data-day-id="${n(e.id)}">${n(t(`pathTab.btnSavePin`))}</button>`),s.push(`<button class="day-action-btn day-action-btn--danger-fill day-pin-delete-btn" data-day-id="${n(e.id)}">${n(t(`pathTab.btnCancelPinEdit`))}</button>`);else{let r=e.lat?t(o?`pathTab.btnEditAnchorPin`:`pathTab.btnEditPin`):t(o?`pathTab.btnSetAnchorPin`:`pathTab.btnAddPin`);s.push(`<button class="day-action-btn day-action-btn--neutral day-pin-toggle-btn" data-day-id="${n(e.id)}">${l(`pin`,r)}</button>`)}return o?(s.push(`<button class="day-action-btn day-action-btn--neutral path-documents-btn" data-day-id="${n(e.id)}">${l(`document`,t(`pathTab.btnDocuments`))}</button>`),s.push(`<button class="day-action-btn day-action-btn--neutral path-photos-btn" data-day-id="${n(e.id)}">${l(`photo`,t(`pathTab.btnPhotos`))}</button>`)):(s.push(`<button class="day-action-btn day-action-btn--neutral day-journaling-btn" data-day-id="${n(e.id)}">${l(`journal`,t(`pathTab.btnJournaling`))}</button>`),s.push(`<button class="day-action-btn day-action-btn--danger day-delete-btn" data-day-id="${n(e.id)}">${l(`trash`,t(`pathTab.btnDeleteDay`))}</button>`)),`<div class="path-options-stack">${s.join(``)}</div>`}function v(r){let{activeTrip:a,tripDays:s,tripIsEditable:c,editingDayId:l}=r,u=[...s].sort((e,t)=>e.dayNumber-t.dayNumber),d=u.find(e=>e.dayNumber===0)||null,f=u.filter(e=>e.dayNumber>0),p=T(a,u),h=u.find(e=>e.id===p)||null;if(u.length===0)return`<div class="card glass" style="padding:28px; border-radius:18px; text-align:center; color:var(--text-secondary);">${n(t(`pathTab.emptyState`))}</div>`;let v=f.length,y=h?.dayNumber===0?o(`path.summaryHub`,v,{count:v}):h?t(`path.summaryDay`,{day:h.dayNumber,total:v}):o(`path.summaryNone`,v,{count:v}),b=e(),x=u.map(e=>{let r=e.id===p,a=e.dayNumber===0,o=!a&&e.date===b,s=`path-chip${a?` path-chip--anchor`:``}${o?` path-chip--today`:``}${r?` is-selected`:``}`,c=a?t(`pathTab.chipHubTooltip`):`${o?t(`pathTab.chipTodayPrefix`)+` · `:``}${t(`tripMedia.dayBucketDay`,{n:e.dayNumber})}${e.name?` — `+e.name:``}${e.date?` · `+(i(e.date)||e.date):``}`,l=a?`<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" stroke-width="1.2" stroke-linejoin="round" aria-hidden="true"><polygon points="12 2 15 8.5 22 9.3 17 14.3 18.2 21.3 12 18 5.8 21.3 7 14.3 2 9.3 9 8.5"/></svg>`:String(e.dayNumber);return`<button type="button" class="${s}" data-path-chip-day-id="${n(e.id)}" title="${n(c)}" aria-label="${n(c)}" aria-pressed="${r}">${l}</button>`}).join(``),S=c?`<button type="button" class="path-chip path-chip--add" id="pathAddDayChip" title="${n(t(`pathTab.addNewDay`))}" aria-label="${n(t(`pathTab.addNewDay`))}">+</button>`:``,C=u.findIndex(e=>e.id===p),w=C<=0,E=C<0||C>=u.length-1,D=[];if(d){let e=h?.id===d.id,t=m(d.id);D.push(`
