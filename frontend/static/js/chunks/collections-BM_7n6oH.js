import{C as e,Ct as t,Et as n,Ft as r,H as i,Ht as a,K as o,Qt as s,Rt as c,Tt as l,Z as u,Zt as d,at as f,et as p,j as m,o as h,ot as g,qt as _,w as v,wt as y,x as b,xt as x,z as S}from"../app.bundle.js";var C=async(e,t)=>{let n=d.archivedTrips.find(t=>t.id===e)||d.trips.find(t=>t.id===e);if(n&&(n.isPublic=t!==`private`,n.publicShowExpenses=t===`public-full`,s(`state:changed`),d.user))try{await f(n)}catch{}},w=e=>{let t=d.archivedTrips.find(t=>t.id===e);t&&r({title:b(`errors.restoreTripTitle`),message:b(`errors.restoreTripBody`),confirmText:b(`errors.restoreTripConfirmBtn`),onConfirm:()=>{t.isArchived=!1,t.expenses&&(d.expenses=[...d.expenses,...t.expenses],delete t.expenses),t.tripDays&&(d.tripDays=[...d.tripDays,...t.tripDays],delete t.tripDays);let n=t.settlements;n&&n.length>0&&(d.settlements=[...d.settlements||[],...n],delete t.settlements),d.trips.push(t),d.archivedTrips=d.archivedTrips.filter(t=>t.id!==e),d.activeTripId=e,s(`state:changed`),u(e),g(`home`)}})},T=e=>{r({title:b(`errors.deleteTripTitle`),message:b(`errors.deleteTripBody`),confirmText:b(`errors.deleteTripConfirmBtn`),onConfirm:async()=>{d.archivedTrips=d.archivedTrips.filter(t=>t.id!==e),s(`state:changed`);let t=m(e);t&&t.catch(e=>console.error(`Delete archived trip failed:`,e)),g(`collections`)}})};function E(e){let u=typeof e==`string`?d.archivedTrips.find(t=>t.id===e)||d.trips.find(t=>t.id===e):e,f=document.createElement(`div`);if(!u)return f.innerHTML=`<p style="padding: 40px; text-align: center;">${c(b(`archivedDetail.notFound`))}</p>`,f;let m=(u.expenses||[]).filter(e=>!e.isSettlement),T=m.reduce((e,t)=>e+(t.euroValue||0),0),E=u.tripDays||[],D=E.filter(e=>(e.dayNumber||0)>0).length,O=Array.isArray(u.photos)?u.photos:[],k=Array.isArray(u.documents)?u.documents:[],A=E.reduce((e,t)=>e+(t.photos||[]).length,0)+O.length,j=E.reduce((e,t)=>e+(t.tickets||[]).length,0)+k.length,M=null;if(u.coverUrl&&(M=u.coverUrl),!M&&O.length>0&&(M=O[0].src),!M){for(let e of E)if(e.photos&&e.photos.length>0){M=e.photos[0];break}}let N=M?`background: linear-gradient(135deg, rgba(0,45,91,0.55), rgba(88,86,214,0.45)), url(${c(M)}) center/cover no-repeat;`:`background: linear-gradient(135deg, #007aff 0%, #5856d6 60%, #34c759 130%);`,P=`#ffffff`,F=`rgba(255,255,255,0.85)`,I=`rgba(255,255,255,0.16)`,L=`1px solid rgba(255,255,255,0.25)`,R=(e,t,n)=>`
    ...
    `;f.innerHTML=`
        ...
        ${(()=>{let e=e=>{if(!e)return null;let t=E.find(t=>t.id===e);return t?Number(t.dayNumber)===0?b(`archivedDetail.dayBadgeHub`):b(`tripMedia.dayBucketDay`,{n:t.dayNumber}):null},t=e=>{if(!e)return!1;let t=E.find(t=>t.id===e);return!!t&&Number(t.dayNumber)===0},n=n=>{if(t(n))return`<span style="background:rgba(52,199,89,0.12); color:#1a6b3c; padding:2px 10px; border-radius:999px; font-size:0.65rem; font-weight:800; text-transform:uppercase; letter-spacing:0.06em;">${c(b(`archivedDetail.dayBadgeHub`))}</span>`;let r=e(n);return r?`<span style="background:rgba(0,113,227,0.08); color:var(--accent-blue); padding:2px 10px; border-radius:999px; font-size:0.65rem; font-weight:800; text-transform:uppercase; letter-spacing:0.06em;">${c(r)}</span>`:`<span style="background:rgba(0,0,0,0.05); color:rgba(0,0,0,0.45); padding:2px 10px; border-radius:999px; font-size:0.65rem; font-weight:800; text-transform:uppercase; letter-spacing:0.06em;">${c(b(`archivedDetail.dayBucketUnsorted`))}</span>`},r=[],i=b(`tripMedia.docsFallbackName`);k.forEach(e=>r.push({name:e.name||i,url:e.url||``,dayId:e.dayId||null,source:`trip`,_key:e.id||`${e.name}-${e.url}`})),E.forEach(e=>{(e.tickets||[]).forEach((t,n)=>r.push({name:t.name||i,url:t.url||``,dayId:e.id,source:`day`,_key:`${e.id}#${n}`}))});let a=e=>{if(!e)return-1;let t=E.find(t=>t.id===e);return t?t.dayNumber:999};r.sort((e,t)=>a(e.dayId)-a(t.dayId));let o=[];O.forEach(e=>o.push({src:e.src||``,dayId:e.dayId||null,source:`trip`,_key:e.id||e.src})),E.forEach(e=>{(e.photos||[]).forEach((t,n)=>o.push({src:t,dayId:e.id,source:`day`,_key:`${e.id}#${n}`}))}),o.sort((e,t)=>a(e.dayId)-a(t.dayId));let s=e=>/^data:image\//i.test(e||``)||/\.(jpe?g|png|gif|webp|avif|heic|heif|bmp|tiff?|svg)(\?.*)?$/i.test(e||``);return(r.length===0?``:`
            ...
            `)})()}
    `,f.querySelector(`#backToCollectionsBtn`)?.addEventListener(`click`,()=>g(`collections`));let z=f.querySelector(`#shareTripBtn`);return z&&S(u.id).then(e=>{e?.shared&&(z.dataset.shared=`1`,z.dataset.postId=String(e.post_id),y(z,!0))}),f.addEventListener(`click`,async e=>{let f=e.target,m=f?.closest(`.restore-trip-btn`);if(m?.dataset.tripId){w(m.dataset.tripId);return}let _=f?.closest(`#cloneTripBtn`);if(_?.dataset.tripId){_.setAttribute(`disabled`,`true`);let e=_.innerHTML;_.innerHTML=c(b(`archivedDetail.cloneStatusCloning`));try{let t=await v(_.dataset.tripId);if(!t?.ok||!t.body?.tripId){a(b(`archivedDetail.cloneError`)),_.removeAttribute(`disabled`),_.innerHTML=e;return}let n=t.body.tripId;d.activeTripId=n,await i(),d.activeTripId=n,s(`state:changed`),a(b(`archivedDetail.cloneSuccess`)),g(`home`)}catch(t){console.error(`Clone failed:`,t),a(b(`archivedDetail.cloneError`)),_.removeAttribute(`disabled`),_.innerHTML=e}return}let S=f?.closest(`#shareTripBtn`);if(S){h({trip:u,onShareToFeed:()=>{if(S.dataset.shared===`1`){let e=Number(S.dataset.postId||0);if(!e)return;r({title:b(`archivedDetail.unshareConfirmTitle`),message:b(`archivedDetail.unshareConfirmBody`),confirmText:b(`archivedDetail.unshareConfirmBtn`),onConfirm:async()=>{let t=await p(e);if(!t||!t.ok){a(b(`archivedDetail.unshareError`));return}S.dataset.shared=`0`,S.dataset.postId=``,y(S,!1),a(b(`archivedDetail.unshareSuccess`))}});return}t(u,async e=>{let t=await o(u.id,e);if(!t||!t.ok){let e=t?.status??`no-response`,n=t?.body?.error||``;a(`Share failed — HTTP ${e}`+(n?` · ${n}`:``)),console.error(`[collections.share] failed`,{tripId:u.id,status:e,body:t?.body});return}let n=Number(t.body?.post_id)||0;n&&(S.dataset.shared=`1`,S.dataset.postId=String(n),y(S,!0)),t.body?.status===`already_shared`?a(b(e?`archivedDetail.shareUpdated`:`archivedDetail.shareAlready`)):a(b(`archivedDetail.shareSuccess`))})}});return}let C=f?.closest(`a[href]`);if(C&&l(C.href)){let t=e;if(!t.metaKey&&!t.ctrlKey&&!t.shiftKey&&t.button!==1){t.preventDefault();let e=C.querySelector(`span`)?.textContent?.trim()||`Document`;n(C.href,e);return}}let T=f?.closest(`.archived-day-block`);if(T?.dataset.dayId){let e=(u.tripDays||[]).find(e=>e.id===T.dataset.dayId);e&&x(e);return}}),f.addEventListener(`change`,e=>{let t=e.target?.closest(`.trip-privacy-select`);t?.dataset.tripId&&C(t.dataset.tripId,t.value)}),f}var D=async t=>{let n=document.getElementById(`app-container`);if(!n)return;let r=d.archivedTrips.find(e=>e.id===t)||d.trips.find(e=>e.id===t);if(r){n.innerHTML=``,n.appendChild(E(r));return}n.innerHTML=`<div style="padding:60px 20px; text-align:center; color:var(--text-secondary); font-size:0.95rem;">${b(`collections.loadingTrip`)}</div>`;try{let r=await e(`/api/public-trip/${encodeURIComponent(t)}`);if(!r.ok){n.innerHTML=`<div style="padding:60px 20px; text-align:center; color:var(--text-secondary);">${b(`collections.tripUnavailable`)}</div>`;return}let i=await r.json();if(!i?.trip){n.innerHTML=`<div style="padding:60px 20px; text-align:center; color:var(--text-secondary);">${b(`collections.tripNotFound`)}</div>`;return}n.innerHTML=``,n.appendChild(E(i.trip))}catch(e){console.error(`viewArchivedDetails fetch failed:`,e),n.innerHTML=`<div style="padding:60px 20px; text-align:center; color:var(--text-secondary);">${b(`collections.loadFailed`)}</div>`}};export{C as i,T as n,w as r,D as t};
//# sourceMappingURL=collections-BM_7n6oH.js.map