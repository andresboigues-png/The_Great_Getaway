import{r as e,t}from"./vendor-react-CAxw18f3.js";import{Bt as n,Ct as r,Dt as i,Et as a,Mt as o,Tt as s,Wt as c,Yt as l,bt as u,f as d,jt as f,kt as p,tt as m,v as h,xt as g,y as _,zt as v}from"../app.bundle.js";import{t as y}from"./store-1WQ1kZp7.js";var b=e();function x(e){if(!e)return{balances:{},roster:[],expenses:[]};let t=(v.expenses||[]).filter(t=>t.tripId===e.id),n=c(e),r=n.length>0?n:Array.from(new Set(t.flatMap(e=>[e.who,...Object.keys(e.splits||{})]).filter(Boolean))),i={};r.forEach(e=>i[e]=0);for(let e of t){let t=e.euroValue||e.value||0;if(i[e.who]!==void 0&&(i[e.who]+=t),e.splits&&Object.keys(e.splits).length>0)for(let[n,r]of Object.entries(e.splits))i[n]!==void 0&&(i[n]-=t*(Number(r)/100));else{let e=t/Math.max(r.length,1);r.forEach(t=>{i[t]!==void 0&&(i[t]-=e)})}}return{balances:i,roster:r,expenses:t}}function S(e){let t=[],n=[];for(let[r,i]of Object.entries(e))i>.01?t.push({person:r,amount:i}):i<-.01&&n.push({person:r,amount:Math.abs(i)});t.sort((e,t)=>t.amount-e.amount),n.sort((e,t)=>t.amount-e.amount);let r=[],i=0,a=0;for(;i<n.length&&a<t.length;){let e=n[i],o=t[a],s=Math.min(e.amount,o.amount);r.push({from:e.person,to:o.person,amount:s}),e.amount-=s,o.amount-=s,e.amount<.01&&i++,o.amount<.01&&a++}return r}function C(){let e={};for(let t of[...v.trips,...v.archivedTrips||[]])for(let n of c(t))n in e||(e[n]=0);let t=(v.archivedTrips||[]).flatMap(e=>e.expenses||[]),n=[...v.expenses,...t],r={};for(let e of[...v.trips,...v.archivedTrips||[]])r[e.id]=c(e);for(let t of n){let n=t.euroValue||t.value||0;if(e[t.who]!==void 0&&(e[t.who]+=n),t.splits&&Object.keys(t.splits).length>0)for(let[r,i]of Object.entries(t.splits))e[r]!==void 0&&(e[r]-=n*(Number(i)/100));else{let i=r[t.tripId]||[],a=i.length>0?i:Array.from(new Set([t.who,...Object.keys(t.splits||{})].filter(Boolean))),o=n/Math.max(a.length,1);a.forEach(t=>{e[t]!==void 0&&(e[t]-=o)})}}return e}function w(e){if(!e)return[];let t=(v.expenses||[]).filter(t=>t.tripId===e.id),n=c(e),r={};n.forEach(e=>r[e]={paid:0,share:0});for(let e of t){let t=e.euroValue||e.value||0;if(r[e.who]&&(r[e.who].paid+=t),e.splits&&Object.keys(e.splits).length>0)for(let[n,i]of Object.entries(e.splits))r[n]&&(r[n].share+=t*(Number(i)/100));else{let e=t/Math.max(n.length,1);n.forEach(t=>{r[t]&&(r[t].share+=e)})}}return Object.entries(r).map(([e,t])=>({name:e,paid:t.paid,share:t.share,net:t.paid-t.share}))}function T(e,t,n,r){let i=n===`global`?``:E(r),a=`
    ...
    `:`
        ...
        `}function E(e){if(v.trips.length===0)return``;let t=v.trips.find(t=>t.id===e),n=t?(v.expenses||[]).filter(e=>e.tripId===t.id&&e.isSettlement).reduce((e,t)=>e+(t.euroValue||0),0):0,i=v.trips.map(t=>{let n=(v.expenses||[]).filter(e=>e.tripId===t.id&&e.isSettlement).reduce((e,t)=>e+(t.euroValue||0),0),i=n>0?` — ${f(n,`EUR`)} ${h(`settlement.settledSuffix`)}`:``;return`<option value="${r(t.id)}"${t.id===e?` selected`:``}>${r(t.name)}${i}</option>`}).join(``);return`
    ...
    `}function A(){let e=C(),t=Object.entries(e).sort((e,t)=>t[1]-e[1]),n=Math.max(...Object.values(e).map(Math.abs),1),i=t.some(([,e])=>Math.abs(e)>.01);if(t.length===0)return`
        ...
        `:``}
    `}function j(e,t,r,a,o){if(t===r){i(h(`settlement.toastSenderEqualsReceiver`));return}if(!Number.isFinite(a)||a<=0){i(h(`settlement.toastAmountInvalid`));return}let c=p(a,o,`EUR`),u={id:s(),tripId:e,label:`Settlement: ${t} → ${r}`,value:a,euroValue:c,currency:o,who:t,categoryId:v.categories[0]?.id??``,country:h(`settlement.expenseCountry`),date:new Date().toISOString().split(`T`)[0]??``,splits:{[r]:100},isSettlement:!0};v.expenses.push(u),n(l.STATE_CHANGED),i(`Recorded ${f(c,`EUR`)} ${t} → ${r}`)}function M(e){u({title:h(`settlement.toastUnsettleConfirmTitle`),message:h(`settlement.toastUnsettleConfirmMessage`),confirmText:h(`settlement.toastUnsettleConfirmBtn`),onConfirm:()=>{v.expenses=v.expenses.filter(t=>t.id!==e),n(l.STATE_CHANGED)}})}function N(e){let t=c(v.trips.find(t=>t.id===e)).map(e=>`<option value="${r(e)}">${r(e)}</option>`).join(``),n=o(),{root:s,close:l}=g({variant:`glass-light`,cardStyle:`width: 440px; max-width: calc(100vw - 32px);`,innerHTML:`
        ...
        `});a(s,`#cancelManualSettleBtn`).onclick=()=>l(),a(s,`#manualSettleForm`).onsubmit=t=>{t.preventDefault();let r=a(s,`#manualSettleFrom`).value,o=a(s,`#manualSettleTo`).value,c=parseFloat(a(s,`#manualSettleAmount`).value);if(r===o){i(h(`settlement.toastSenderEqualsReceiver`));return}j(e,r,o,c,n),l()}}function P(e){let t=v.expenses.find(t=>t.id===e);if(!t)return;let s=c(v.trips.find(e=>e.id===t.tripId)),u=s.map(e=>`<option value="${r(e)}" ${t.who===e?`selected`:``}>${r(e)}</option>`).join(``),d=Object.keys(t.splits||{})[0],f=s.map(e=>`<option value="${r(e)}" ${d===e?`selected`:``}>${r(e)}</option>`).join(``),m=o(),{root:_,close:y}=g({variant:`glass-light`,cardStyle:`width: 440px; max-width: calc(100vw - 32px);`,innerHTML:`
