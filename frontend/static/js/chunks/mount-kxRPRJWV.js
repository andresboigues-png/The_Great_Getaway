import{r as e,t}from"./vendor-react-CAxw18f3.js";import{Ct as n,Dt as r,Et as i,Jt as a,Mt as o,Rt as s,Tt as c,Ut as l,bt as u,f as d,jt as f,kt as p,tt as m,v as h,xt as g,y as _,zt as v}from"../app.bundle.js";import{t as y}from"./store-0p8UCYvu.js";var b=e();function x(e){if(!e)return{balances:{},roster:[],expenses:[]};let t=(s.expenses||[]).filter(t=>t.tripId===e.id),n=l(e),r=n.length>0?n:Array.from(new Set(t.flatMap(e=>[e.who,...Object.keys(e.splits||{})]).filter(Boolean))),i={};r.forEach(e=>i[e]=0);for(let e of t){let t=e.euroValue||e.value||0;if(i[e.who]!==void 0&&(i[e.who]+=t),e.splits&&Object.keys(e.splits).length>0)for(let[n,r]of Object.entries(e.splits))i[n]!==void 0&&(i[n]-=t*(Number(r)/100));else{let e=t/Math.max(r.length,1);r.forEach(t=>{i[t]!==void 0&&(i[t]-=e)})}}return{balances:i,roster:r,expenses:t}}function S(e){let t=[],n=[];for(let[r,i]of Object.entries(e))i>.01?t.push({person:r,amount:i}):i<-.01&&n.push({person:r,amount:Math.abs(i)});t.sort((e,t)=>t.amount-e.amount),n.sort((e,t)=>t.amount-e.amount);let r=[],i=0,a=0;for(;i<n.length&&a<t.length;){let e=n[i],o=t[a],s=Math.min(e.amount,o.amount);r.push({from:e.person,to:o.person,amount:s}),e.amount-=s,o.amount-=s,e.amount<.01&&i++,o.amount<.01&&a++}return r}function C(){let e={};for(let t of[...s.trips,...s.archivedTrips||[]])for(let n of l(t))n in e||(e[n]=0);let t=(s.archivedTrips||[]).flatMap(e=>e.expenses||[]),n=[...s.expenses,...t],r={};for(let e of[...s.trips,...s.archivedTrips||[]])r[e.id]=l(e);for(let t of n){let n=t.euroValue||t.value||0;if(e[t.who]!==void 0&&(e[t.who]+=n),t.splits&&Object.keys(t.splits).length>0)for(let[r,i]of Object.entries(t.splits))e[r]!==void 0&&(e[r]-=n*(Number(i)/100));else{let i=r[t.tripId]||[],a=i.length>0?i:Array.from(new Set([t.who,...Object.keys(t.splits||{})].filter(Boolean))),o=n/Math.max(a.length,1);a.forEach(t=>{e[t]!==void 0&&(e[t]-=o)})}}return e}function w(e){if(!e)return[];let t=(s.expenses||[]).filter(t=>t.tripId===e.id),n=l(e),r={};n.forEach(e=>r[e]={paid:0,share:0});for(let e of t){let t=e.euroValue||e.value||0;if(r[e.who]&&(r[e.who].paid+=t),e.splits&&Object.keys(e.splits).length>0)for(let[n,i]of Object.entries(e.splits))r[n]&&(r[n].share+=t*(Number(i)/100));else{let e=t/Math.max(n.length,1);n.forEach(t=>{r[t]&&(r[t].share+=e)})}}return Object.entries(r).map(([e,t])=>({name:e,paid:t.paid,share:t.share,net:t.paid-t.share}))}function T(e,t,n,r){let i=E(r),a=`
    ...
    `:`
        ...
        `}function E(e){if(s.trips.length===0)return``;let t=s.trips.find(t=>t.id===e),r=t?(s.expenses||[]).filter(e=>e.tripId===t.id&&e.isSettlement).reduce((e,t)=>e+(t.euroValue||0),0):0,i=s.trips.map(t=>{let r=(s.expenses||[]).filter(e=>e.tripId===t.id&&e.isSettlement).reduce((e,t)=>e+(t.euroValue||0),0),i=r>0?` — ${f(r,`EUR`)} ${h(`settlement.settledSuffix`)}`:``;return`<option value="${n(t.id)}"${t.id===e?` selected`:``}>${n(t.name)}${i}</option>`}).join(``);return`
        <div class="settlement-trip-picker" style="margin-top: 18px; margin-bottom: 16px; display:flex; align-items:center; gap:12px; flex-wrap:wrap;">
            ...
            ${t&&r>0?`
                <span style="display:inline-flex; align-items:center; padding:6px 12px; border-radius:999px; background:rgba(0,113,227,0.08); color:#005bb8; font-size:0.78rem; font-weight:800; flex-shrink:0;">
                    ${f(r,`EUR`)} ${h(`settlement.settledSuffix`)}
                </span>
            `:``}
        </div>
    ...
    `:``,d=Object.entries(r).map(([e,t])=>{let r=t>.01,i=t<-.01;return`
        ...
        `:``}
    `}function k(e,t){let r=(s.expenses||[]).filter(t=>t.tripId===e.id&&t.isSettlement).sort((e,t)=>new Date(t.date).getTime()-new Date(e.date).getTime());if(r.length===0)return`
        ...
        `;let i={};for(let e of r){let t=e.date||`undated`;i[t]||(i[t]=[]),i[t].push(e)}let a=new Date().toISOString().slice(0,10),o=new Date;o.setDate(o.getDate()-1);let c=o.toISOString().slice(0,10),l=e=>{if(e===`undated`)return h(`settlement.historyDateNoDate`);if(e===a)return h(`settlement.historyDateToday`);if(e===c)return h(`settlement.historyDateYesterday`);let t=new Date(e);return isNaN(t.getTime())?e:t.toLocaleDateString(void 0,{weekday:`short`,month:`short`,day:`numeric`,year:`numeric`})},u=Object.keys(i).sort((e,t)=>e===`undated`?1:t===`undated`?-1:t.localeCompare(e));return`
        <div class="card glass" style="padding: 22px 24px; border-radius: 28px;">
            ...
            <div style="display:flex; flex-direction:column; gap:18px;">
                ${u.map(r=>{let a=i[r],o=a.reduce((e,t)=>e+(t.euroValue||0),0);return`
                        <div>
                            <div style="display:flex; align-items:baseline; justify-content:space-between; margin-bottom:8px; padding: 0 4px;">
                                ...
                                <span style="font-size:0.72rem; font-weight:700; color:var(--text-secondary);">${_(`settlement.historyDayTotalPlural`,a.length,{amount:f(o,`EUR`)})}</span>
                            </div>
                            <div style="display:flex; flex-direction:column; gap:8px;">
    ...
    `}function A(){let e=C(),t=Object.entries(e).sort((e,t)=>t[1]-e[1]),r=Math.max(...Object.values(e).map(Math.abs),1),i=t.some(([,e])=>Math.abs(e)>.01);if(t.length===0)return`
        ...
        `:``}
    `}function j(e,t,n,i,o){if(t===n){r(h(`settlement.toastSenderEqualsReceiver`));return}if(!Number.isFinite(i)||i<=0){r(h(`settlement.toastAmountInvalid`));return}let l=p(i,o,`EUR`),u={id:c(),tripId:e,label:`Settlement: ${t} → ${n}`,value:i,euroValue:l,currency:o,who:t,categoryId:s.categories[0]?.id??``,country:h(`settlement.expenseCountry`),date:new Date().toISOString().split(`T`)[0]??``,splits:{[n]:100},isSettlement:!0};s.expenses.push(u),v(a.STATE_CHANGED),r(`Recorded ${f(l,`EUR`)} ${t} → ${n}`)}function M(e){u({title:h(`settlement.toastUnsettleConfirmTitle`),message:h(`settlement.toastUnsettleConfirmMessage`),confirmText:h(`settlement.toastUnsettleConfirmBtn`),onConfirm:()=>{s.expenses=s.expenses.filter(t=>t.id!==e),v(a.STATE_CHANGED)}})}function N(e){let t=l(s.trips.find(t=>t.id===e)).map(e=>`<option value="${n(e)}">${n(e)}</option>`).join(``),a=o(),{root:c,close:u}=g({variant:`glass-light`,cardStyle:`width: 440px; max-width: calc(100vw - 32px);`,innerHTML:`
            ...
            <form id="manualSettleForm" style="display:flex; flex-direction:column; gap: var(--space-3); margin-top: var(--space-4);">
                ...
                <label class="form-label" style="margin-top:6px;">Amount (${n(a)})</label>
        ...
        `});i(c,`#cancelManualSettleBtn`).onclick=()=>u(),i(c,`#manualSettleForm`).onsubmit=t=>{t.preventDefault();let n=i(c,`#manualSettleFrom`).value,o=i(c,`#manualSettleTo`).value,s=parseFloat(i(c,`#manualSettleAmount`).value);if(n===o){r(h(`settlement.toastSenderEqualsReceiver`));return}j(e,n,o,s,a),u()}}function P(e){let t=s.expenses.find(t=>t.id===e);if(!t)return;let c=l(s.trips.find(e=>e.id===t.tripId)),u=c.map(e=>`<option value="${n(e)}" ${t.who===e?`selected`:``}>${n(e)}</option>`).join(``),d=Object.keys(t.splits||{})[0],f=c.map(e=>`<option value="${n(e)}" ${d===e?`selected`:``}>${n(e)}</option>`).join(``),m=o(),{root:_,close:y}=g({variant:`glass-light`,cardStyle:`width: 440px; max-width: calc(100vw - 32px);`,innerHTML:`
            ...
            <form id="editSettlementForm" style="display:flex; flex-direction:column; gap: var(--space-3); margin-top: var(--space-4);">
                ...
                <label class="form-label" style="margin-top:6px;">Amount (${n(m)})</label>
