import{Bt as e,C as t,Ht as n,Kt as r,S as i,Ut as a,Wt as o,Xt as s,an as c}from"../app.bundle.js";var l=(t,r)=>`<span style="display:inline-flex;align-items:center;gap:6px;justify-content:center">${e(t)}${a(n(r))}</span>`,u=`home_path_card_collapsed_day_ids`;function d(){try{let e=localStorage.getItem(u);if(!e)return new Set;let t=JSON.parse(e);return new Set(Array.isArray(t)?t:[])}catch{return new Set}}function f(e){try{localStorage.setItem(u,JSON.stringify([...e]))}catch{}}function p(e){let t=d();return t.has(e)?(t.delete(e),f(t),!1):(t.add(e),f(t),!0)}function m(e){return d().has(e)}function h(e,t){let n=d(),r=n.has(e);t&&!r?(n.add(e),f(n)):!t&&r&&(n.delete(e),f(n))}function g(t,r,c){let{isAnchor:l,isSelected:u}=r,d=l?`<div style="background: var(--gradient-anchor-deep); color: white; width: 48px; height: 48px; border-radius: 50%; border: 3px solid white; display: flex; align-items: center; justify-content: center; flex-shrink:0; box-shadow: 0 8px 18px rgba(212,160,23,0.28);">
           ...
           </div>`,f=l?i(`pathTab.hubTitle`):a(t.name||i(`tripMedia.dayBucketDay`,{n:t.dayNumber})),p=[];if(l){p.push(c&&c.country?a(s(c.country)):a(i(`pathTab.hubSubtitleFallback`)));let n=(c.documents||[]).filter(e=>e.dayId===t.id),r=(c.photos||[]).filter(e=>e.dayId===t.id),o=n.length+(t.tickets||[]).length,l=r.length+(t.photos||[]).length;l&&p.push(`<span style="display:inline-flex; align-items:center; gap:4px; background:rgba(52,199,89,0.12); color:#1a6b3c; padding:2px 8px; border-radius:999px; font-size:0.7rem; font-weight:800;">${e(`photo`,{size:13})}${l}</span>`),o&&p.push(`<span style="display:inline-flex; align-items:center; gap:4px; background:rgba(88,86,214,0.12); color:#5856d6; padding:2px 8px; border-radius:999px; font-size:0.7rem; font-weight:800;">${e(`document`,{size:13})}${o}</span>`)}else p.push(`<button type="button" class="day-card__date-btn" data-day-id="${a(t.id)}" aria-label="${a(i(`pathTab.setDatePlaceholder`))}" style="display:inline-flex; align-items:center; gap:5px; background:none; border:none; padding:0; margin:0; font:inherit; color:inherit; cursor:pointer;">${e(`calendar`,{size:14})}${o(t.date)||i(`pathTab.setDatePlaceholder`)}</button>`),t.lat?p.push(`<span style="color: #005bb8;">${a(i(`pathTab.locationSet`))}</span>`):p.push(`<span class="day-card__pin-hint" style="display:inline-flex; align-items:center; gap:4px;">${e(`pinned`,{size:13})}${a(n(i(`pathTab.pinThisDay`)))}</span>`),t.date&&p.push(`<span class="day-card__weather" data-weather-date="${a(t.date)}"></span>`);let m=u&&t.notes&&!l?`
        <div style="margin-top: 12px; padding: 12px 14px; background: rgba(0,113,227,0.04); border-radius: 14px; border-left: 3px solid var(--accent-blue);">
            ...
            <p style="margin: 0; font-size: 0.9rem; line-height: 1.45; color: #002d5b; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden;">${a(t.notes)}</p>
        </div>
    `:``,h=`
        <button type="button" class="path-card-collapse-btn" data-day-id="${a(t.id)}"
    ...
    `;return`
        ...
        ${m}
    `}function _(e,t,n,r){if(!e||!n)return``;let{isAnchor:o}=t,s=[];if(o?s.push(`<button class="path-primary-btn path-primary-btn--anchor path-checklist-btn" data-day-id="${a(e.id)}">${l(`checklist`,i(`pathTab.btnChecklist`))}</button>`):s.push(`<button class="path-primary-btn day-detail-btn" data-day-id="${a(e.id)}">${l(`plan`,i(`pathTab.btnOpenFullPlan`))}</button>`),r===e.id)s.push(`<button class="day-action-btn day-action-btn--success day-pin-save-btn" data-day-id="${a(e.id)}">${a(i(`pathTab.btnSavePin`))}</button>`),s.push(`<button class="day-action-btn day-action-btn--danger-fill day-pin-delete-btn" data-day-id="${a(e.id)}">${a(i(`pathTab.btnCancelPinEdit`))}</button>`);else{let t=e.lat?i(o?`pathTab.btnEditAnchorPin`:`pathTab.btnEditPin`):i(o?`pathTab.btnSetAnchorPin`:`pathTab.btnAddPin`);s.push(`<button class="day-action-btn day-action-btn--neutral day-pin-toggle-btn" data-day-id="${a(e.id)}">${l(`pin`,t)}</button>`)}return o?(s.push(`<button class="day-action-btn day-action-btn--neutral path-documents-btn" data-day-id="${a(e.id)}">${l(`document`,i(`pathTab.btnDocuments`))}</button>`),s.push(`<button class="day-action-btn day-action-btn--neutral path-photos-btn" data-day-id="${a(e.id)}">${l(`photo`,i(`pathTab.btnPhotos`))}</button>`)):(s.push(`<button class="day-action-btn day-action-btn--neutral day-journaling-btn" data-day-id="${a(e.id)}">${l(`journal`,i(`pathTab.btnJournaling`))}</button>`),s.push(`<button class="day-action-btn day-action-btn--danger day-delete-btn" data-day-id="${a(e.id)}">${l(`trash`,i(`pathTab.btnDeleteDay`))}</button>`)),`<div class="path-options-stack">${s.join(``)}</div>`}function v(e){let{activeTrip:n,tripDays:s,tripIsEditable:c,editingDayId:l}=e,u=[...s].sort((e,t)=>e.dayNumber-t.dayNumber),d=u.find(e=>e.dayNumber===0)||null,f=u.filter(e=>e.dayNumber>0),p=T(n,u),h=u.find(e=>e.id===p)||null;if(u.length===0)return`<div class="card glass" style="padding:28px; border-radius:18px; text-align:center; color:var(--text-secondary);">${a(i(`pathTab.emptyState`))}</div>`;let v=f.length,y=h?.dayNumber===0?t(`path.summaryHub`,v,{count:v}):h?i(`path.summaryDay`,{day:h.dayNumber,total:v}):t(`path.summaryNone`,v,{count:v}),b=r(),x=u.map(e=>{let t=e.id===p,n=e.dayNumber===0,r=!n&&e.date===b,s=`path-chip${n?` path-chip--anchor`:``}${r?` path-chip--today`:``}${t?` is-selected`:``}`,c=n?i(`pathTab.chipHubTooltip`):`${r?i(`pathTab.chipTodayPrefix`)+` · `:``}${i(`tripMedia.dayBucketDay`,{n:e.dayNumber})}${e.name?` — `+e.name:``}${e.date?` · `+(o(e.date)||e.date):``}`,l=n?`<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" stroke-width="1.2" stroke-linejoin="round" aria-hidden="true"><polygon points="12 2 15 8.5 22 9.3 17 14.3 18.2 21.3 12 18 5.8 21.3 7 14.3 2 9.3 9 8.5"/></svg>`:String(e.dayNumber);return`<button type="button" class="${s}" data-path-chip-day-id="${a(e.id)}" title="${a(c)}" aria-label="${a(c)}" aria-pressed="${t}">${l}</button>`}).join(``),S=c?`<button type="button" class="path-chip path-chip--add" id="pathAddDayChip" title="${a(i(`pathTab.addNewDay`))}" aria-label="${a(i(`pathTab.addNewDay`))}">+</button>`:``,C=u.findIndex(e=>e.id===p),w=C<=0,E=C<0||C>=u.length-1,D=[];if(d){let e=h?.id===d.id,t=m(d.id);D.push(`
            <div class="path-column path-column--anchor${t?` is-collapsed`:``}">
                <div class="path-card path-card--anchor${e?` is-selected`:``}" data-day-id="${a(d.id)}">
        ...
        `)}if(h&&h.dayNumber>0){let e=m(h.id);D.push(`
            <div class="path-column path-column--selected${e?` is-collapsed`:``}">
                <div class="path-card path-card--selected" data-day-id="${a(h.id)}">
        ...
        <div class="path-strip">
            <button type="button" class="path-nav-btn" id="pathPrevBtn" title="${a(i(`pathTab.previousDay`))}" aria-label="${a(i(`pathTab.previousDay`))}" ${w?`disabled`:``}>
            ...
            <button type="button" class="path-nav-btn" id="pathNextBtn" title="${a(i(`pathTab.nextDay`))}" aria-label="${a(i(`pathTab.nextDay`))}" ${E?`disabled`:``}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"></polyline></svg>
            </button>
        </div>
        <div class="path-summary">${a(y)}</div>
...
//# sourceMappingURL=pathSelection-1vF3xQWB.js.map