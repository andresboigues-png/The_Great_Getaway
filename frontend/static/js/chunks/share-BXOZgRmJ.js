import{n as e}from"./rolldown-runtime-Kw0j5LDr.js";import{A as t,An as n,En as r,Kn as i,Sn as a,T as o,Tt as s,at as c,jn as l,n as u,qn as d,w as f,wt as p,yt as m}from"../app.bundle.js";var h=e({openShareChooserModal:()=>v,openShareTripModal:()=>_,openTripInviteResponseModal:()=>g}),g=e=>{let o=e.related_id?String(e.related_id):``;if(!o)return;let p=!1,h=e.id,g=u(`trip_invite`,e.message),{root:_,close:v}=a({variant:`glass-light`,cardStyle:`width: 440px;`,onClose:()=>{p||h!=null&&m(h)},innerHTML:`
        ...
        `});n(_,`#tripInviteAcceptBtn`).onclick=async()=>{p=!0;let e=await c(o,!0);if(!e||!e.ok){l(f(`modals.inviteErrorInvalid`)),v();return}v(),await t(),i.trips.find(e=>e.id===o)&&(i.activeTripId=o,d(`state:changed`)),l(f(`modals.inviteSuccessJoined`),`success`),s(`home`)},n(_,`#tripInviteDeclineBtn`).onclick=async()=>{p=!0;let e=await c(o,!1);!e||!e.ok?l(f(`modals.inviteErrorNotActive`)):l(f(`modals.inviteToastDeclined`),`info`),v()}},_=e=>{if(!e)return;let t=i.trips.find(t=>t.id===e.id)||i.archivedTrips.find(t=>t.id===e.id)||e,s=t.shareToken||null,c=!!t.shareShowCost,u=!!t.shareShowPlans,{root:m,close:h}=a({variant:`glass`,cardStyle:`width: 460px; position: relative;`,innerHTML:`
            ${`
    ...
    `}
        ...
        `});n(m,`#modalCloseX`).onclick=()=>h();let g=n(m,`#shareStateBlock`),_=n(m,`#shareGenerateBtn`),v=n(m,`#shareSecondaryBtn`),y=n(m,`#shareCostToggle`),b=n(m,`#sharePlansToggle`),x=s,S=e=>`${window.location.origin}/share/${e}`,C=()=>{if(x){let e=S(x),n=t.shareViews||0;g.innerHTML=`
            ...
            `,_.textContent=f(`share.generateBtn`),v.textContent=f(`share.closeBtn`)};C();let w=async()=>{if(x){let e=S(x);try{await navigator.clipboard.writeText(e),l(f(`share.linkCopied`),`success`)}catch{let t=document.createElement(`textarea`);t.value=e,document.body.appendChild(t),t.select();try{document.execCommand(`copy`)}catch{}document.body.removeChild(t),l(f(`share.linkCopied`),`success`)}return}_.disabled=!0,_.textContent=f(`share.generating`);try{let t=await p(`/api/trips/${encodeURIComponent(e.id)}/share`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({showCost:y.checked,showPlans:b.checked})});if(!t.ok)throw Error(`share HTTP ${t.status}`);let n=await t.json();x=n.token;let r=i.trips.find(t=>t.id===e.id)||i.archivedTrips.find(t=>t.id===e.id);r&&(r.shareToken=x,r.shareShowCost=!!n.showCost,r.shareShowPlans=!!n.showPlans,typeof r.shareViews!=`number`&&(r.shareViews=0)),d(`state:changed`),C();try{await navigator.clipboard.writeText(S(x))}catch{}l(f(`share.linkReady`),`success`)}catch(e){console.error(`Generate share link failed:`,e),l(f(`share.generateFailed`)),_.disabled=!1,C()}},T=async()=>{if(!x){h();return}v.disabled=!0,v.textContent=f(`share.unsharing`);try{let t=await p(`/api/trips/${encodeURIComponent(e.id)}/share`,{method:`DELETE`});if(!t.ok)throw Error(`unshare HTTP ${t.status}`);x=null;let n=i.trips.find(t=>t.id===e.id)||i.archivedTrips.find(t=>t.id===e.id);n&&(n.shareToken=null,n.shareShowCost=!1,n.shareShowPlans=!1),d(`state:changed`),C(),l(f(`share.linkRevoked`),`success`)}catch(e){console.error(`Unshare failed:`,e),l(f(`share.revokeFailed`))}finally{v.disabled=!1}},E=async(t,n)=>{if(x)try{let t=await p(`/api/trips/${encodeURIComponent(e.id)}/share`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({showCost:y.checked,showPlans:b.checked})});if(!t.ok)throw Error(`update HTTP ${t.status}`);let n=await t.json();x=n.token;let r=i.trips.find(t=>t.id===e.id)||i.archivedTrips.find(t=>t.id===e.id);r&&(r.shareToken=x,r.shareShowCost=!!n.showCost,r.shareShowPlans=!!n.showPlans),d(`state:changed`),C()}catch(e){console.error(`Toggle ${n} failed:`,e),t.checked=!t.checked,l(f(`share.toggleFailed`))}};y.addEventListener(`change`,()=>void E(y,`showCost`)),b.addEventListener(`change`,()=>void E(b,`showPlans`)),_.onclick=w,v.onclick=T};function v(e){let{trip:t,onShareToFeed:i,showFeedOption:o=!0}=e;if(!t)return;let{root:s,close:c}=a({variant:`glass`,cardStyle:`width: 420px; position: relative;`,innerHTML:`
            ${`
        <button type="button" id="modalCloseX" aria-label="${r(f(`share.closeAriaLabel`))}"
    ...
    `}
        ...
        `});n(s,`#modalCloseX`).onclick=()=>c();let l=n(s,`#shareChooserFeedBtn`),u=n(s,`#shareChooserLinkBtn`),d=n(s,`#shareChooserCancelBtn`);l&&(l.onclick=()=>{c(),i()}),u.onclick=()=>{c(),_(t)},d.onclick=()=>c()}export{h as i,_ as n,g as r,v as t};
//# sourceMappingURL=share-BXOZgRmJ.js.map