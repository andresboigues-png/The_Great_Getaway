import{r as e,t}from"./vendor-react-CYVQMBjw.js";import{H as n,J as r,K as i,Q as a,W as o,X as s,Y as c,Z as l,at as u,et as d,n as f,q as p,rt as m,t as h,tt as g,x as _,z as v}from"../app.bundle.js";import{t as y}from"./store-BH9LnXCz.js";var b=e();function x(e){if(!e)return{balances:{},roster:[],expenses:[]};let t=(d.expenses||[]).filter(t=>t.tripId===e.id),n=m(e),r=n.length>0?n:Array.from(new Set(t.flatMap(e=>[e.who,...Object.keys(e.splits||{})]).filter(Boolean))),i={};r.forEach(e=>i[e]=0);for(let e of t){let t=e.euroValue||e.value||0;if(i[e.who]!==void 0&&(i[e.who]+=t),e.splits&&Object.keys(e.splits).length>0)for(let[n,r]of Object.entries(e.splits))i[n]!==void 0&&(i[n]-=t*(Number(r)/100));else{let e=t/Math.max(r.length,1);r.forEach(t=>{i[t]!==void 0&&(i[t]-=e)})}}return{balances:i,roster:r,expenses:t}}function S(e){let t=[],n=[];for(let[r,i]of Object.entries(e))i>.01?t.push({person:r,amount:i}):i<-.01&&n.push({person:r,amount:Math.abs(i)});t.sort((e,t)=>t.amount-e.amount),n.sort((e,t)=>t.amount-e.amount);let r=[],i=0,a=0;for(;i<n.length&&a<t.length;){let e=n[i],o=t[a],s=Math.min(e.amount,o.amount);r.push({from:e.person,to:o.person,amount:s}),e.amount-=s,o.amount-=s,e.amount<.01&&i++,o.amount<.01&&a++}return r}function C(){let e={};for(let t of[...d.trips,...d.archivedTrips||[]])for(let n of m(t))n in e||(e[n]=0);let t=(d.archivedTrips||[]).flatMap(e=>e.expenses||[]),n=[...d.expenses,...t],r={};for(let e of[...d.trips,...d.archivedTrips||[]])r[e.id]=m(e);for(let t of n){let n=t.euroValue||t.value||0;if(e[t.who]!==void 0&&(e[t.who]+=n),t.splits&&Object.keys(t.splits).length>0)for(let[r,i]of Object.entries(t.splits))e[r]!==void 0&&(e[r]-=n*(Number(i)/100));else{let i=r[t.tripId]||[],a=i.length>0?i:Array.from(new Set([t.who,...Object.keys(t.splits||{})].filter(Boolean))),o=n/Math.max(a.length,1);a.forEach(t=>{e[t]!==void 0&&(e[t]-=o)})}}return e}function w(e){if(!e)return[];let t=(d.expenses||[]).filter(t=>t.tripId===e.id),n=m(e),r={};n.forEach(e=>r[e]={paid:0,share:0});for(let e of t){let t=e.euroValue||e.value||0;if(r[e.who]&&(r[e.who].paid+=t),e.splits&&Object.keys(e.splits).length>0)for(let[n,i]of Object.entries(e.splits))r[n]&&(r[n].share+=t*(Number(i)/100));else{let e=t/Math.max(n.length,1);n.forEach(t=>{r[t]&&(r[t].share+=e)})}}return Object.entries(r).map(([e,t])=>({name:e,paid:t.paid,share:t.share,net:t.paid-t.share}))}function T(e,t,n,r){let i=E(r),a=`
    ...
    `:`
        ...
        `}function E(e){if(d.trips.length===0)return``;let t=d.trips.find(t=>t.id===e),n=t?(d.expenses||[]).filter(e=>e.tripId===t.id&&e.isSettlement).reduce((e,t)=>e+(t.euroValue||0),0):0,r=d.trips.map(t=>{let n=(d.expenses||[]).filter(e=>e.tripId===t.id&&e.isSettlement).reduce((e,t)=>e+(t.euroValue||0),0),r=n>0?` — ${i(n,`EUR`)} ${h(`settlement.settledSuffix`)}`:``;return`<option value="${o(t.id)}"${t.id===e?` selected`:``}>${o(t.name)}${r}</option>`}).join(``);return`
        <div class="settlement-trip-picker" style="margin-top: 18px; margin-bottom: 16px; display:flex; align-items:center; gap:12px; flex-wrap:wrap;">
            ...
            ${t&&n>0?`
                <span style="display:inline-flex; align-items:center; padding:6px 12px; border-radius:999px; background:rgba(0,113,227,0.08); color:#005bb8; font-size:0.78rem; font-weight:800; flex-shrink:0;">
                    ${i(n,`EUR`)} ${h(`settlement.settledSuffix`)}
                </span>
            `:``}
        </div>
    `}function D(e,t){let n=(d.expenses||[]).filter(t=>t.tripId===e.id&&t.isSettlement).length,r=(e,n,r)=>`
    ...
    `}function O(e,t){let{balances:n}=x(e),r=S(n),a=w(e),s=a.reduce((e,t)=>e+t.paid,0),c=[...a].sort((e,t)=>t.paid-e.paid)[0],l=[...a].sort((e,t)=>e.net-t.net)[0],u=[...a].sort((e,t)=>t.net-e.net)[0],d=s>0?`
    ...
    `:``,f=Object.entries(n).map(([e,t])=>{let n=t>.01,r=t<-.01;return`
        ...
        `:``}
    `}function k(e,t){let n=(d.expenses||[]).filter(t=>t.tripId===e.id&&t.isSettlement).sort((e,t)=>new Date(t.date).getTime()-new Date(e.date).getTime());if(n.length===0)return`
        ...
        `;let r={};for(let e of n){let t=e.date||`undated`;r[t]||(r[t]=[]),r[t].push(e)}let a=new Date().toISOString().slice(0,10),s=new Date;s.setDate(s.getDate()-1);let c=s.toISOString().slice(0,10),l=e=>{if(e===`undated`)return h(`settlement.historyDateNoDate`);if(e===a)return h(`settlement.historyDateToday`);if(e===c)return h(`settlement.historyDateYesterday`);let t=new Date(e);return isNaN(t.getTime())?e:t.toLocaleDateString(void 0,{weekday:`short`,month:`short`,day:`numeric`,year:`numeric`})},u=Object.keys(r).sort((e,t)=>e===`undated`?1:t===`undated`?-1:t.localeCompare(e));return`
        <div class="card glass" style="padding: 22px 24px; border-radius: 28px;">
                ...
                ${u.map(n=>{let a=r[n],s=a.reduce((e,t)=>e+(t.euroValue||0),0);return`
                        <div>
                            <div style="display:flex; align-items:baseline; justify-content:space-between; margin-bottom:8px; padding: 0 4px;">
                                ...
                                <span style="font-size:0.72rem; font-weight:700; color:var(--text-secondary);">${f(`settlement.historyDayTotalPlural`,a.length,{amount:i(s,`EUR`)})}</span>
                            </div>
                            <div style="display:flex; flex-direction:column; gap:8px;">
    ...
    `}function A(){let e=C(),t=Object.entries(e).sort((e,t)=>t[1]-e[1]),n=Math.max(...Object.values(e).map(Math.abs),1),r=t.some(([,e])=>Math.abs(e)>.01);if(t.length===0)return`
        ...
        `:``}
    `}function j(e,t,r,a,o){if(t===r){l(h(`settlement.toastSenderEqualsReceiver`));return}if(!Number.isFinite(a)||a<=0){l(h(`settlement.toastAmountInvalid`));return}let s=n(a,o,`EUR`),c={id:p(),tripId:e,label:`Settlement: ${t} → ${r}`,value:a,euroValue:s,currency:o,who:t,categoryId:d.categories[0]?.id??``,country:h(`settlement.expenseCountry`),date:new Date().toISOString().split(`T`)[0]??``,splits:{[r]:100},isSettlement:!0};d.expenses.push(c),g(u.STATE_CHANGED),l(`Recorded ${i(s,`EUR`)} ${t} → ${r}`)}function M(e){s({title:h(`settlement.toastUnsettleConfirmTitle`),message:h(`settlement.toastUnsettleConfirmMessage`),confirmText:h(`settlement.toastUnsettleConfirmBtn`),onConfirm:()=>{d.expenses=d.expenses.filter(t=>t.id!==e),g(u.STATE_CHANGED)}})}function N(e){let t=m(d.trips.find(t=>t.id===e)).map(e=>`<option value="${o(e)}">${o(e)}</option>`).join(``),n=r(),{root:i,close:s}=a({variant:`glass-light`,cardStyle:`width: 440px; max-width: calc(100vw - 32px);`,innerHTML:`
        ...
        `});c(i,`#cancelManualSettleBtn`).onclick=()=>s(),c(i,`#manualSettleForm`).onsubmit=t=>{t.preventDefault();let r=c(i,`#manualSettleFrom`).value,a=c(i,`#manualSettleTo`).value,o=parseFloat(c(i,`#manualSettleAmount`).value);if(r===a){l(h(`settlement.toastSenderEqualsReceiver`));return}j(e,r,a,o,n),s()}}function P(e){let t=d.expenses.find(t=>t.id===e);if(!t)return;let i=m(d.trips.find(e=>e.id===t.tripId)),s=i.map(e=>`<option value="${o(e)}" ${t.who===e?`selected`:``}>${o(e)}</option>`).join(``),f=Object.keys(t.splits||{})[0],p=i.map(e=>`<option value="${o(e)}" ${f===e?`selected`:``}>${o(e)}</option>`).join(``),_=r(),{root:v,close:y}=a({variant:`glass-light`,cardStyle:`width: 440px; max-width: calc(100vw - 32px);`,innerHTML:`
            ...
            <form id="editSettlementForm" style="display:flex; flex-direction:column; gap: var(--space-3); margin-top: var(--space-4);">
                ...
                <label class="form-label" style="margin-top:6px;">Amount (${o(_)})</label>
                <input type="number" step="0.01" min="0.01" id="editSettleAmount" value="${n(t.euroValue||0,`EUR`,_).toFixed(2)}" class="glass-input" required style="padding: var(--space-3); border-radius: 12px;">
