import{r as e,t}from"./vendor-react-CYVQMBjw.js";import{$ as n,C as r,E as i,H as a,I as o,M as s,O as c,Q as l,R as u,T as d,U as f,X as p,Y as m,Z as h,h as g,m as _,p as v,q as y,r as b,t as x,y as S}from"../app.bundle.js";var C=e();function w(e){if(!e)return``;let t=Array.isArray(e.items)?e.items.filter(Boolean):[];if(t.length>0)return`<ul class="ai-plan-block__list">${t.map(e=>T(e)).join(``)}</ul>`;if(e.description){let t=String(e.description).split(/\n+/).map(e=>e.trim()).filter(Boolean);return t.length>1?`<ul class="ai-plan-block__list">${t.map(e=>`<li>${a(e.replace(/^[-•*]\s*/,``))}</li>`).join(``)}</ul>`:`<div class="ai-plan-block__desc">${a(e.description)}</div>`}return``}function T(e){if(typeof e==`string`)return`<li class="ai-plan-block__item">${a(e)}</li>`;if(!e||typeof e!=`object`)return``;let t=e,n=String(t.text||``);if(!n)return``;if(t.verified&&t.placeId){let e=t.photoUrl?`<img class="ai-place-card__photo" src="${a(t.photoUrl)}" alt="" referrerpolicy="no-referrer" loading="lazy">`:`<div class="ai-place-card__photo ai-place-card__photo--empty" aria-hidden="true">📍</div>`,r=typeof t.rating==`number`?`<span class="ai-place-card__rating">★ ${t.rating.toFixed(1)}${t.userRatingsTotal?` <span class="ai-place-card__rating-count">(${E(t.userRatingsTotal)})</span>`:``}</span>`:``,i=t.address?`<span class="ai-place-card__address">${a(t.address)}</span>`:``,o=t.why?`<span class="ai-place-card__why">${a(t.why)}</span>`:``,s=t.fact?`<span class="ai-place-card__fact">✨ ${a(t.fact)}</span>`:``,c=t.mapsUrl||`https://www.google.com/maps/place/?q=place_id:${encodeURIComponent(t.placeId)}`,l=t.verifiedName||n;return`
...
`):e.activity&&e.description?`${e.activity}: ${e.description}`:e.activity||e.description||``}var k=null,A=[];function j(){let e=document.createElement(`div`),t=l.trips.find(e=>e.id===l.activeTripId);if(!t)return e.innerHTML=`
            ...
            </div>`,setTimeout(()=>{e.querySelector(`#aiStartJourneyBtn`)?.addEventListener(`click`,()=>s()),typeof google<`u`&&google.maps&&h(new google.maps.Map(document.getElementById(`emptyMap`),{center:{lat:20,lng:0},zoom:2,minZoom:2,restriction:{latLngBounds:{north:85,south:-85,west:-180,east:180},strictBounds:!0},styles:[]}),[])},100),e;let S=t.country||``,C=(l.tripDays||[]).filter(e=>e.tripId===l.activeTripId&&e.dayNumber>0&&e.date).map(e=>e.date).sort(),T=l.expenses.filter(e=>e.tripId===l.activeTripId&&e.date).sort((e,t)=>e.date.localeCompare(t.date)).map(e=>e.date),E=t.dateFrom||C[0]||T[0]||``,D=t.dateTo||C[C.length-1]||T[T.length-1]||``,j=t.aiPlan||null,M=t.aiContext||``,N=t.aiNumDays||1,P=o(t),F=`font-family:-apple-system,BlinkMacSystemFont,'SF Pro Display','SF Pro Text',sans-serif;`;return e.innerHTML=`
        <div style="${F}">
            <!-- Header -->
            ...
            <div class="ai-page-2col" style="display:grid;grid-template-columns:380px 1fr;gap:24px;margin-bottom:32px;">
                ...
                <div id="aiControlsPanel" style="display:flex;flex-direction:column;gap:16px;min-height:700px;">
                    ...
                    <!-- Generate -->
                    ${P?`<button id="generateBtn" class="ai-generate-btn" style="width:100%; border-radius: var(--radius-lg);flex:0 0 auto;">✦ Generate My Itinerary</button>`:(()=>{let e=u(t);return`<div class="card glass" style="padding:16px; border-radius: var(--radius-lg); text-align:center; color: var(--text-secondary); font-size: 0.85rem; flex:0 0 auto;">
            ...
            <div id="itineraryOutput" style="margin-bottom: 60px;"></div>
                ...
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:24px;">
                    <div>
                        <!-- Phase G UX: text colour switched from white to a
                             ...
                             cards below. -->
                        <h2 style="margin:0;font-size:2rem;font-weight:800;letter-spacing:-0.03em;background:var(--gradient-title);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;${F}">${o}-Day ${s} Itinerary</h2>
                ...
                ${P?`<div style="display:flex;gap:12px;margin-top:24px;"><button id="acceptPlanBtn" class="btn" style="flex:2;background:var(--accent-blue);color:white;padding:16px;font-size:1.1rem;border-radius:16px;font-weight:700;box-shadow:0 10px 20px rgba(0,122,255,0.2);cursor:pointer;">Accept Plan & Add to Trip</button></div>`:``}`;let u=y(c,`#itineraryDays`),f=[];if(i.forEach((e,t)=>{let n=document.createElement(`div`);n.className=`card glass`,n.style.cssText=`border-radius:18px;overflow:hidden;transition:box-shadow 0.3s,border-color 0.3s;${F}`,n.innerHTML=`
                    ...
                    </div>`,u.appendChild(n),f.push(n)}),k){A.forEach(e=>e.setMap(null)),A=[];let e=new google.maps.LatLngBounds,t=new google.maps.Geocoder,n=(n,r)=>{let i=n.mainLocation||n.title||s;!n.mainLocation&&n.title&&(i=n.title.replace(/Exploring |Day Trip to |Visit |Touring |Arrival in |Departure from |Day \d+:? /gi,``).trim()),t.geocode({address:i+`, `+s},(t,i)=>{if(i===`OK`&&t[0]){let i=t[0].geometry.location;n.lat=i.lat(),n.lon=i.lng();let a=new google.maps.Marker({position:i,map:k,label:{text:String(n.day),color:`white`,fontWeight:`800`},icon:{path:google.maps.SymbolPath.CIRCLE,scale:16,fillColor:`#0071e3`,fillOpacity:1,strokeWeight:2,strokeColor:`white`}});a.addListener(`click`,()=>{f.forEach(e=>{e.style.boxShadow=``,e.style.borderColor=``});let e=f[r];e&&(e.style.boxShadow=`0 0 0 3px var(--accent-blue), 0 8px 32px rgba(0,113,227,0.25)`,e.style.borderColor=`var(--accent-blue)`,e.scrollIntoView({behavior:`smooth`,block:`center`}))}),A.push(a),e.extend(i),A.length>0&&k.fitBounds(e)}})};i.forEach((e,t)=>setTimeout(()=>n(e,t),t*500))}let p=document.getElementById(`acceptPlanBtn`);p&&(p.onclick=()=>{if(!i)return;let e=l.tripDays.filter(e=>e.tripId===t.id&&e.dayNumber>0);l.tripDays=l.tripDays.filter(e=>!(e.tripId===t.id&&e.dayNumber>0)),e.forEach(e=>b(e.id)),d(t),i.forEach((e,n)=>{let i=e.date||new Date().toISOString().split(`T`)[0],a=`day_`+Date.now()+`_`+n,o={id:a,tripId:t.id,date:i,name:e.title||`Day ${n+1}`,dayNumber:n+1,lat:e.lat,lng:e.lon,photos:[],tickets:[],notes:``,plan:{morning:O(e.morning),afternoon:O(e.afternoon),evening:O(e.evening)}};l.tripDays.push(o),v(o);let s=[[`morning`,e.morning],[`afternoon`,e.afternoon],[`evening`,e.evening]];for(let[e,n]of s){let i=Array.isArray(n?.items)?n.items:[];for(let n of i)r(t,n,a,e)}}),_(t),n(`state:changed`),p.innerHTML=`✓ Plan Accepted! (View in Home)`,p.style.background=`#34c759`,p.disabled=!0})},C=()=>{let r=e.querySelector(`#aiTodoListPanel`);if(!r)return;let o=i(t).filter(e=>e.forManual),s=o.filter(e=>e.forAI);if(o.length===0){r.innerHTML=`
                ...
                `,r.querySelector(`#aiGoToTodoBtn`)?.addEventListener(`click`,()=>g(`todo`));return}if(s.length===0){r.innerHTML=`
                    <div class="card glass" style="padding: 20px; border-radius: 18px; border: 1.5px dashed rgba(155, 89, 182, 0.35); background: rgba(155, 89, 182, 0.04);">
                        <div style="display:flex; align-items:center; gap:10px; margin-bottom:6px;">
                            <span style="font-size: 1.2rem;">📋</span>
                            <h3 style="margin:0; color:#7c3a9e; font-weight:800; letter-spacing:-0.01em;">${o.length} item${o.length===1?``:`s`} on your to-do list</h3>
                        </div>
                ...
                `,r.querySelector(`#aiGoToTodoBtn`)?.addEventListener(`click`,()=>g(`todo`));return}let u=e.querySelector(`#aiDateFrom`),d=e.querySelector(`#aiDateTo`),p=!!(u?.value&&d?.value),m=(l.tripDays||[]).filter(e=>e.tripId===t.id&&e.dayNumber>0).sort((e,t)=>e.dayNumber-t.dayNumber),h=e=>`
            ...
            `).join(``);r.innerHTML=`
                <div class="card glass" style="padding:20px; border-radius:18px; border: 1.5px solid rgba(155, 89, 182, 0.25);">
                    <div style="display:flex; align-items:center; gap:10px; margin-bottom:14px; flex-wrap:wrap;">
                        <span style="font-size: 1.2rem;">📋</span>
                        <h3 style="margin:0; color:#7c3a9e; font-weight:800; letter-spacing:-0.01em;">Ticked for this generation <span style="background:rgba(155,89,182,0.12); color:#7c3a9e; font-size:0.7rem; padding:2px 8px; border-radius:999px; margin-left:6px;">${s.length} item${s.length===1?``:`s`}</span></h3>
            ...
            `,r.querySelector(`#aiManageTodoBtn`)?.addEventListener(`click`,()=>g(`todo`)),r.querySelectorAll(`.marked-day-select, .marked-time-select`).forEach(e=>{e.onchange=()=>{let i=e.dataset.placeId;if(!i)return;let a=r.querySelector(`.ai-marked-card[data-place-id="${i}"]`);if(!a)return;let o=a.querySelector(`.marked-day-select`),s=a.querySelector(`.marked-time-select`);c(t,i,o?.value||null,s?.value||null),n(`state:changed`),_(t)}})};C(),s&&u(s,N,S);let T=e.querySelector(`#aiExtraContext`);T&&(T.oninput=e=>{t.aiContext=e.target.value,n(`state:changed`)});let E=e.querySelector(`#aiKeyInput`),D=e.querySelector(`#aiKeyToggleBtn`),M=e.querySelector(`#aiKeyHelpBtn`),I=e.querySelector(`#aiKeyStatus`),L=()=>{if(!I)return;let e=(l.geminiApiKey||``).trim();if(!e){I.textContent=`No key saved — paste one above to enable AI generation.`,I.style.color=`#a85d00`;return}let t=e.startsWith(`AIza`)&&e.length>=30;I.textContent=t?`✓ Key saved on this device.`:`⚠ Saved, but the format looks off (Gemini keys usually start with "AIza"). Click i for help.`,I.style.color=t?`#1a6b3c`:`#a85d00`};L(),E&&E.addEventListener(`input`,()=>{l.geminiApiKey=E.value,n(`state:changed`),L()}),D&&E&&D.addEventListener(`click`,()=>{let e=E.type===`text`;E.type=e?`password`:`text`,D.textContent=e?`👁`:`🙈`,D.title=e?`Show key`:`Hide key`}),M&&M.addEventListener(`click`,()=>{let{root:e,close:t}=p({cardClass:`card glass`,cardStyle:`width: 520px; max-width: calc(100vw - 32px); max-height: 90vh; overflow-y: auto; padding: 28px 32px; border-radius: 28px; background: white;`,innerHTML:`
                    ...
                    `});e.querySelector(`#aiKeyHelpClose`)?.addEventListener(`click`,t),e.querySelector(`#aiKeyHelpDone`)?.addEventListener(`click`,t)}),[`#aiDateFrom`,`#aiDateTo`].forEach(t=>{let n=e.querySelector(t);n&&n.addEventListener(`change`,()=>C())});let R=e.querySelector(`#generateBtn`),z=async()=>{let r=y(e,`#itineraryOutput`),o=y(e,`#aiDateFrom`).value,c=y(e,`#aiDateTo`).value,d=document.getElementById(`aiExtraContext`)?.value??``;if(!o||!c){m(`Pick your travel dates first.`);return}let f=new Date(o),p=new Date(c),h=Math.max(1,Math.round((p.getTime()-f.getTime())/864e5)+1),g=i(t).filter(e=>e.forAI&&e.forManual),_=``;if(g.length>0){let e=(l.tripDays||[]).filter(e=>e.tripId===t.id&&e.dayNumber>0),n=t=>e.find(e=>e.id===t)?.dayNumber;_=`\n\nThe user has marked these specific places to include in the itinerary. Please incorporate them where they fit, respecting any day/time assignments where given:\n${g.map(e=>{let t=e.dayId?n(e.dayId):null,r=t?`, on Day ${t}`:``,i=e.timeOfDay?`, ${e.timeOfDay}`:``,a=e.address?` (${e.address})`:``;return`- ${e.name}${a}${r}${i}`}).join(`
`)}`}let v=d+_;t.aiContext=d,t.aiNumDays=h,n(`state:changed`),r.innerHTML=`<div style="text-align:center;padding:60px;"><div class="spinner-ring" style="width:40px;height:40px;border:3px solid rgba(0,113,227,0.15);border-top-color:#005bb8;border-radius:50%;animation:spin 1s linear infinite;margin:0 auto 20px;"></div><div style="color:var(--text-primary);font-weight:600;">Consulting Gemini AI…</div><div style="color:var(--text-secondary);font-size:0.82rem;margin-top:6px;">This usually takes 5-15 seconds. Maps lookups for each place add a few more.</div></div>`,r.scrollIntoView({behavior:`smooth`});let b=R?.innerHTML||``;R&&(R.disabled=!0,R.innerHTML=`⌛ Generating…`);try{let e=await(await x(`/api/generate_itinerary`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({destination:S,numDays:h,dateFrom:o,dateTo:c,context:v,gemini_key:(l.geminiApiKey||``).trim()})})).json();if(e.error)throw Error(e.error);s=e.itinerary,s==null?delete t.aiPlan:t.aiPlan=s,n(`state:changed`),u(s,h,S),r.scrollIntoView({behavior:`smooth`})}catch(e){let t=e.message||``,n=`Something went wrong while generating your plan.`,i=``;/UNAVAILABLE|503|overloaded/i.test(t)?(n=`Gemini is overloaded right now.`,i=`This usually clears in 30-60 seconds.`):/quota|limit|RESOURCE_EXHAUSTED|429/i.test(t)?(n=`Daily AI quota reached for this key.`,i=`Try again tomorrow, or use a different Gemini key in Settings → AI Engine.`):/key|api[_ ]?key|UNAUTHENTICATED|401|403/i.test(t)?(n=`AI key isn't accepted by Gemini.`,i=`Open Settings → AI Engine and check the key, or generate a new one.`):/network|fetch|timed?[\- ]?out|ECONN/i.test(t)&&(n=`Network hiccup talking to the AI.`,i=`Check your connection and retry.`),r.innerHTML=`
                    <div class="card glass" style="text-align:center;padding:32px 28px;">
                        <div style="font-size:2.4rem;margin-bottom:8px;">😬</div>
                        <h2 style="color:#a82424;margin:0 0 6px;font-size:1.2rem;">${a(n)}</h2>
                        ...
                        <details style="margin:0 0 18px;text-align:left;background:rgba(255,59,48,0.04);border:1px solid rgba(255,59,48,0.16);border-radius:10px;padding:8px 12px;">
                            ...
                            <pre style="margin:8px 0 0;font-size:0.72rem;color:#666;font-family:monospace;white-space:pre-wrap;word-break:break-word;">${a(t||`Unknown error`)}</pre>
                        </details>
                ...
                `,m(n),r.querySelector(`#aiRetryBtn`)?.addEventListener(`click`,()=>z())}finally{R&&(R.disabled=!1,R.innerHTML=b)}};R?.addEventListener(`click`,z)},0),e}var M=t();function N(){let e=(0,C.useRef)(null);return(0,C.useEffect)(()=>{let t=e.current;t&&(t.innerHTML=``,t.appendChild(j()))},[]),(0,M.jsx)(`div`,{ref:e})}function P(e){S(e,(0,C.createElement)(N))}export{P as mountAI};
//# sourceMappingURL=mount-7T2bOSmJ.js.map