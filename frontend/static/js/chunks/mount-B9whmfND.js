import{r as e,t}from"./vendor-react-CYVQMBjw.js";import{G as n,K as r,S as i,U as a,X as o,Y as s,c,d as l,f as u,m as d,nt as f,o as p,p as m,r as h,s as g,t as _,u as v,v as y,x as b}from"../app.bundle.js";var x=e(),S=new Set([`friend_shared_trip`,`friend_reposted_trip`]),C=new Set([`friend_created_trip`,`friend_archived_trip`,`friend_joined_trip`,`new_friendship`]),w={muted:`142,142,147`,like:`255,59,48`,comment:`0,113,227`,repost:`52,199,89`,bookmark:`255,149,0`};function T(e){if(!e)return``;let t=typeof e==`string`&&e.includes(` `)&&!e.includes(`T`)?e.replace(` `,`T`)+`Z`:e,n=new Date(t);return Number.isNaN(n.getTime())?``:n.toISOString().slice(0,10)}function E(e){let t=new Map,n=[];for(let r of e){if(S.has(r.type)){n.push(r);continue}let e=`${r.actor?.id||`anon`}|${r.type}|${T(r.when)}`,i=t.get(e);i||(i=[],t.set(e,i),n.push({__slot:e})),i.push(r)}return n.map(e=>{let n=e.__slot;if(!n)return e;let r=t.get(n)||[];if(r.length===1)return r[0];let i=r[0];return{bundled:!0,id:`bundle_${n}`,type:i.type,actor:i.actor,when:i.when,members:r}})}function D(e){let t=`<strong style="color:#002d5b;">${r(e.actor.name)}</strong>`,n=e.members.length,i=n===1?`trip`:`trips`;switch(e.type){case`friend_created_trip`:return`${t} started planning <strong style="color:#002d5b;">${n} new ${i}</strong>`;case`friend_archived_trip`:return`${t} just completed <strong style="color:#002d5b;">${n} ${i}</strong> 🎉`;case`friend_joined_trip`:return`${t} joined <strong style="color:#002d5b;">${n} ${i}</strong>`;case`new_friendship`:return`You and <strong style="color:#002d5b;">${n} new people</strong> are now friends 🤝`;default:return`${t} did ${n} new things`}}function O(e,t=44){let n=(e?.name||`?`).charAt(0).toUpperCase(),i=`<div style="width:${t}px; height:${t}px; border-radius:50%; background: var(--gradient-day); color:white; display:flex; align-items:center; justify-content:center; font-weight:800; font-size:${Math.round(t*.4)}px; flex-shrink:0; box-shadow: 0 2px 8px rgba(0,113,227,0.18);">${r(n)}</div>`,a=e?.picture?`<img src="${r(e.picture)}" alt="" referrerpolicy="no-referrer"
    ...
    `;let t=!1,i=e=>{let t=e.coverUrl?`background-image: url('${r(e.coverUrl)}'); background-size: cover; background-position: center;`:`background: linear-gradient(135deg, #00c7be 0%, #007aff 100%);`,n=e.owner.picture?`<img src="${r(e.owner.picture)}" alt="" referrerpolicy="no-referrer" style="width:24px; height:24px; border-radius:50%; object-fit:cover;">`:`<div style="width:24px; height:24px; border-radius:50%; background:rgba(0,113,227,0.18); display:flex; align-items:center; justify-content:center; color:#005bb8; font-size:0.7rem; font-weight:800;">${r((e.owner.firstName||`?`).slice(0,1).toUpperCase())}</div>`;return`
        ...
        `},g=()=>{let a=s(e,`#feedList`);if(!a)return;if(B===`explore`){if(H===null){a.innerHTML=`
                ...
                `;return}if(H.length===0){a.innerHTML=n({accent:`blue`,emoji:`🌍`,title:`No public trips yet`,body:`Be the first — share one of your own to seed the Explore feed for everyone.`});return}a.innerHTML=`
                <div style="display:grid; grid-template-columns:repeat(auto-fill, minmax(260px, 1fr)); gap:14px;">
                    ${H.map(i).join(``)}
                </div>
            `;return}if(!t&&R.length===0){a.innerHTML=`
                <div class="card glass" style="padding: 32px; border-radius: 24px; text-align:center;">
            ...
            `;return}let o=e=>B===`posts`?S.has(e.type):C.has(e.type),c=R.filter(e=>!(!o(e)||V&&!e.is_bookmarked));if(c.length===0){let t,r,i,o;V?(t=_(B===`posts`?`feed.emptyBookmarkedPostsTitle`:`feed.emptyBookmarkedActionsTitle`),r=_(`feed.emptyBookmarkedBody`),i=_(`feed.emptyBookmarkedCta`),o=()=>{V=!1;let t=e.querySelector(`#feedBookmarkToggle .apple-toggle__input`);t&&(t.checked=!1),g()}):B===`posts`?(t=_(`feed.emptyPostsTitle`),r=_(`feed.emptyPostsBody`),i=_(`feed.emptyPostsCta`),o=()=>{B=`actions`,g(),e.querySelectorAll(`.home-tabnav__tab`).forEach(e=>e.classList.toggle(`is-active`,e.dataset.feedTab===`actions`))}):(t=_(`feed.emptyActionsTitle`),r=_(`feed.emptyActionsBody`),i=_(`feed.emptyActionsCta`),o=()=>y(`friends`)),a.innerHTML=n({accent:`purple`,emoji:V?`🔖`:`🌱`,title:t,body:r,ctaLabel:i,ctaId:`feedEmptyCtaBtn`});let s=a.querySelector(`#feedEmptyCtaBtn`);s&&(s.onclick=o);return}let l=f.user?.id;a.innerHTML=E(c).map(e=>{if(e.bundled){let t=e,n=j(t.type),i=k(t.when),a=I.has(t.id),o=t.members.map(e=>{let t=A(e),n=!!e.is_bookmarked;return`
            ...
            `}).join(``)},x=async()=>{if(!f.user){t=!0,g();return}try{let e=await h(`/api/feed`);if(!e.ok)return;let t=await e.json();Array.isArray(t)&&(R=t)}catch(e){console.error(`Feed refresh failed:`,e)}finally{t=!0,g()}};e.querySelectorAll(`.home-tabnav__tab[data-feed-tab]`).forEach(t=>{t.onclick=()=>{let n=t.dataset.feedTab;!n||B===n||(B=n,e.querySelectorAll(`.home-tabnav__tab[data-feed-tab]`).forEach(e=>{e.classList.toggle(`is-active`,e.dataset.feedTab===n)}),n===`explore`&&W(g),g())}});let T=e.querySelector(`#feedBookmarkToggle .apple-toggle__input`);return T&&T.addEventListener(`change`,()=>{V=!!T.checked,g()}),e.addEventListener(`click`,async e=>{let t=e.target;if(!t)return;let n=t.closest(`.feed-avatar-btn`);if(n?.dataset.feedAvatarUserId){y(`profile`,{userId:n.dataset.feedAvatarUserId});return}let r=t.closest(`.feed-trip-card`);if(r?.dataset.tripId){b(r.dataset.tripId);return}let i=t.closest(`.feed-like-btn`);if(i?.dataset.eventId){let e=i.dataset.eventId,t=i.dataset.liked===`1`,n=!t,r=R.find(t=>t.id===e);r&&(r.is_liked=n,r.like_count=Math.max(0,(r.like_count||0)+(t?-1:1))),i.dataset.liked=n?`1`:`0`,i.style.setProperty(`--accent`,n?w.like:w.muted),i.innerHTML=M(`heart`,n),L(i);let a=i.parentElement?.querySelector(`.feed-action-count`),o=e=>e>=3?String(e):``;a&&r&&(a.textContent=o(r.like_count??0));let s=await m(e);s.ok&&s.body&&r&&(r.is_liked=!!s.body.liked,r.like_count=Number(s.body.count)||0,a&&(a.textContent=o(r.like_count)));return}let s=t.closest(`.feed-bookmark-btn`);if(s?.dataset.eventId){let e=s.dataset.eventId,t=s.dataset.bookmarked!==`1`,n=R.find(t=>t.id===e);n&&(n.is_bookmarked=t),s.dataset.bookmarked=t?`1`:`0`,s.style.setProperty(`--accent`,t?w.bookmark:w.muted),s.innerHTML=M(`bookmark`,t),L(s),V&&!t&&g(),await u(e);return}let f=t.closest(`.feed-bundle-toggle`);if(f?.dataset.bundleId){let e=f.dataset.bundleId;I.has(e)?I.delete(e):I.add(e),g();return}let h=t.closest(`.feed-comment-btn`);if(h?.dataset.eventId){let e=h.dataset.eventId,t=h.closest(`.feed-event`)?.querySelector(`.feed-thread`);if(!t)return;if(t.style.display!==`none`){t.style.display=`none`;return}t.style.display=`block`,z[e]?G(t,e,z[e]):(t.innerHTML=`<div style="font-size:0.82rem; color:var(--text-secondary); padding:6px 0;">${_(`feed.commentsLoading`)}</div>`,z[e]=await c(e)||[],G(t,e,z[e]));let n=t.querySelector(`input[name="body"]`);n&&n.focus();return}let v=t.closest(`.feed-comment-delete-btn`);if(v?.dataset.commentId){let e=Number(v.dataset.commentId),t=v.closest(`.feed-comment-row`),n=v.closest(`.feed-thread`),r=n?.dataset.eventId;t&&t.remove(),r&&z[r]&&(z[r]=z[r].filter(t=>t.id!==e));let i=r?R.find(e=>e.id===r):null;if(i){i.comment_count=Math.max(0,(i.comment_count||0)-1);let e=((n?.closest(`.feed-event`))?.querySelector(`.feed-comment-btn`))?.parentElement?.querySelector(`.feed-action-count`);e&&(e.textContent=i.comment_count>0?String(i.comment_count):``)}(await p(e)).ok||o(`Couldn't delete — try again in a moment.`);return}let S=t.closest(`.feed-unshare-btn`);if(S?.dataset.postId){let e=Number(S.dataset.postId);a({title:_(`feed.toastUnshareConfirmTitle`),message:_(`feed.toastUnshareConfirmMessage`),confirmText:_(`feed.toastUnshareConfirmBtn`),onConfirm:async()=>{let t=await d(e);if(!t||!t.ok){o(_(`feed.toastUnshareFailed`));return}await x(),o(_(`feed.toastRemovedFromFeed`))}});return}let C=t.closest(`.feed-repost-btn`);if(C?.dataset.postId){let e=Number(C.dataset.postId),t=C.style.getPropertyValue(`--accent`)||w.muted;C.disabled=!0,C.style.setProperty(`--accent`,w.muted);let n=await l(e);n.ok&&n.body?.status!==`same_user`?(o(n.body?.status===`already_reposted`?_(`feed.toastAlreadyReposted`):_(`feed.toastReposted`)),C.style.setProperty(`--accent`,w.repost),C.innerHTML=`<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="20 6 9 17 4 12"></polyline></svg>`,L(C)):n.body?.status===`same_user`?(C.disabled=!1,C.style.setProperty(`--accent`,t),o(_(`feed.toastRepostOwnShare`))):(C.disabled=!1,C.style.setProperty(`--accent`,t),o(_(`feed.toastRepostFailed`)));return}}),e.addEventListener(`submit`,async e=>{let t=e.target;if(!t?.classList?.contains(`feed-comment-form`))return;e.preventDefault();let n=t.dataset.eventId;if(!n)return;let r=t.querySelector(`input[name="body"]`),i=r?.value.trim();if(!i)return;let a=t.querySelector(`.feed-comment-submit`);r&&(r.value=``),a&&(a.disabled=!0);let s=await v(n,i);if(a&&(a.disabled=!1),!s.ok||!s.body?.comment){r&&(r.value=i),o(`Couldn't post comment — try again.`);return}let c=s.body.comment;z[n]||(z[n]=[]),z[n].push(c);let l=t.closest(`.feed-thread`);l&&G(l,n,z[n]);let u=l?.querySelector(`input[name="body"]`);u&&u.focus();let d=R.find(e=>e.id===n);if(d){d.comment_count=(d.comment_count||0)+1;let e=((l?.closest(`.feed-event`))?.querySelector(`.feed-comment-btn`))?.parentElement?.querySelector(`.feed-action-count`);e&&(e.textContent=String(d.comment_count))}}),g(),x(),e}var q=t();function J(){let e=(0,x.useRef)(null);return(0,x.useEffect)(()=>{let t=e.current;t&&(t.innerHTML=``,t.appendChild(K()))},[]),(0,q.jsx)(`div`,{ref:e})}function Y(e){i(e,(0,x.createElement)(J))}export{Y as mountFeed};
//# sourceMappingURL=mount-B9whmfND.js.map