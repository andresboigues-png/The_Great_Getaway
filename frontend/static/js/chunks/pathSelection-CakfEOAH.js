import{Ht as e,Jt as t,S as n,Ut as r,ft as i,mt as a,rn as o,x as s}from"../app.bundle.js";var c=(t,n)=>`<span style="display:inline-flex;align-items:center;gap:6px;justify-content:center">${i(t)}${e(a(n))}</span>`,l=`home_path_card_collapsed_day_ids`;function u(){try{let e=localStorage.getItem(l);if(!e)return new Set;let t=JSON.parse(e);return new Set(Array.isArray(t)?t:[])}catch{return new Set}}function d(e){try{localStorage.setItem(l,JSON.stringify([...e]))}catch{}}function f(e){let t=u();return t.has(e)?(t.delete(e),d(t),!1):(t.add(e),d(t),!0)}function p(e){return u().has(e)}function m(e,t){let n=u(),r=n.has(e);t&&!r?(n.add(e),d(n)):!t&&r&&(n.delete(e),d(n))}function h(n,o,c){let{isAnchor:l,isSelected:u}=o,d=l?`<div style="background: var(--gradient-anchor-deep); color: white; width: 48px; height: 48px; border-radius: 50%; border: 3px solid white; display: flex; align-items: center; justify-content: center; flex-shrink:0; box-shadow: 0 8px 18px rgba(212,160,23,0.28);">
               ...
               <span style="font-size: 0.6rem; font-weight: 800; text-transform: uppercase; opacity: 0.85; letter-spacing: 0.05em; line-height:1;">${e(s(`pathTab.dayBadgeLabel`))}</span>
               <span style="font-size: 1.25rem; font-weight: 800; line-height: 1.05;">${n.dayNumber}</span>
           </div>`,f=l?s(`pathTab.hubTitle`):e(n.name||s(`tripMedia.dayBucketDay`,{n:n.dayNumber})),p=[];if(l){p.push(c&&c.country?e(t(c.country)):e(s(`pathTab.hubSubtitleFallback`)));let r=(c.documents||[]).filter(e=>e.dayId===n.id),a=(c.photos||[]).filter(e=>e.dayId===n.id),o=r.length+(n.tickets||[]).length,l=a.length+(n.photos||[]).length;l&&p.push(`<span style="display:inline-flex; align-items:center; gap:4px; background:rgba(52,199,89,0.12); color:#1a6b3c; padding:2px 8px; border-radius:999px; font-size:0.7rem; font-weight:800;">${i(`photo`,{size:13})}${l}</span>`),o&&p.push(`<span style="display:inline-flex; align-items:center; gap:4px; background:rgba(88,86,214,0.12); color:#5856d6; padding:2px 8px; border-radius:999px; font-size:0.7rem; font-weight:800;">${i(`document`,{size:13})}${o}</span>`)}else p.push(`<span style="display:inline-flex; align-items:center; gap:5px;">${i(`calendar`,{size:14})}${r(n.date)||s(`pathTab.setDatePlaceholder`)}</span>`),n.lat?p.push(`<span style="color: #005bb8;">${e(s(`pathTab.locationSet`))}</span>`):p.push(`<span class="day-card__pin-hint" style="display:inline-flex; align-items:center; gap:4px;">${i(`pinned`,{size:13})}${e(a(s(`pathTab.pinThisDay`)))}</span>`),n.date&&p.push(`<span class="day-card__weather" data-weather-date="${e(n.date)}"></span>`);let m=u&&n.notes&&!l?`
        <div style="margin-top: 12px; padding: 12px 14px; background: rgba(0,113,227,0.04); border-radius: 14px; border-left: 3px solid var(--accent-blue);">
            <div style="font-size: 0.7rem; font-weight: 800; text-transform: uppercase; color: #005bb8; margin-bottom: 4px; letter-spacing: 0.05em;">${e(s(`pathTab.journalPreviewLabel`))}</div>
    ...
    `:``,h=`
        <button type="button" class="path-card-collapse-btn" data-day-id="${e(n.id)}"
            aria-label="${e(s(`pathTab.toggleOptionsAria`,{title:f}))}" title="${e(s(`pathTab.toggleOptionsTitle`))}">
    ...
    `;return`
        ...
        ${m}
    `}function g(t,n,r,i){if(!t||!r)return``;let{isAnchor:a}=n,o=[];if(a?o.push(`<button class="path-primary-btn path-primary-btn--anchor path-checklist-btn" data-day-id="${e(t.id)}">${c(`checklist`,s(`pathTab.btnChecklist`))}</button>`):o.push(`<button class="path-primary-btn day-detail-btn" data-day-id="${e(t.id)}">${c(`plan`,s(`pathTab.btnOpenFullPlan`))}</button>`),i===t.id)o.push(`<button class="day-action-btn day-action-btn--success day-pin-save-btn" data-day-id="${e(t.id)}">${e(s(`pathTab.btnSavePin`))}</button>`),o.push(`<button class="day-action-btn day-action-btn--danger-fill day-pin-delete-btn" data-day-id="${e(t.id)}">${e(s(`pathTab.btnCancelPinEdit`))}</button>`);else{let n=t.lat?s(a?`pathTab.btnEditAnchorPin`:`pathTab.btnEditPin`):s(a?`pathTab.btnSetAnchorPin`:`pathTab.btnAddPin`);o.push(`<button class="day-action-btn day-action-btn--neutral day-pin-toggle-btn" data-day-id="${e(t.id)}">${c(`pin`,n)}</button>`)}return a?(o.push(`<button class="day-action-btn day-action-btn--neutral path-documents-btn" data-day-id="${e(t.id)}">${c(`document`,s(`pathTab.btnDocuments`))}</button>`),o.push(`<button class="day-action-btn day-action-btn--neutral path-photos-btn" data-day-id="${e(t.id)}">${c(`photo`,s(`pathTab.btnPhotos`))}</button>`)):(o.push(`<button class="day-action-btn day-action-btn--neutral day-journaling-btn" data-day-id="${e(t.id)}">${c(`journal`,s(`pathTab.btnJournaling`))}</button>`),o.push(`<button class="day-action-btn day-action-btn--danger day-delete-btn" data-day-id="${e(t.id)}">${c(`trash`,s(`pathTab.btnDeleteDay`))}</button>`)),`<div class="path-options-stack">${o.join(``)}</div>`}function _(t){let{activeTrip:i,tripDays:a,tripIsEditable:o,editingDayId:c}=t,l=[...a].sort((e,t)=>e.dayNumber-t.dayNumber),u=l.find(e=>e.dayNumber===0)||null,d=l.filter(e=>e.dayNumber>0),f=w(i,l),m=l.find(e=>e.id===f)||null;if(l.length===0)return`<div class="card glass" style="padding:28px; border-radius:18px; text-align:center; color:var(--text-secondary);">${e(s(`pathTab.emptyState`))}</div>`;let _=d.length,v=m?.dayNumber===0?n(`path.summaryHub`,_,{count:_}):m?s(`path.summaryDay`,{day:m.dayNumber,total:_}):n(`path.summaryNone`,_,{count:_}),y=(()=>{let e=new Date;return`${e.getFullYear()}-${String(e.getMonth()+1).padStart(2,`0`)}-${String(e.getDate()).padStart(2,`0`)}`})(),b=l.map(t=>{let n=t.id===f,i=t.dayNumber===0,a=!i&&t.date===y,o=`path-chip${i?` path-chip--anchor`:``}${a?` path-chip--today`:``}${n?` is-selected`:``}`,c=i?s(`pathTab.chipHubTooltip`):`${a?s(`pathTab.chipTodayPrefix`)+` · `:``}${s(`tripMedia.dayBucketDay`,{n:t.dayNumber})}${t.name?` — `+t.name:``}${t.date?` · `+(r(t.date)||t.date):``}`,l=i?`<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" stroke-width="1.2" stroke-linejoin="round" aria-hidden="true"><polygon points="12 2 15 8.5 22 9.3 17 14.3 18.2 21.3 12 18 5.8 21.3 7 14.3 2 9.3 9 8.5"/></svg>`:String(t.dayNumber);return`<button type="button" class="${o}" data-path-chip-day-id="${e(t.id)}" title="${e(c)}" aria-label="${e(c)}" aria-pressed="${n}">${l}</button>`}).join(``),x=o?`<button type="button" class="path-chip path-chip--add" id="pathAddDayChip" title="${e(s(`pathTab.addNewDay`))}" aria-label="${e(s(`pathTab.addNewDay`))}">+</button>`:``,S=l.findIndex(e=>e.id===f),C=S<=0,T=S<0||S>=l.length-1,E=[];if(u){let t=m?.id===u.id,n=p(u.id);E.push(`
