import{Ct as e,Dt as t,H as n,L as r,P as i,Rt as a,S as o,W as s,X as c,Y as l,b as u,bt as d,dt as f,gt as p,ht as m,i as h,j as g,jt as _,mt as v,pt as y,v as b,x,zt as S}from"../app.bundle.js";var C=async(e,t)=>{let n=a.archivedTrips.find(t=>t.id===e)||a.trips.find(t=>t.id===e);if(n&&(n.isPublic=t!==`private`,n.publicShowExpenses=t===`public-full`,S(`state:changed`),a.user))try{await l(n)}catch{}},w=e=>{let t=a.archivedTrips.find(t=>t.id===e);t&&d({title:`Restore Trip?`,message:`This will move the trip back to your active list.`,confirmText:`Restore`,onConfirm:()=>{t.isArchived=!1,t.expenses&&(a.expenses=[...a.expenses,...t.expenses],delete t.expenses),t.tripDays&&(a.tripDays=[...a.tripDays,...t.tripDays],delete t.tripDays),a.trips.push(t),a.archivedTrips=a.archivedTrips.filter(t=>t.id!==e),a.activeTripId=e,S(`state:changed`),n(e),c(`home`)}})},T=e=>{d({title:`Delete Permanently?`,message:`This trip and all its memories will be gone forever.`,confirmText:`Delete`,onConfirm:async()=>{if(a.archivedTrips=a.archivedTrips.filter(t=>t.id!==e),S(`state:changed`),a.user)try{await fetch(x(`/api/trips/delete`),{method:`DELETE`,headers:{"Content-Type":`application/json`},body:JSON.stringify({user_id:a.user.id,trip_id:e})})}catch{}c(`collections`)}})};function E(n){let l=typeof n==`string`?a.archivedTrips.find(e=>e.id===n)||a.trips.find(e=>e.id===n):n,u=document.createElement(`div`);if(!l)return u.innerHTML=`<p style="padding: 40px; text-align: center;">Trip not found.</p>`,u;let b=(l.expenses||[]).filter(e=>!e.isSettlement),x=b.reduce((e,t)=>e+(t.euroValue||0),0),T=l.tripDays||[],E=T.length,D=Array.isArray(l.photos)?l.photos:[],O=Array.isArray(l.documents)?l.documents:[],k=T.reduce((e,t)=>e+(t.photos||[]).length,0)+D.length,A=T.reduce((e,t)=>e+(t.tickets||[]).length,0)+O.length,j=null;if(l.coverUrl&&(j=l.coverUrl),!j&&D.length>0&&(j=D[0].src),!j){for(let e of T)if(e.photos&&e.photos.length>0){j=e.photos[0];break}}let M=j?`background: linear-gradient(135deg, rgba(0,45,91,0.55), rgba(88,86,214,0.45)), url(${e(j)}) center/cover no-repeat;`:`background: linear-gradient(135deg, #007aff 0%, #5856d6 60%, #34c759 130%);`,N=`#ffffff`,P=`rgba(255,255,255,0.85)`,F=`rgba(255,255,255,0.16)`,I=`1px solid rgba(255,255,255,0.25)`,L=(t,n,r)=>`
    ...
    `;u.innerHTML=`
        <div class="archived-hero" style="position:relative; overflow:hidden; border-radius:36px; padding:48px 52px; ${M} box-shadow: 0 30px 80px rgba(0, 45, 91, 0.25); margin-bottom: 32px; border: 1px solid rgba(255,255,255,0.18);">
            ...
            <div style="position:absolute; top:24px; right:24px; display:flex; gap:8px; z-index:2;">
                ...
                <!-- Share button — now ALWAYS visible (no isPublic
                     ...
                     first. -->
                <button id="shareTripBtn" type="button" data-trip-id="${e(l.id)}" title="Share this trip" aria-label="Share this trip"
                ...
                <button id="cloneTripBtn" type="button" data-trip-id="${e(l.id)}" title="Start a new trip based on this one" aria-label="Clone this trip"
                    style="background:rgba(255,255,255,0.16); border:1px solid rgba(255,255,255,0.3); color:#ffffff; padding:10px 18px; border-radius:999px; font-weight:800; font-size:0.85rem; cursor:pointer; backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); display:inline-flex; align-items:center; gap:6px;">
                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                ...
                <button class="restore-trip-btn" data-trip-id="${e(l.id)}" type="button" style="background:#ffffff; color:#002d5b; padding:10px 18px; border-radius:999px; font-weight:800; font-size:0.85rem; cursor:pointer; box-shadow:0 4px 14px rgba(0,0,0,0.18); border: 0;">↺ Restore Trip</button>
            </div>
            ...
            <div style="position:relative; z-index:1; max-width: calc(100% - 260px);">
                ...
                <h1 style="font-size: 3.2rem; margin: 0; letter-spacing: -0.04em; color: ${N}; font-weight: 800; line-height: 1; text-shadow: 0 2px 24px rgba(0,0,0,0.2);">${e(l.name)}</h1>
                ${l.country?`<div style="margin-top:10px; font-size:1rem; color:${P}; font-weight:600; display:flex; align-items:center; gap:8px;">📍 ${e(l.country)}</div>`:``}
            </div>
        ...
        ${(()=>{let t=e=>{if(!e)return null;let t=T.find(t=>t.id===e);return t?Number(t.dayNumber)===0?`⚓ Anchor`:`Day ${t.dayNumber}`:null},n=e=>{if(!e)return!1;let t=T.find(t=>t.id===e);return!!t&&Number(t.dayNumber)===0},r=r=>{if(n(r))return`<span style="background:rgba(52,199,89,0.12); color:#1a6b3c; padding:2px 10px; border-radius:999px; font-size:0.65rem; font-weight:800; text-transform:uppercase; letter-spacing:0.06em;">⚓ Anchor</span>`;let i=t(r);return i?`<span style="background:rgba(0,113,227,0.08); color:var(--accent-blue); padding:2px 10px; border-radius:999px; font-size:0.65rem; font-weight:800; text-transform:uppercase; letter-spacing:0.06em;">${e(i)}</span>`:`<span style="background:rgba(0,0,0,0.05); color:rgba(0,0,0,0.45); padding:2px 10px; border-radius:999px; font-size:0.65rem; font-weight:800; text-transform:uppercase; letter-spacing:0.06em;">Unsorted</span>`},i=[];O.forEach(e=>i.push({name:e.name||`Document`,url:e.url||``,dayId:e.dayId||null,source:`trip`,_key:e.id||`${e.name}-${e.url}`})),T.forEach(e=>{(e.tickets||[]).forEach((t,n)=>i.push({name:t.name||`Document`,url:t.url||``,dayId:e.id,source:`day`,_key:`${e.id}#${n}`}))});let a=e=>{if(!e)return-1;let t=T.find(t=>t.id===e);return t?t.dayNumber:999};i.sort((e,t)=>a(e.dayId)-a(t.dayId));let o=[];D.forEach(e=>o.push({src:e.src||``,dayId:e.dayId||null,source:`trip`,_key:e.id||e.src})),T.forEach(e=>{(e.photos||[]).forEach((t,n)=>o.push({src:t,dayId:e.id,source:`day`,_key:`${e.id}#${n}`}))}),o.sort((e,t)=>a(e.dayId)-a(t.dayId));let s=e=>/^data:image\//i.test(e||``)||/\.(jpe?g|png|gif|webp|avif|heic|heif|bmp|tiff?|svg)(\?.*)?$/i.test(e||``);return(i.length===0?``:`
            ...
            `)})()}
    `,u.querySelector(`#backToCollectionsBtn`)?.addEventListener(`click`,()=>c(`collections`));let R=u.querySelector(`#shareTripBtn`);return R&&g(l.id).then(e=>{e?.shared&&(R.dataset.shared=`1`,R.dataset.postId=String(e.post_id),v(R,!0))}),u.addEventListener(`click`,async e=>{let n=e.target,u=n?.closest(`.restore-trip-btn`);if(u?.dataset.tripId){w(u.dataset.tripId);return}let g=n?.closest(`#cloneTripBtn`);if(g?.dataset.tripId){g.setAttribute(`disabled`,`true`);let e=g.innerHTML;g.innerHTML=`Cloning…`;try{let n=await o(g.dataset.tripId);if(!n?.ok||!n.body?.tripId){t(`Couldn't clone — try again in a moment.`),g.removeAttribute(`disabled`),g.innerHTML=e;return}await i(),a.activeTripId=n.body.tripId,S(`state:changed`),t(`Trip cloned! Edit your draft on Home.`),c(`home`)}catch(n){console.error(`Clone failed:`,n),t(`Couldn't clone — try again in a moment.`),g.removeAttribute(`disabled`),g.innerHTML=e}return}let _=n?.closest(`#shareTripBtn`);if(_){h({trip:l,onShareToFeed:()=>{if(_.dataset.shared===`1`){let e=Number(_.dataset.postId||0);if(!e)return;d({title:`Unshare this trip?`,message:`It'll disappear from your friends' feeds. Any reposts of it will be removed too.`,confirmText:`Unshare`,onConfirm:async()=>{let n=await s(e);if(!n||!n.ok){t(`Couldn't unshare — try again in a moment.`);return}_.dataset.shared=`0`,_.dataset.postId=``,v(_,!1),t(`Removed from your feed.`)}});return}y(l,async e=>{let n=await r(l.id,e);if(!n||!n.ok){t(`Couldn't share — try again in a moment.`);return}let i=Number(n.body?.post_id)||0;i&&(_.dataset.shared=`1`,_.dataset.postId=String(i),v(_,!0)),n.body?.status===`already_shared`?t(e?`Updated your share.`:`Already shared to your feed.`):t(`Shared to your feed.`)})}});return}let b=n?.closest(`a[href]`);if(b&&m(b.href)){let t=e;if(!t.metaKey&&!t.ctrlKey&&!t.shiftKey&&t.button!==1){t.preventDefault();let e=b.querySelector(`span`)?.textContent?.trim()||`Document`;p(b.href,e);return}}let x=n?.closest(`.archived-day-block`);if(x?.dataset.dayId){let e=(l.tripDays||[]).find(e=>e.id===x.dataset.dayId);e&&f(e);return}}),u.addEventListener(`change`,e=>{let t=e.target?.closest(`.trip-privacy-select`);t?.dataset.tripId&&C(t.dataset.tripId,t.value)}),u}var D=async e=>{let t=document.getElementById(`app-container`);if(!t)return;let n=a.archivedTrips.find(t=>t.id===e)||a.trips.find(t=>t.id===e);if(n){t.innerHTML=``,t.appendChild(E(n));return}t.innerHTML=`<div style="padding:60px 20px; text-align:center; color:var(--text-secondary); font-size:0.95rem;">${b(`collections.loadingTrip`)}</div>`;try{let n=await u(`/api/public-trip/${encodeURIComponent(e)}`);if(!n.ok){t.innerHTML=`<div style="padding:60px 20px; text-align:center; color:var(--text-secondary);">${b(`collections.tripUnavailable`)}</div>`;return}let r=await n.json();if(!r?.trip){t.innerHTML=`<div style="padding:60px 20px; text-align:center; color:var(--text-secondary);">${b(`collections.tripNotFound`)}</div>`;return}t.innerHTML=``,t.appendChild(E(r.trip))}catch(e){console.error(`viewArchivedDetails fetch failed:`,e),t.innerHTML=`<div style="padding:60px 20px; text-align:center; color:var(--text-secondary);">${b(`collections.loadFailed`)}</div>`}};export{C as i,T as n,w as r,D as t};
//# sourceMappingURL=collections-CvxEYynl.js.map