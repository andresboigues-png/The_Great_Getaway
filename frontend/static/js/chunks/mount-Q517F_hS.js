import{r as e,t}from"./vendor-react-CAxw18f3.js";import{At as n,C as r,D as i,Dt as a,Ht as o,L as s,N as c,Nt as l,Pt as u,_ as d,a as f,d as p,f as m,kt as h,p as g,r as _,w as v,y}from"../app.bundle.js";import{t as b}from"./collections-D5W_9_Nb.js";var x=e(),S=new Set([`friend_shared_trip`,`friend_reposted_trip`]),C=new Set([`friend_created_trip`,`friend_archived_trip`,`friend_joined_trip`,`new_friendship`]),w={muted:`142,142,147`,like:`255,59,48`,comment:`0,113,227`,repost:`52,199,89`,bookmark:`255,149,0`};function T(e){if(!e)return``;let t=typeof e==`string`&&e.includes(` `)&&!e.includes(`T`)?e.replace(` `,`T`)+`Z`:e,n=new Date(t);return Number.isNaN(n.getTime())?``:n.toISOString().slice(0,10)}function E(e){let t=new Map,n=[];for(let r of e){if(S.has(r.type)){n.push(r);continue}let e=`${r.actor?.id||`anon`}|${r.type}|${T(r.when)}`,i=t.get(e);i||(i=[],t.set(e,i),n.push({__slot:e})),i.push(r)}return n.map(e=>{let n=e.__slot;if(!n)return e;let r=t.get(n)||[];if(r.length===1)return r[0];let i=r[0];return{bundled:!0,id:`bundle_${n}`,type:i.type,actor:i.actor,when:i.when,members:r}})}function D(e){let t=`<strong style="color:#002d5b;">${n(e.actor.name)}</strong>`,r=e.members.length,i=r===1?`trip`:`trips`;switch(e.type){case`friend_created_trip`:return`${t} started planning <strong style="color:#002d5b;">${r} new ${i}</strong>`;case`friend_archived_trip`:return`${t} just completed <strong style="color:#002d5b;">${r} ${i}</strong> 🎉`;case`friend_joined_trip`:return`${t} joined <strong style="color:#002d5b;">${r} ${i}</strong>`;case`new_friendship`:return`You and <strong style="color:#002d5b;">${r} new people</strong> are now friends 🤝`;default:return`${t} did ${r} new things`}}function O(e,t=44){let r=(e?.name||`?`).charAt(0).toUpperCase(),i=`<div style="width:${t}px; height:${t}px; border-radius:50%; background: var(--gradient-day); color:white; display:flex; align-items:center; justify-content:center; font-weight:800; font-size:${Math.round(t*.4)}px; flex-shrink:0; box-shadow: 0 2px 8px rgba(0,113,227,0.18);">${n(r)}</div>`,a=e?.picture?`<img src="${n(e.picture)}" alt="" referrerpolicy="no-referrer"
    ...
    `}function P(e){let t=S.has(e.type),r=!!e.is_bookmarked,i=N({className:`feed-bookmark-btn`,accent:r?w.bookmark:w.muted,dataAttrs:`data-event-id="${n(e.id)}" data-bookmarked="${r?`1`:`0`}"`,title:r?`Remove bookmark`:`Bookmark`,svg:M(`bookmark`,r),marginLeftAuto:!0});if(!t)return`
        ...
        <div class="feed-actions" style="display:flex; align-items:center; gap:10px; margin-top: 10px; padding-top: 10px; border-top: 1px solid rgba(0,45,91,0.06);">
            ${N({className:`feed-like-btn`,accent:a?w.like:w.muted,dataAttrs:`data-event-id="${n(e.id)}" data-liked="${a?`1`:`0`}"`,title:a?`Unlike`:`Like`,svg:M(`heart`,a),count:o,countThreshold:3})}
            ${N({className:`feed-comment-btn`,accent:w.comment,dataAttrs:`data-event-id="${n(e.id)}"`,title:`Comments`,svg:M(`comment`),count:s})}
        ...
        <div class="feed-comment-row" data-comment-id="${e.id}" style="display:flex; align-items:flex-start; gap:10px; padding:8px 0; border-bottom:1px dashed rgba(0,45,91,0.06);">
            ...
            <div style="flex:1; min-width:0;">
                <div style="display:flex; align-items:baseline; gap:8px; flex-wrap:wrap;">
                    ...
                    <span style="font-size:0.7rem; color:var(--text-secondary); font-weight:600; text-transform:uppercase; letter-spacing:0.06em;">${n(k(e.when))}</span>
                </div>
    ...
    `}var I=new Set;function L(e){e.classList.remove(`tap-pop`),e.offsetWidth,e.classList.add(`tap-pop`),e.addEventListener(`animationend`,()=>{e.classList.remove(`tap-pop`)},{once:!0})}var R=[],z={},B=`posts`,V=!1,H=null,U=null;function W(e){return H!==null&&U===null?(e(),Promise.resolve()):U?U.then(e):(U=(async()=>{try{let e=await m();e.error?(console.warn(`explore fetch failed:`,e.error),H=[]):H=e.items||[]}finally{U=null,e()}})(),U)}function G(e,t,r){let i=o.user?.id;e.innerHTML=`
        <div class="feed-comment-list">${r.length>0?r.map(e=>F(e,e.author?.id===i)).join(``):`<div style="font-size:0.82rem; color:var(--text-secondary); padding:6px 0;">${_(`feed.commentsEmpty`)}</div>`}</div>
    ...
    `;let t=!1,s=e=>{let t=e.coverUrl?`background-image: url('${n(e.coverUrl)}'); background-size: cover; background-position: center;`:`background: linear-gradient(135deg, #00c7be 0%, #007aff 100%);`,r=e.owner.picture?`<img src="${n(e.owner.picture)}" alt="" referrerpolicy="no-referrer" style="width:24px; height:24px; border-radius:50%; object-fit:cover;">`:`<div style="width:24px; height:24px; border-radius:50%; background:rgba(0,113,227,0.18); display:flex; align-items:center; justify-content:center; color:#005bb8; font-size:0.7rem; font-weight:800;">${n((e.owner.firstName||`?`).slice(0,1).toUpperCase())}</div>`;return`
            <a class="card glass feed-explore-card" href="/share/${n(e.shareToken)}"
               style="display:block; text-decoration:none; color:inherit; padding:0; border-radius:18px; overflow:hidden; box-shadow:0 4px 14px rgba(0,45,91,0.06); border:1px solid rgba(0,199,190,0.18);">
                <div style="${t} height: 160px; position: relative;">
                    <div style="position:absolute; right:10px; top:10px; background:rgba(0,0,0,0.55); color:white; padding:4px 10px; border-radius:999px; font-size:0.72rem; font-weight:700; backdrop-filter: blur(8px);">
                        👁 ${n(String(e.shareViews))}
                    </div>
                </div>
                <div style="padding: 14px 16px;">
                    <div style="font-size:1.05rem; font-weight:800; color:var(--text-primary); letter-spacing:-0.02em; margin-bottom:4px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">${n(e.name)}</div>
                    <div style="font-size:0.82rem; color:var(--text-secondary); font-weight:600; margin-bottom:10px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">${n(e.country)}</div>
                    <div style="display:flex; align-items:center; gap:8px;">
                        ${r}
                        <span style="font-size:0.8rem; color:var(--text-secondary); font-weight:600;">by ${n(e.owner.firstName||`Traveller`)}</span>
                    </div>
                </div>
            </a>
        `},m=()=>{let r=l(e,`#feedList`);if(!r)return;if(B===`explore`){if(H===null){r.innerHTML=`
                ...
                `;return}if(H.length===0){r.innerHTML=h({accent:`blue`,emoji:`🌍`,title:`No public trips yet`,body:`Be the first — share one of your own to seed the Explore feed for everyone.`});return}r.innerHTML=`
                <div style="display:grid; grid-template-columns:repeat(auto-fill, minmax(260px, 1fr)); gap:14px;">
                    ${H.map(s).join(``)}
                </div>
            `;return}if(!t&&R.length===0){r.innerHTML=`
                <div class="card glass" style="padding: 32px; border-radius: 24px; text-align:center;">
            ...
            `;return}let i=e=>B===`posts`?S.has(e.type):C.has(e.type),a=R.filter(e=>!(!i(e)||V&&!e.is_bookmarked));if(a.length===0){let t,n,i,a;V?(t=_(B===`posts`?`feed.emptyBookmarkedPostsTitle`:`feed.emptyBookmarkedActionsTitle`),n=_(`feed.emptyBookmarkedBody`),i=_(`feed.emptyBookmarkedCta`),a=()=>{V=!1;let t=e.querySelector(`#feedBookmarkToggle .apple-toggle__input`);t&&(t.checked=!1),m()}):B===`posts`?(t=_(`feed.emptyPostsTitle`),n=_(`feed.emptyPostsBody`),i=_(`feed.emptyPostsCta`),a=()=>{B=`actions`,m(),e.querySelectorAll(`.home-tabnav__tab`).forEach(e=>e.classList.toggle(`is-active`,e.dataset.feedTab===`actions`))}):(t=_(`feed.emptyActionsTitle`),n=_(`feed.emptyActionsBody`),i=_(`feed.emptyActionsCta`),a=()=>c(`friends`)),r.innerHTML=h({accent:`purple`,emoji:V?`🔖`:`🌱`,title:t,body:n,ctaLabel:i,ctaId:`feedEmptyCtaBtn`});let o=r.querySelector(`#feedEmptyCtaBtn`);o&&(o.onclick=a);return}let u=o.user?.id;r.innerHTML=E(a).map(e=>{if(e.bundled){let t=e,r=j(t.type),i=k(t.when),a=I.has(t.id),o=t.members.map(e=>{let t=A(e),r=!!e.is_bookmarked;return`
                        <div class="feed-bundle-member" data-event-id="${n(e.id)}" style="display:flex; align-items:center; gap:10px; padding:8px 0; border-top:1px dashed rgba(0,45,91,0.06);">
                            ...
                            <button type="button" class="icon-btn-circle feed-bookmark-btn" style="--accent: ${r?w.bookmark:w.muted};" data-event-id="${n(e.id)}" data-bookmarked="${r?`1`:`0`}" title="${r?`Remove bookmark`:`Bookmark`}" aria-label="${r?`Remove bookmark`:`Bookmark`}">
                                ${M(`bookmark`,r)}
                            </button>
                        </div>
                    `}).join(``);return`
                ...
                `}let t=e,r=j(t.type),i=k(t.when),a=t.caption?`
            ...
            `:``,o=(t.type===`friend_shared_trip`||t.type===`friend_reposted_trip`)&&t.trip?.id?(()=>{let e=t.trip.country?n(t.trip.country):``;return`
                    <button type="button" class="feed-trip-card" data-trip-id="${n(t.trip.id)}"
                        ...
                        <div style="flex:1; min-width:0;">
                            <div style="font-weight:800; color:#002d5b; font-size:0.98rem; line-height:1.25; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">${n(t.trip.name||_(`feed.tripFallback`))}</div>
                ...
                `})():``,s=t.type===`friend_shared_trip`&&t.actor?.id===u&&t.post_id?`
            ...
            `:``;return`
                <div class="card glass feed-event" data-event-id="${n(t.id)}"
                    ...
                    <div style="display:flex; align-items:flex-start; gap:14px;">
                        ...
                        <div style="flex:1; min-width:0;">
                            <div style="font-size: 0.95rem; line-height:1.4; color: var(--text-secondary);">
                                <span style="margin-right:6px;">${r.icon}</span>${A(t)}
                            </div>
                            ${i?`<div style="font-size: 0.72rem; color: var(--text-secondary); margin-top: 4px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.06em;">${n(i)}</div>`:``}
                        </div>
                        ${s}
            ...
            `}).join(``)},x=async()=>{if(!o.user){t=!0,m();return}try{let e=await f(`/api/feed`);if(!e.ok)return;let t=await e.json();Array.isArray(t)&&(R=t)}catch(e){console.error(`Feed refresh failed:`,e)}finally{t=!0,m()}};e.querySelectorAll(`.home-tabnav__tab[data-feed-tab]`).forEach(t=>{t.onclick=()=>{let n=t.dataset.feedTab;!n||B===n||(B=n,e.querySelectorAll(`.home-tabnav__tab[data-feed-tab]`).forEach(e=>{e.classList.toggle(`is-active`,e.dataset.feedTab===n)}),n===`explore`&&W(m),m())}});let T=e.querySelector(`#feedBookmarkToggle .apple-toggle__input`);return T&&T.addEventListener(`change`,()=>{V=!!T.checked,m()}),e.addEventListener(`click`,async e=>{let t=e.target;if(!t)return;let n=t.closest(`.feed-avatar-btn`);if(n?.dataset.feedAvatarUserId){c(`profile`,{userId:n.dataset.feedAvatarUserId});return}let o=t.closest(`.feed-trip-card`);if(o?.dataset.tripId){b(o.dataset.tripId);return}let s=t.closest(`.feed-like-btn`);if(s?.dataset.eventId){let e=s.dataset.eventId,t=s.dataset.liked===`1`,n=!t,r=R.find(t=>t.id===e);r&&(r.is_liked=n,r.like_count=Math.max(0,(r.like_count||0)+(t?-1:1))),s.dataset.liked=n?`1`:`0`,s.style.setProperty(`--accent`,n?w.like:w.muted),s.innerHTML=M(`heart`,n),L(s);let i=s.parentElement?.querySelector(`.feed-action-count`),a=e=>e>=3?String(e):``;i&&r&&(i.textContent=a(r.like_count??0));let o=await v(e);o.ok&&o.body&&r&&(r.is_liked=!!o.body.liked,r.like_count=Number(o.body.count)||0,i&&(i.textContent=a(r.like_count)));return}let l=t.closest(`.feed-bookmark-btn`);if(l?.dataset.eventId){let e=l.dataset.eventId,t=l.dataset.bookmarked!==`1`,n=R.find(t=>t.id===e);n&&(n.is_bookmarked=t),l.dataset.bookmarked=t?`1`:`0`,l.style.setProperty(`--accent`,t?w.bookmark:w.muted),l.innerHTML=M(`bookmark`,t),L(l),V&&!t&&m(),await r(e);return}let d=t.closest(`.feed-bundle-toggle`);if(d?.dataset.bundleId){let e=d.dataset.bundleId;I.has(e)?I.delete(e):I.add(e),m();return}let f=t.closest(`.feed-comment-btn`);if(f?.dataset.eventId){let e=f.dataset.eventId,t=f.closest(`.feed-event`)?.querySelector(`.feed-thread`);if(!t)return;if(t.style.display!==`none`){t.style.display=`none`;return}t.style.display=`block`,z[e]?G(t,e,z[e]):(t.innerHTML=`<div style="font-size:0.82rem; color:var(--text-secondary); padding:6px 0;">${_(`feed.commentsLoading`)}</div>`,z[e]=await g(e)||[],G(t,e,z[e]));let n=t.querySelector(`input[name="body"]`);n&&n.focus();return}let h=t.closest(`.feed-comment-delete-btn`);if(h?.dataset.commentId){let e=Number(h.dataset.commentId),t=h.closest(`.feed-comment-row`),n=h.closest(`.feed-thread`),r=n?.dataset.eventId;t&&t.remove(),r&&z[r]&&(z[r]=z[r].filter(t=>t.id!==e));let i=r?R.find(e=>e.id===r):null;if(i){i.comment_count=Math.max(0,(i.comment_count||0)-1);let e=((n?.closest(`.feed-event`))?.querySelector(`.feed-comment-btn`))?.parentElement?.querySelector(`.feed-action-count`);e&&(e.textContent=i.comment_count>0?String(i.comment_count):``)}(await p(e)).ok||u(`Couldn't delete — try again in a moment.`);return}let S=t.closest(`.feed-unshare-btn`);if(S?.dataset.postId){let e=Number(S.dataset.postId);a({title:_(`feed.toastUnshareConfirmTitle`),message:_(`feed.toastUnshareConfirmMessage`),confirmText:_(`feed.toastUnshareConfirmBtn`),onConfirm:async()=>{let t=await i(e);if(!t||!t.ok){u(_(`feed.toastUnshareFailed`));return}await x(),u(_(`feed.toastRemovedFromFeed`))}});return}let C=t.closest(`.feed-repost-btn`);if(C?.dataset.postId){let e=Number(C.dataset.postId),t=C.style.getPropertyValue(`--accent`)||w.muted;C.disabled=!0,C.style.setProperty(`--accent`,w.muted);let n=await y(e);n.ok&&n.body?.status!==`same_user`?(u(n.body?.status===`already_reposted`?_(`feed.toastAlreadyReposted`):_(`feed.toastReposted`)),C.style.setProperty(`--accent`,w.repost),C.innerHTML=`<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="20 6 9 17 4 12"></polyline></svg>`,L(C)):n.body?.status===`same_user`?(C.disabled=!1,C.style.setProperty(`--accent`,t),u(_(`feed.toastRepostOwnShare`))):(C.disabled=!1,C.style.setProperty(`--accent`,t),u(_(`feed.toastRepostFailed`)));return}}),e.addEventListener(`submit`,async e=>{let t=e.target;if(!t?.classList?.contains(`feed-comment-form`))return;e.preventDefault();let n=t.dataset.eventId;if(!n)return;let r=t.querySelector(`input[name="body"]`),i=r?.value.trim();if(!i)return;let a=t.querySelector(`.feed-comment-submit`);r&&(r.value=``),a&&(a.disabled=!0);let o=await d(n,i);if(a&&(a.disabled=!1),!o.ok||!o.body?.comment){r&&(r.value=i),u(`Couldn't post comment — try again.`);return}let s=o.body.comment;z[n]||(z[n]=[]),z[n].push(s);let c=t.closest(`.feed-thread`);c&&G(c,n,z[n]);let l=c?.querySelector(`input[name="body"]`);l&&l.focus();let f=R.find(e=>e.id===n);if(f){f.comment_count=(f.comment_count||0)+1;let e=((c?.closest(`.feed-event`))?.querySelector(`.feed-comment-btn`))?.parentElement?.querySelector(`.feed-action-count`);e&&(e.textContent=String(f.comment_count))}}),m(),x(),e}var q=t();function J(){let e=(0,x.useRef)(null);return(0,x.useEffect)(()=>{let t=e.current;t&&(t.innerHTML=``,t.appendChild(K()))},[]),(0,q.jsx)(`div`,{ref:e})}function Y(e){s(e,(0,x.createElement)(J))}export{Y as mountFeed};
//# sourceMappingURL=mount-Q517F_hS.js.map